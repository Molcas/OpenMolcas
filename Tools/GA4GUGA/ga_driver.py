...
Examples:
  ...
  ga_driver.py --fcidump FCIDUMP_20site --norb 20 --fitness FAST_DIAG_MIN_OSONLY \\
    ...
    required.add_argument('--fcidump', type=str, required=True,
                          help='Path to the source FCIDUMP file')
    ...
    required.add_argument('--fitness', type=str, required=True,
                          ...
                          help='Fitness function: MIN_MAX_DIFF (maximize diff between highest/lowest CSF energies), '
                               'MAX_DIAG_ELEM (maximize highest CSF energy), MIN_DIAG_ELEM (minimize highest CSF energy), '
                               'DIAG_ELEM_SMS_MAPPING (max/min S-Ms consistent CSF energy), '
                               'FAST_DIAG_MIN_OSONLY (minimize S-Ms CSF energy, ordering-dependent terms only)')
    ...
    required.add_argument('--restricted-ordering-len', type=int, required=True,
                          help='Length of the effective (restricted) ordering that GA optimizes before expansion')

    # Optional arguments
    parser.add_argument('--crossover', type=str, default='order_co',
                        choices=['order_co', 'pmx_co', 'cycle_co'],
                        help='Crossover operator for GA simulation. order_co = order crossover (default: order_co)')
    ...
    parser.add_argument('--restart-file', type=str, default=None,
                        help='Restart GA from a saved population file')
    ...
    fitness_specific.add_argument('--ref-csf', type=parse_list_arg, default=None,
                        help='(For DIAG_ELEM_SMS_MAPPING/FAST_DIAG_MIN_OSONLY) '
                             'Reference S-Ms consistent CSF in natural ordering (step-vector notation: '
    ...
    if args.ref_csf is not None:
        kwargs['sms_ref_csf'] = args.ref_csf
