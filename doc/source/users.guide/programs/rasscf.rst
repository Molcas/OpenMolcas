...
.. xmldoc:: <MODULE NAME="RASSCF">
            ...
            SEWARD, and starting orbitals from either a previous SCF
            calculation (CAS, RAS, GAS or Stochastic-CAS) or from any of the other wave function
            generating programs, or from the GUESSORB facility.
...
In addition to the normal RASSCF dependencies, the Stochastic-CASSCF requires that the NECI program is installed externally
(the NECI program can also be embedded into OpenMolcas. This form of installation is howeven not fully developed and not suggested).
For the NECI program a parallel installation (using MPI) is assumed as well as the use of hdf5 libraries to store and process
the walker population. Using the FCIQMC algorithm the user will produce one- and two-body density matrix files (ONERDM, TwoRDM_XXXX), that are required for the subsequent orbital optimization step.
...
The Input and the FCIDUMP files are the only files necessary to NECI to run a FCIQMC simulation from scratch.
...
users are invited to use the dneci.x binary (this will run the FCIQMC dynamic in replica mode) :cite:`Overy2014`.
The FCIQMC dymanics can be followed in the fciqmc.out output file or in the NECI generated FCIMCStats file.
In the fciqmc.out there are important pieces of information, such as the list of Slater determinants dominating the FCI wave function and the RDM energy. The latter is passed to Molcas as shown in the script below.
When a stationary condition is reached and density matrices sampled these are passed to the &RASSCF program to continue.
...
:file:`$Project.TwoRDM_XXXX`
  ...
  to OpenMolcas to continue with the Stochastic-CASSSCF calculation.
...
Two input keywords are strictly required in the RASSCF mudule for activating the Stochastic-CASSCF:
...
:kword:`EXNE`
  This keyword enable the decoupled version of the Stochastic-CASSCF where Molcas and NECI are installed and run independently.

  .. xmldoc:: <KEYWORD MODULE="RASSCF" NAME="EXNE" KIND="SINGLE" LEVEL="ADVANCED" EXCLUSIVE="DMRG">
              ...
              calculations, where OpenMolcas and NECI are installed and run independently.
              </HELP>
              </KEYWORD>
...
:kword:`DMPO`
  ...
  .. xmldoc:: <KEYWORD MODULE="RASSCF" NAME="DMPO" KIND="SINGLE" LEVEL="ADVANCED" EXCLUSIVE="DMRG">
              %%Keyword: DMPO <advanced>
              <HELP>
              This keyword is used in the context of the Stochastic-CASSCF method
              to produce a FCIDUMP file in a ASCII format that can be recogniced by
              the NECI program and quit OpenMolcas in a clean way (no CI or CASSCF calculation will be done).
              </HELP>
              </KEYWORD>
...
:kword:`DEFD`
  ...
  provided, the more populated determinant is used are reference, overwriting the user choice. Possible changes of reference
  determinants can be tracked in the FCIQMC output file. An example of the DEFD keyword follows::
...
It contains a list of the occupied spin-orbitals in the order given by the INPORB file (space symmetry sorted).
  ...
  .. xmldoc:: <KEYWORD MODULE="RASSCF" NAME="DEFD" KIND="SINGLE" LEVEL="ADVANCED" EXCLUSIVE="DMRG">
              %%Keyword: DEFD <advanced>
              <HELP>
              This keyword is used in the context of the Stochastic-CASSCF method
              to provide the FCIQMC algorithm with an initial guess for the reference Slater determinant.
              It is followed (new line) by a string of integer representing spin-orbitals in the order
...
:kword:`DMRG`
  ...
  .. xmldoc:: <KEYWORD MODULE="RASSCF" NAME="DMRG" LEVEL="BASIC" APPEAR="Number of DMRG renormalized states" KIND="INT" DEFAULT_VALUE="0">
              %%Keyword: DMRG <basic>
              <HELP>
