...
:kword:`WRITEGrid`
  Write the final numerical quadrature grid to :file:`$Project.GridFile`, where
  ``$Project`` is the project name derived from the input filename. Each line
  contains the three Cartesian coordinates of a grid point followed by its
  quadrature weight. The file is generated when a program, such as
  :program:`KSDFT` or :program:`MCPDFT`, performs numerical quadrature; running
  :program:`SEWARD` alone only stores the grid settings and does not create the
  file. After the quadrature calculation, :program:`pymolcas` automatically
  copies the file from the working directory to the input directory, or to the
  configured output directory. By default, the grid is not written.

  .. xmldoc:: <KEYWORD MODULE="SEWARD" NAME="WRITEGRID" APPEAR="Write numerical grid" KIND="SINGLE" LEVEL="ADVANCED">
              %%Keyword: WriteGrid (NQ) <advanced>
              <HELP>
              Write the final numerical quadrature grid to $Project.GridFile.
              Each line contains three Cartesian coordinates and the
              quadrature weight. The file is generated by a program that
              performs numerical quadrature and is copied from the working
              directory to the input or configured output directory.
              </HELP>
              </KEYWORD>
