...
:file:`NTORB` and `MD_NTO`
  This output is only created if :kword:`NTOCalc` is given in the input. The files
  will contain natural transition orbitals in INPORB (`NTORB`) and Molden (`MD_NTO`) formats.
