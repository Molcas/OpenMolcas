...
if (.not. SameLx) then
  call DGEMM_('N','T',nSsh(iSymB),nSsh(iSymA),numV,1.0d0,Ly0,nSsh(iSymB),Lx0,nSsh(iSymA),1.0d0,AddEx,nSsh(iSymB))
else
  call DGEMM_('N','T',nSsh(iSymA),nSsh(iSymA),numV,1.0d0,Lx0,nSsh(iSymA),Lx0,nSsh(iSymA),1.0d0,AddEx,nSsh(iSymA))
end if
