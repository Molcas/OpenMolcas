...
# endif
  ...
  ! For small matrices the overhead of a BLAS call (argument checking,
  ...
  if ((m_ > 0) .and. (n_ > 0) .and. (k_ > 0) .and. (m_*n_*k_ <= mnkSmall)) then
    ...
    NoTrB = (transb == 'N') .or. (transb == 'n')
    if (NoTrA .and. NoTrB) then
      do j=1,n_
        do l=1,k_
          tmp = alpha*b(l,j)
      ...
      do j=1,n_
        do i=1,m_
          c(i,j) = c(i,j)+alpha*sum(a(1:k_,i)*b(1:k_,j))
        end do
      end do
    else
      do j=1,n_
        do i=1,m_
          c(i,j) = c(i,j)+alpha*sum(a(1:k_,i)*b(j,1:k_))
        end do
      end do
    end if
    return
