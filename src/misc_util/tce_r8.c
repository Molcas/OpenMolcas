...
/**************************************************************************/
/*                                                                        */
/* This routine performs packing by truncation of insignificant bytes in  */
/* the numbers.                                                           */
/*                                                                        */
/*------------------------------------------------------------------------*/
/*                                                                        */
...
#ifdef _CAPITALS_
#define tce_r8 TCE_R8
#define one_ulp ONE_ULP
#else
#ifndef ADD_
#define tce_r8 tce_r8_
...
/* marked as volatile to prevent over-optimization */
   ...
   INT i,k,n,skip,keep;
   if(*Init_do_setup_e == 1) do_setup=1;
   if(do_setup) {
      do_setup=0;
      one_ulp(&ulp);
...
/* ptr_64=(unsigned long long int *)out; */
   ...
   for(k=0; k<*n_in; k++) {
      ...
      keep=tab[ptr_16[IND]];
#ifdef _BIG_ENDIAN_
      for(i=0; i<keep; i++) out[n++]=ptr_8[i];
#else
      for(i=0; i<keep; i++) out[n++]=ptr_8[7-i];
#endif
   }
