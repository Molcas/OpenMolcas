...
do i=1,nOper
  ...
  if (i == 3) jOper = 5
  RotVec(1) = One
  if (iand(iOper(jOper),1) == 1) RotVec(1) = -One
  RotVec(2) = One
  if (iand(iOper(jOper),2) == 2) RotVec(2) = -One
  RotVec(3) = One
  if (iand(iOper(jOper),4) == 4) RotVec(3) = -One
  mCenter = nCenter
  do iAt=0,mCenter-1
    ...
    do jAt=0,nCenter-1
      if (Byte4 == AtomLbl(Lw2+jAt)) then
        ...
        if ((Xnew == Xold2) .and. (Ynew == Yold2) .and. (Znew == Zold2)) Go To 999
      end if
    end do
    nCenter = nCenter+1
