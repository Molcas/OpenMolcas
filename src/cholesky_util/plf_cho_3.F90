...
use Cholesky, only: iShlSO, iShP2Q, iShP2RS, iSOShl, LuPri, nBstSh, nDim_Batch, ShA, ShAB, ShB, ShC, ShCD, ShD
...
if (jPrint >= 99) call RecPrt(' In Plf_Cho_3: AOInt',' ',AOInt,ijkl,iCmp*jCmp*kCmp*lCmp)
            ...
            do jSOj=jSO,jSO+jBas-1
              do iSOi=iSO,iSO+iBas-1
                ...
                if ((ISHLI == SHC) .and. (ISHLJ == SHD) .and. (ISHLK == SHA) .and. (ISHLL == SHB)) then
                  ...
                  call CHO_QUIT('Logical error in PLF_Cho_3',103)
                end if
