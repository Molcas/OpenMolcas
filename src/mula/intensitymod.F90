...
!       DipMat = 0.0d0
     ...
     &   nDimTot,nPolyTerm,ndata,nvar,MaxNumAt)
      ...
      End Do
      End If
!!  max_mOrd = nDimTot-1 ; max_nOrd = nDimTot-1
!!  If ( max_mQuanta.eq.0 ) max_mOrd = 0
!!  If ( max_nQuanta.eq.0 ) max_nOrd = 0
      Do k = 1,2
      Do m = 1,ndimtot
      Do n = 1,ndimtot
      sum = 0.0d0
...
!! Do iOrd = 0,max_mOrd
!!    level2 = mMat(iOrd,:)
!!    FreqDiffMat(iOrd) = TransEnergy(0.0d0,x_anharm1,harmfreq1,level1, &
!!                                    0.0d0,x_anharm1,harmfreq1,level2)
!! End Do
!!
