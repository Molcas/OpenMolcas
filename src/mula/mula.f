...
!!
!!----      Calculate the expansion point geometry and save the full
...
!!
!!----      If we only wanted the expansion point geometry, its time to quit now.
      If ( lExpan ) Then
      Write(6,*) 'Bye bye'
      Write(6,*)
...
!!-----------------------------------------------------------------------!
!!
!!      Transform transition dipole gradients from cartesian to
...
!GGt          lMBatch = 131072*8                       ! CGGt Test only !!!!
...
!!
     ...
     &                           CPTF6-CPTF0, TIOTF6-TIOTF0
        ...
        lISC = .False.
!      Call GetMem('Test_F','LIST','INTE',iDum,iDum)               ! CGGt
        GoTo 999
...
!!-----------------------------------------------------------------------!
!!
!!----      Calculate dimensions given max level of excitation for the
!!      different states.
      Call TabDim2_drv(m_max,nOsc,nvTabDim)
      mTabDim = nvTabDim-1
      Call TabDim2_drv(n_max,nOsc,nvTabDim)
...
!!-----------------------------------------------------------------------!
!!
!!      Calculate vibrational Hessian if a variational calculation
!!      is chosen
!!
!!-----------------------------------------------------------------------!
!!
