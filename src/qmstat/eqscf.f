...
!***********************************************************************
      Subroutine EqScf(iQ_Atoms,nAtomsCC,Coord,nBas,nBas_C,nCnC_C       &
     &                ,iSupDeAll,iV1DeAll)
...
!     Parameter (BoltzK=1.0d-3*CONST_BOLTZMANN_/CONV_AU_TO_KJ_)
      ...
      Dimension Smat(MxOT),Vmat(MxOT),BoMaH(MxAt),BoMaO(MxAt)
...
!****
      ...
      Character MemQFal*20,Head*200,ChCo*2,ChCo2*2
      Logical DidWeAccept,Haveri,CalledBefore,SampleThis,InCutOff
...
!----------------------------------------------------------------------*
      ...
      delR=delR/Conver1        !angstrom-->Bohr
      CalledBefore=.false.
      SampleThis=.true.
      iSupDeAll=iSupM
      iV1DeAll=iV1
...
!--------------------------------------------------------------------------*
     ...
     &//'o run production!'
          ...
          Write(6,*)'   If you wish to make such large samples, you can'&
     &//' run several samplings and collect several sampfiles.'
...
!*********
      Else
        Write(6,*)
        Write(6,*)'Error 1 in classical subroutine.'
        Call Quit(_RC_INTERNAL_ERROR_)
      Endif
!----------------------------------------------------------------------*
...
!
          Call ExScf(iCStart,nBaseQ,nBaseC,nCnC_C,iQ_Atoms              &
...
!-- Polarize system.
!
          Call PolScf(iDist,iDistIm,iDT,iFI,iFP,iFil,iCStart,iTri       &
     &               ,VMat,Smat,DiFac,Ract,icnum,energy,NVarv           &
     &               ,iMOC,Haveri,iQ_Atoms,ip_ExpVal,Poli)
...
!
          Call BoostRep(AddRep,SmatPure,iMOC,iOrb(1),InCutOff)
...
!--------------------------------------------------------------------------*
          If(iPrint.ge.10) then
            ...
            Write(6,*)'         Of which is'
            Write(6,*)'            Pairwise solvent-solvent '           &
     &//'interaction:',EnCLAS
     ...
     &',',-xyzMyQ(2),',',-xyzMyQ(3),')'
            Write(6,*)'         Radie:',Ract
            If(Haveri) then
              Write(6,*)'    WARNING! SOME OF THE NUMBERS ABOVE HAVE NO'&
     &//' MEANING SINCE THE POLARIZATION DID NOT CONVERGE!!!'
              GoTo 8194
            Endif
          Endif
...
!   data on the extract file. For centre specific stuff, call good-old
!   Allen for them.
!
          If(iRead.eq.9) then
            If(lExtr(6)) then
...
6348            Continue
            ...
            Endif
            If(lExtr(7)) then
              Call AllenGinsberg('SCF  ',Eint,Poli,ChaNuc,Cha,DipMy     &
     &                          ,Quad,MxOT,iMOC,iOrb(1),iExtr_Atm       &
     &                          ,.false.,iOcc1,iQ_Atoms                 &
     &                          ,ip_ExpCento,E_Nuc_Part,lSlater         &
     &                          ,Eint_Nuc)

            Endif
            Call Extract(iLuExtr,i9,Etot,xyzMyQ,FockM,iMOC,iDt,iOrb(1)  &
     &                  ,Dum,xyzQuQ,ip_ExpVal,ip_ExpCento               &
     &                  ,E_Nuc_Rubbet,E_Nuc_Part)
!***JoseMEP**********
      ! If MEP option. Add electr. potential, field, etc for
      ! all solvent config.
            If(lExtr(8)) then
              Labjhr='Add '
...
!----------------------------------------------------------------------*
          If(QmProd) then !<-- Are we producing.
            If(SampleThis) then !<-- Should we sample this (relevant
                                !when paralleltempering is used).
              If(Inter.ne.0) then
