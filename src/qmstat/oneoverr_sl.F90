...
subroutine OneOverR_Sl(iFil,Ax,Ay,Az,BoMaH,BoMaO,EEDisp,iCNum,Eint,iQ_Atoms,outxyz,Eint_Nuc)
!----------------------------------------------------------------------*
...
do k=1,iCi
  ...
  do j=iCnum+1,nPart
    ...
    ! centers are ignored.                                             *
    !------------------------------------------------------------------*
    if (lAtom) call DispEnergy(EEDisp,BoMah,BoMaO,rg1,rg2,rg3,Rab13i,Rab23i,Rab33i,k)
    Ux1 = RabX1*s1i
    Uy1 = RabY1*s1i
    Uz1 = RabZ1*s1i
    Ux2 = RabX2*s2i
    ...
    if (distMin <= Cut_Elc) then
      ...
      if (lQuad) then
        ...
        call Sl_Grad(nSlSiteC,lMltSlC,CTemp,DTemp,DInvTemp,SlExpC,SlFactC,SlPC,nMltTemp,SlExpQ(1,k),DifSlExp,EintSl,EintSl_Nuc, &
                     lAtom)
        ...
        ijhr = min(nMltTemp,1)
        call Sl_Grad(nSlSiteC,lMltSlC,CTemp,DTemp,DInvTemp,SlExpC,SlFactC,SlPC,ijhr,SlExpQ(1,k),DifSlExp,EintSl,EintSl_Nuc,lAtom)
