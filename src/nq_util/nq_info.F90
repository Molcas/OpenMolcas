...
integer, parameter :: Other_Type = 0, LDA_Type = 1, GGA_Type = 2, meta_GGA_Type1 = 3, meta_GGA_Type2 = 4
integer Grid_Type, Fixed_Grid, Moving_Grid
parameter(Fixed_Grid=0,Moving_Grid=1)
integer Angular_Prunning, On, Off, Rotational_Invariance, Functional_Type
integer Packing
parameter(On=1,Off=0)
integer iQStrt, NASHT, NASHT4, NPOt1, nOrbt, nPot2, maxUVX, ndc, nUVXt, nVXt, nAngularGrids, L_Quad_save, nR_Save, nx, ny, nz, &
        number_of_subblocks, ip_nR_Eff, ip_R, ipMem, nMem, L_Quad, nR, nAtoms, nMaxExp, nTotGP, nbrmxbas, iAngMax, ip_ioffsh, &
        iOpt_Angular, mIrrep, mTmp, mRad, nAOMax, NQ_Direct, iQEnd, ip_OrbDip(3)
common/Quad_i/iQStrt,NASHT,NASHT4,NPot1,nOrbt,nPot2,maxUVX,ndc,nAngularGrids,L_Quad_save,nR_Save,Angular_Prunning,nx,ny,nz, &
              number_of_subblocks,ip_nR_Eff,ip_R,ipMem,nMem,L_Quad,nR,nAtoms,nMaxExp,nTotGP,nbrmxbas,iAngMax,ip_ioffsh, &
...
real*8 rQStrt, Threshold_save, Crowding, Threshold, Energy_integrated, Dens_I, Grad_I, Tau_I, Dens_a1, Dens_b1, Dens_a2, Dens_b2, &
       Dens_t1, Dens_t2, Block_Size, x_min, x_max, y_min, y_max, z_min, z_max, Fade, ThrC, T_Y, rQEnd
...
common/Quad_c/cQStrt,Quadrature,Pad,MBC,cQEnd
