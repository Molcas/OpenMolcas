...
#     endif
      if (R2_Trial(iNQ) > RMax**2) Go To 99
      nExpTmp = nExpTmp+1
    end do  ! iExp
99  continue
    if (nExpTmp /= 0) then
      ...
      list_bas(1,ilist_s) = nBas_Eff(NrExp,NrBas,Shells(iShll)%Exp,Shells(iShll)%pCff,list_exp(ilist_s))
#     ifdef _ANALYSIS_
...
if (Do_Grad .and. (nGrad_Eff == 0)) Go To 998
if (Grid_Status == Use_Old) Go To 997
...
# endif
  ...
  nTotGP_Save = nTotGP
  call Subblock(iNQ,x_NQ,y_NQ,z_NQ,InBox(iNQ),x_min_,x_max_,y_min_,y_max_,z_min_,z_max_,list_p,nlist_p,Grid,Weights,mGrid,.true., &
...
if (nogp+number_of_grid_points <= mGrid) then
  ...
  More_to_Come = .false.
  if ((.not. Do_Grad) .and. (iBatch /= nBatch)) Go To 888
else
  More_to_Come = .true.
end if
...
call Do_Batch(Kernel,Func,nogp,list_s,nlist_s,List_Exp,List_Bas,Index,nIndex,FckInt,nFckDim,nFckInt,mAO,nD,nP2_ontop,Do_Mo,TabMO, &
              TabSO,nMOs,Do_Grad,Grad,nGrad,mdRho_dR,nGrad_Eff,iNQ,EG_OT,nTmpPUVX,PDFTPot1,PDFTFocI,PDFTFocA)
