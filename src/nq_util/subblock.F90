...
if (R_Value_Min > R_box_Max) Go To 8887
R_Value_Max = NQ_Data(iNQ)%R_Quad(1,iR_Start)
if (R_Value_Max < R_box_Min) Go To 8887
...
# endif
  ...
  if ((.not. InBox) .and. (.not. Check(iOpt_Angular,2))) then
    ...
    if ((x_a(iPoint,iset) < xyz0(1,1)) .or. (x_a(iPoint,iset) > xyz0(1,2)) .or. (y_a(iPoint,iset) < xyz0(2,1)) .or. &
        (y_a(iPoint,iset) > xyz0(2,2)) .or. (z_a(iPoint,iset) < xyz0(3,1)) .or. (z_a(iPoint,iset) > xyz0(3,2))) Go To 7777
  end if
  !                                                                    *
  !*********************************************************************
  !                                                                    *
...
8887 continue
iR_Start = iR_End-1
if ((Angular_Prunning == On) .and. (iR_End /= iEnd_R)) Go To 999
!                                                                      *
!***********************************************************************
!                                                                      *
! Generate weights
