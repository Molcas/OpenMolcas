...
real(kind=wp) :: d_ac, d_ad, d_bc, d_bd, diffnorm, Q_aa, Q_ab, Q_ac, Q_ad, Q_bb, Q_bc, Q_bd, Q_cc, Q_dd, sumOff
        ...
        if (cd >= ab) then
          do iAtom=1,nAtoms
            ...
            Hessian(ab,cd) = Hessian(ab,cd)+Eight*Q_ab*(d_ac*Q_ad-d_ad*Q_ac-d_bc*Q_bd+d_bd*Q_bc)+ &
                             Two*d_ac*Q_bd*(Two*Q_aa-Q_bb-Q_dd)-Two*d_ad*Q_bc*(Two*Q_aa-Q_bb-Q_cc)- &
                             Two*d_bc*Q_ad*(Two*Q_bb-Q_aa-Q_dd)+Two*d_bd*Q_ac*(Two*Q_bb-Q_aa-Q_cc)
          end do
...
if (DebugHess) then
  ...
  diffnorm = sqrt(DDot_(fsdim*2,NumHess-Hessian,1,NumHess-Hessian,1))/real(fsdim,kind=wp)**2
