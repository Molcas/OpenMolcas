...
integer(kind=iwp) :: i, iFirst, iLast, j, k, l, mDiis, nExplicit
real(kind=wp) :: Cpu1, Cpu2, gg, norm, thr, Tim1, Tim2, Tim3
#ifdef _DEBUGPRINT_
...
! find the list indices corresponding to the first and last data point
...
coords(:,1:iLast-iFirst+1) = DispList(:,iFirst:iLast)
grads(:,1:iLast-iFirst+1) = GradList(:,iFirst:iLast)
...
#   endif
  ...
  !number of subspace basis vectors, potentially linear dependent => difference vecs of ndiis displacements and gradients
  !+2 additional vecs (see below)
...
!if (SORANGE) SOFact = One * SOFact ! Dummy statement
...
call GEK_Optimizer(mDiis,nDiis,Max_IterGEK,q_diis,g_diis,dq_diis,FuncList(iFirst:),H_diis,dqdq,Step_Trunc,UpMeth,SOFact,bias)
...
write(u6,'(A,F12.6,2X,A,F12.3,2x,A,I4)') 'Angle(dq_NR,dq) (deg) =', &
  acos(dot_product(dq_NR_diis,dq_diis)/(norm_dq_NR*norm_dq))/Pi*180.0_wp,'norm(dq)/norm(dq_NR) = ', &
  norm_dq/norm_dq_NR,'IterGEK=',IterGEK
...
write(u6,*) 'FULL SPACE'
write(u6,'(A,F12.6,2X,A,F12.3,2x,A,I4)') 'Angle(dq_NR,dq) (deg) =',acos(dot_product(dq_NR,dq)/(norm_dq_NR*norm_dq))/Pi*180.0_wp, &
...
subroutine moveref()
  ...
  coords(:,1:ndiis) = CoordsAbs(:,1:ndiis)
...
subroutine sizecontrol()
  ...
  do i=1,fsdim
    do j=1,ndiis
      if (coords(i,j) > 0.1_wp) write(u6,*) 'coords(i,j) too big i,j =',i,j,coords(i,j)
    end do
  end do
...
subroutine getUtot()
  ...
  integer(kind=iwp) :: I
  real(kind=wp), allocatable :: aux_mat(:,:), disp_summed(:), kappa_summed(:,:), Umat_i(:,:), UmatKsum(:,:), UmatProd(:,:), &
                                xUmatProd(:,:)
...
!call expkap_localisation(kappa_summed,nOrb2Loc,Umat_i,xUmatProd,UmatKsum)
  call expkap_localisation(kappa_summed,nOrb2Loc,UmatKsum,xUmatProd)
