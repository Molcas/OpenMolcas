...
case (2,3,4,5)
    ...
    call GetNumGrad()
    call GetNumHess(CMO,nOrb2Loc,nBasis,nAtoms,fsdim,NumHessSymm,.true.,nBas_Start,nBas_per_Atom)
end select
...
do while ((nIter < nMxIter) .and. (.not. Converged))
    ...
    case (2,3,4,5) ! Employing NxN rotations
        ...
        call GetNumHess(CMO,nOrb2Loc,nBasis,nAtoms,fsdim,NumHessSymm,.false.,nBas_Start,nBas_per_Atom)
