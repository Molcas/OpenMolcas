...
use Constants, only: Zero, Two, Eight, Twelve
use Definitions, only: wp, iwp, u6

implicit none
integer(kind=iwp), intent(in) :: nOrb2Loc, nBasis, fsdim
...
real(kind=wp) :: dx, fm2dx, fmdx, fp2dx, fpdx, fref, gradNorm
real(kind=wp), allocatable :: diff(:), DispMat(:,:), gref(:), infDisp(:), infUmat(:,:), PA(:,:,:), rotated_CMO(:,:)
...
do i=1,fsdim
  ...
  select case (NumGradMeth)
    ...
    case (fourpoint)
      ...
      ! compute numerical Gradient
      NumGrad(i) = (Eight*(fpdx-fmdx)-fp2dx+fm2dx)/(Twelve*dx)
