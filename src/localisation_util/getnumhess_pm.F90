...
use Constants, only: Zero, Two, Eight, Twelve, Half
...
logical(kind=iwp), intent(in) :: debug2
integer(kind=iwp) :: i, k, l, npos, NumHessMeth
real(kind=wp) :: dx, GradNorm
real(kind=wp), allocatable :: diff(:), DispMat(:,:), gm2dx(:), gmdx(:), gp2dx(:), gpdx(:), gref(:), href(:), infDisp(:), &
                              NumHdiag(:), NumHess(:,:), oldPA(:,:,:), PA(:,:,:), rotated_CMO(:,:), Umat(:,:)
integer(kind=iwp), parameter :: fourpoint = 1, symm = 2, asymm = 3
logical(kind=iwp), parameter :: debug = .false.

call mma_allocate(PA,nOrb2Loc,nOrb2Loc,nAtoms,Label='PA')
...
call GetHdiag_PM(nAtoms,nOrb2Loc,PA,href,npos,gradnorm,.false.) ! don't modify it
...
do i=1,fsdim
  ...
  select case (NumHessMeth)
    ...
    case (fourpoint)
      ...
      ! compute numerical hessian columnwise
      NumHess(:,i) = (Eight*(gpdx-gmdx)-gp2dx+gm2dx)/(Twelve*dx)
...
do k=1,fsdim
  do l=k+1,fsdim
    NumHessSymm(k,l) = Half*(NumHess(k,l)+NumHess(l,k))
    NumHessSymm(l,k) = NumHess(k,l)
  end do
