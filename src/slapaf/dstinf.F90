...
use Slapaf_Info, only: Cx, Coor, Weights, Energy, MF, qInt, dqInt, Dmp_Slapaf, AtomLbl
use Slapaf_Parameters, only: MaxItr, iOptC, Numerical, Max_Center, mTROld, RtRnc, lOld_Implicit, stop, iter
...
logical do_printcoords, do_fullprintcoords, Just_Frequencies, Found
...
do isAtom=1,size(Coor,2)+nsAtom_p
  ...
  do iIrrep=0,nIrrep-1
    ...
    if (nTemp > nIrrep*(size(Coor,2)+nsAtom_p)) then
      ...
      write(6,*) 'nTemp > nIrrep*SIZE(Coor,2)'
      call Abend()
    end if
    CC(1,nTemp) = x2
    CC(2,nTemp) = y2
...
if (do_printcoords) then
  ...
  if (do_fullprintcoords) then
    if (nTemp >= 2) call Dstncs(LblTMP,CC,nTemp,angstr,Max_Center,5)
...
if (.not. Numerical) then
  ...
  do i=1,nCoord
    write(Lu_xyz,'(A2,3F15.8)') Element(i),(Angstr*xyz(j,i),j=1,3)
  end do
  close(Lu_xyz)
