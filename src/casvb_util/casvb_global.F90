...
integer, parameter :: lbuf = 512, mxact_mo = 16, mxdep = 200, mxfield = 500, mxfiles = 40, mxfrag = 10, mxI = 20, mxirrep_mo = 8, &
                      mxMs = 20, mxobj = 100, mxopth = 10, mxprm = 100, mxS = 20, mxstep = 200, nspinb = 7, nstackrep = 50

integer :: i_dep_on_j(mxdep), i2s_fr(mxS,mxfrag), iact_mo(mxact_mo), iaddr, iaddrm(mxfield), iastr_fr(mxfrag), ibstr_fr(mxfrag), &
           ibuf, ibuffer(lbuf), icase6, icase7, icnt, icode(mxstep), idan(mxfiles), idd(9), ifield, ifollow, ifsc_fr(mxfrag), &
           igrad, iline = 0, ilv(300), inp, inputmode, ioff_i, ioff_r, ioffs(mxobj+1), iopt2step(0:30), ioptcode(30), ioptim, &
           ioptstep, ip, ipAnchr, ipdd, ipos(mxstep), ipp, ipp12e, ipp12s, ipp7, iprint, iprm, iroot, is_set = 0, isaddle, &
           isaddledd, istackrep(nstackrep), iter, iter12e, iter12s, iter7, ivrhs, ix(11), izbuffer(lbuf), j_dep_on_i(mxdep), &
           joffs(mxobj+1), joptstep, jroot, lenline, loopstep, loopstepmx, lstprm(mxprm), maxd, mnion_fr(mxfrag), mxdav, &
           mxion_fr(mxfrag), mxit, mxrhs, n_div, nact_mo, nalf_fr(mxMs,mxfrag), nbas_mo, nbasf_mo(mxirrep_mo), &
           nbasi_mo(mxirrep_mo), nbasisq_mo, nbassqf_mo(mxirrep_mo), nbassqi_mo(mxirrep_mo), nbet_fr(mxMs,mxfrag), nbuf, ncnt, &
           nconf_fr(mxfrag), nconfion_fr(0:mxI,mxfrag), nda_fr(mxMs,mxfrag), ndb_fr(mxMs,mxfrag), ndep_ij, ndep_ji, &
           ndetvb_fr(mxfrag), ndetvb2_fr(mxfrag), nel_fr(mxfrag), nfield, nfieldm, nfold, nfrag, nfrdim, nline = 0, nlold = 0, &
           nMs_fr(mxfrag), nobj, nopth1(2), nopth2(2), noptim, noptstep, nortiter, nortiterdd, nparm, nroot, nS_fr(mxfrag), &
           nsym_mo, nvb_fr(mxfrag), nvbr_fr(mxfrag), nvecmx, nvguess, nvrestart, nvrhs, nvtot, nword
real*8 :: aa1, aa2, alftol, cnrm, cnrmtol, corenrg, cpropt(mxopth), cvbnrm_fr(mxfrag), delopth1(2), delopth2(2), dfx(6), &
          dfxmin(2), dfxtol, dx(3,6), eigwrngtol, exp12tol, expct, f1, f2, f3, f4, file_id, fxbest, grd(3,6), grdwrngtol, hh, &
          hhaccfac(5,2), hhkeep, hhmax(2), hhopt(mxopth), hhrejfac(2), hhstart, hhtol(2), oaa2, oaa3, orththr, orththrdd, &
          ovraa_try, ovrab, ovrab_try, recn, resthr, resthrdd, safety, sgn(6), signtol, singul(3), ww, ww_try, zzacclim(4,2), &
...
logical :: endwhenclose, follow, have_solved_it, maxize, memdebug, mustdeclare, ndres_ok, release(10), scalesmall(2), up2date(mxobj)

integer, parameter :: iunset = -1357924680
