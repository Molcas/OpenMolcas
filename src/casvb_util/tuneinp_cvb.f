...
c 'ENDTUNE' or 'END'
      ...
      if(istr.eq.1)then
c  'CNRMTOL'
        call real_cvb(cnrmtol,1,nread,1)
      elseif(istr.eq.2)then
c  'SAFETY'
        call real_cvb(safety,1,nread,1)
      elseif(istr.eq.3)then
c  'SIGNTOL'
        call real_cvb(signtol,1,nread,1)
      elseif(istr.eq.4)then
c  'ALFTOL'
        call real_cvb(alftol,1,nread,1)
      elseif(istr.eq.5)then
c  'DFXTOL'
        call real_cvb(dfxtol,1,nread,1)
      elseif(istr.eq.6)then
c  'EXP12TOL'
        call real_cvb(exp12tol,1,nread,1)
      elseif(istr.eq.7)then
c  'GRDWRNGT'
        call real_cvb(grdwrngtol,1,nread,1)
      elseif(istr.eq.8)then
c  'EIGWRNG '
        call real_cvb(eigwrntol,1,nread,1)
      elseif(istr.eq.9)then
c  'SINGUL'
        call int_cvb(i,1,nread,1)
...
c  'HHSTART'
        call real_cvb(hhstart,1,nread,1)
      elseif(istr.eq.29)then
c  'RESTHR'
        call real_cvb(resthr,1,nread,1)
      elseif(istr.eq.30)then
c  'NORTITER'
        call int_cvb(nortiter,1,nread,1)
      elseif(istr.eq.31)then
c  'ORTHTHR'
        call real_cvb(orththr,1,nread,1)
