...
if (iprint >= 4) then
  ...
  do k=1,nDIMCF-1
    write(6,'(2(A,i2,A,F20.12,2x))') 'Rnrm(',k,')=',Rnrm(k),'Snrm(',k,')=',Snrm(k)
  end do
end if

! print the cumulative and individual rank weight:
write(6,'(/)')
write(6,'(A)') 'CUMULATIVE WEIGHT OF INDIVIDUAL-RANK OPERATORS ON THE CRYSTAL FIELD SPLITTING:'
if ((nDIMCF-1) >= 2) write(6,'(2x,A,F10.6,A)') 'O2 :------------------------------------------: ',(Snrm(2)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 4) write(6,'(2x,A,F10.6,A)') 'O2 + O4 :-------------------------------------: ',(Snrm(4)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 6) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 :--------------------------------: ',(Snrm(6)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 8) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 + O8 :---------------------------: ',(Snrm(8)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 10) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 + O8 + O10 :---------------------: ',(Snrm(10)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 12) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 + O8 + O10 + O12 :---------------: ',(Snrm(12)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 14) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 + O8 + O10 + O12 + O14 :---------: ',(Snrm(14)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 16) write(6,'(2x,A,F10.6,A)') 'O2 + O4 + O6 + O8 + O10 + O12 + O14 + O16 :---: ',(Snrm(16)/Tnrm)*100.0_wp,' %.'

!-----------------------------------------------------------------------
write(6,*)
write(6,'(A)') 'ENERGY SPLITTING INDUCED BY CUMMULATIVE INDIVIDUAL-RANK OPERATORS (in cm-1).'
field(1) = '      O2       |'
field(2) = '     O2+O4     |'
field(3) = '   O2+O4+O6    |'
field(4) = '  O2+O4+O6+O8  |'
...
do J=1,nfields,4
  ...
  if (mod(nDIMcf,2) == 1) then
    write(6,'(3A,I2,3x,A,5x,A,4x,A,10(5x,A,6x,A))') '  ',LJ,' =',(nDIMcf-1)/2,'|',iprog,'|',('ONLY','|',i=j,jEnd)
  else
    write(6,'(3A,I3,A,5x,A,4x,A,10(5x,A,6x,A))') ' ',LJ,' =',(nDIMcf-1),'/2 |',iprog,'|',('ONLY','|',i=j,jEnd)
  end if

  write(6,'(10x,10A)') '|     INITIAL   |',(field(i),i=j,jEnd)
  write(6,'(16A)') '----------|',('---------------|',i=j,jEnd+1)
  do i=1,nDIMcf
    write(6,'(1x,A,1x,i2,2x,a,12(f14.8,1x,a))') 'w.f.',i,'|',(Winit(i)-Winit(1)),'|',((Ws(k,i)-Ws(k,1)),'|',k=2*j,2*jEnd,2)
  end do
  write(6,'(16A)') '----------|',('---------------|',i=j,jEnd+1)
end do ! j

write(6,'(/)')
write(6,'(A)') 'WEIGHT OF INDIVIDUAL-RANK OPERATORS ON THE CRYSTAL FIELD SPLITTING:'
if ((nDIMCF-1) >= 2) write(6,'(2x,A,F10.6,A)') 'O2  :-----------------------------------------: ',(Rnrm(2)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 4) write(6,'(2x,A,F10.6,A)') 'O4  :-----------------------------------------: ',(Rnrm(4)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 6) write(6,'(2x,A,F10.6,A)') 'O6  :-----------------------------------------: ',(Rnrm(6)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 8) write(6,'(2x,A,F10.6,A)') 'O8  :-----------------------------------------: ',(Rnrm(8)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 10) write(6,'(2x,A,F10.6,A)') 'O10 :-----------------------------------------: ',(Rnrm(10)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 12) write(6,'(2x,A,F10.6,A)') 'O12 :-----------------------------------------: ',(Rnrm(12)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 14) write(6,'(2x,A,F10.6,A)') 'O14 :-----------------------------------------: ',(Rnrm(14)/Tnrm)*100.0_wp,' %.'
if ((nDIMCF-1) >= 16) write(6,'(2x,A,F10.6,A)') 'O16 :-----------------------------------------: ',(Rnrm(16)/Tnrm)*100.0_wp,' %.'

!-----------------------------------------------------------------------
write(6,'(/)')
write(6,'(A)') 'ENERGY SPLITTING INDUCED BY INDIVIDUAL-RANK OPERATORS (in cm-1).'
field(1) = '      O2       |'
field(2) = '      O4       |'
field(3) = '      O6       |'
field(4) = '      O8       |'
...
do J=1,nfields,4
  ...
  write(6,'(16A)') '----------|',('---------------|',i=j,jEnd+1)
  do i=1,nDIMcf
    write(6,'(1x,A,1x,i2,2x,a,12(f14.8,1x,a))') 'w.f.',i,'|',(Winit(i)-Winit(1)),'|',((Wk(k,i)-Wk(k,1)),'|',k=2*j,2*jEnd,2)
  end do
  write(6,'(16A)') '----------|',('---------------|',i=j,jEnd+1)
end do ! j
...
!-----------------------------------------------------------------------
write(6,'(/)')
write(6,'(A)') 'WEIGHT OF INDIVIDUAL CRYSTAL FIELD PARAMETERS ON THE CRYSTAL FIELD SPLITTING: (in descending order):'
write(6,'(A)') 'CFP are given in ITO used in J. Chem. Phys. 137, 064112 (2012).'

write(6,'(100A)') ('-',i=1,55),'|'
write(6,'(A)') '  k |  q  |         B(k,q)        |    Weight (in %)   |'
write(6,'(A)') '----|-----|-----------------------|--------------------|'
do ik=1,ikmax
  ip = projKQ(ik)
  iq = abs(projKQ(ik))
  ...
  if (projKQ(ik) >= 0) then
    write(6,'((1x,I2,1x,A),(1x,I3,1x,A),(ES22.14,1x,A),F19.14,1x,A)') ir,'|',ip,'|',BC(ir,iq),'|',wt,'|'
  else
    write(6,'((1x,I2,1x,A),(1x,I3,1x,A),(ES22.14,1x,A),F19.14,1x,A)') ir,'|',ip,'|',BS(ir,iq),'|',wt,'|'
  end if
end do
write(6,'(A)') '----|-----|-----------------------|--------------------|'
