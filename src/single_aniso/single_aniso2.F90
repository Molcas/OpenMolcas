...
write(6,'(A,I16,A)') 'The code allocated initially:',mem,' bytes of memory for this run.'
call xFlush(6)
!-----------------------------------------------------------------------
IReturn = 0
IPRINT = 2
...
if (ifrestart) then
  ...
  if (input_to_read == 1) then
    ...
    if (DBG) write(6,*) 'SINGLE_ANISO2::  Enter read_formatted_aniso'
    ...
    if (DBG) write(6,*) 'SINGLE_ANISO2::  Exit  read_formatted_aniso'
...
do i=1,nss
  write(6,'(A,I4,A,F25.14)') 'ENERGY OF THE SPIN-ORBIT STATE (',i,') =',ESO(i)
end do
write(6,'(A)') 'LOW-LYING SPIN-FREE ENERGIES:'
do i=1,nstate
  write(6,'(A,I4,A,F25.14)') 'ENERGY OF THE SPIN-FREE STATE  (',i,') =',ESFS(i)
end do
rdiff = 0.0_wp
...
if (compute_g_tensors .and. (nMult > 0)) then
  do IMLTPL=1,NMULT
    ...
    if (ndim(imltpl) == 1) then
      write(6,'(5X,A,I2,A)') 'THE DIMENSION OF THE ',IMLTPL,' MULTIPLET IS 1.'
      write(6,'(5X,A)') 'THERE IS NO G TENSOR FOR EFFECTIVE S = 0.'
      go to 10
    end if
...
if (compute_torque .or. compute_magnetization .or. (Xfield /= 0.0_wp)) then
...
if (compute_magnetization) then
  ...
  write(6,*)
  write(6,'(5X,A)') 'ON USER REQUEST, THE MAGNETIZATION VECTOR AND THE MEAN MAGNETIZATION WAS NOT CALCULATED'
end if
