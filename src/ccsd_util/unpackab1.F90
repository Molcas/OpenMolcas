...
do symp=1,nsym
  ...
  if (symp == symq) then
    ...
    if ((key == 4) .and. (aeqb == 0)) then
      ...
      possr1 = r1%d(ir1,1)
      dimef = r1%d(ir1,2)
      if (r1%d(ir1,2) > 0) call unpckhelp1(wrk(possn),wrk(possr1),dimp,dimef,noa(symp),nva(symp))
    end if
    ...
    if (aeqb == 0) then
      ...
      possr2 = r2%d(ir2,1)
      dimef = r2%d(ir2,2)
      if (r2%d(ir2,2) > 0) call unpckhelp1(wrk(possn),wrk(possr2),dimp,dimef,nob(symp),nvb(symp))
    end if
    ...
    if ((key == 2) .or. (key == 4)) then
      ...
      dime = dimm(3,symp)
      dimf = dimm(4,symq)
      if (r3%d(ir3,2) > 0) call unpckhelp3(wrk(possn),wrk(possr3),dimp,dimp,dime,dimf,noa(symp),nva(symp),nob(symq),nvb(symq))
    end if
    ...
    if (((key == 3) .or. (key == 4)) .and. (aeqb == 0)) then
      ...
      dime = dimm(3,symp)
      dimf = dimm(4,symq)
      if (r4%d(ir4,2) > 0) call unpckhelp4(wrk(possn),wrk(possr4),dimp,dimp,dime,dimf,noa(symp),nva(symp),nob(symq),nvb(symq))
    end if
    ...
    if (key == 4) then
      ...
      dime = dimm(3,symp)
      dimf = dimm(3,symq)
      if (r1%d(ir1,2) > 0) call unpckhelp2(wrk(possnp),wrk(possnm),wrk(possr1),dimp,dimq,dime,dimf,noa(symp),nva(symp),noa(symq), &
                                           nva(symq))
    end if
    ...
    dime = dimm(4,symp)
    dimf = dimm(4,symq)
    if (r2%d(ir2,2) > 0) call unpckhelp2(wrk(possnp),wrk(possnm),wrk(possr2),dimp,dimq,dime,dimf,nob(symp),nvb(symp),nob(symq), &
                                           nvb(symq))
    ...
    if ((key == 2) .or. (key == 4)) then
      ...
      dime = dimm(3,symp)
      dimf = dimm(4,symq)
      if (r3%d(ir3,2) > 0) call unpckhelp3(wrk(possnp),wrk(possr3),dimp,dimq,dime,dimf,noa(symp),nva(symp),nob(symq),nvb(symq))
    end if
    ...
    if ((key == 3) .or. (key == 4)) then
      ...
      dime = dimm(3,symq)
      dimf = dimm(4,symp)
      if (r4%d(ir4,2) > 0) call unpckhelp4(wrk(possnp),wrk(possr4),dimp,dimq,dime,dimf,noa(symq),nva(symq),nob(symp),nvb(symp))
    end if
