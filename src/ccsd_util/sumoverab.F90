...
yes = 0
do a=1,nprocab
  if (idab(a) == myRank) then
    !if ((idab(a) == myRank) .and. (ideffab(a) > 0.0d0)) then
    yes = a
  end if
end do
...
do syma=1,nsym
  do symb=1,syma
    ...
    if (nlength == 0) GO TO 21

    !II.4 skip sum over a,b if # of ab is zero
    if ((nvb(syma)*nvb(symb)) == 0) GO TO 21
    ...
    do a=1,nvb(syma)
      ...
      ! ad 1)
      if ((bstart == a) .and. (syma == symb)) goto 6
      ! ad 2)
      if ((bstart <= nsb) .or. (a <= nsa)) goto 6
      ! ad 3)
      left = limb-bstart+1
