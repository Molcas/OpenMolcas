...
!***********************************************************************
     ...
     &                    lunt2o3)
!
!     this routine calc contributions to W1 i.e. W11,W12,W13
!     and controibution frim W1 to T2 i.e. T24
!
!
!     Mediate : WI(m,n,i,j)
...
!
!
!J.2.1map V2(mn,i,e) <= <ie||mn>bbbb
       call map (wrk,wrksize,                                           &
     & 4,3,4,1,2,mapdw12,mapiw12,1,mapdv2,mapiv2,possv20,               &
     &           posst,rc)
!
!J.2.2mult V3(mn,i,j) <= V2(mn,i,e) . T1o(e,j)bb
       call mult (wrk,wrksize,                                          &
     & 4,2,4,1,mapdv2,mapiv2,1,mapdt12,mapit12,1,mapdv3,                &
     &            mapiv3,ssc,possv30,rc)
!
!J.2.3pack V2(mn,ij) <= V3(mn,i,j)
       call fack (wrk,wrksize,                                          &
     & 4,4,mapdv3,1,mapiv3,mapdv2,mapiv2,possv20,rc)
!
!J.2.4add V1(mn,ij) <- V2(mn,ij)
       call add (wrk,wrksize,                                           &
     & 4,4,0,0,0,0,1,1,1.0d0,mapdv2,1,mapdv1,mapiv1,1,rc)
!
!
!
!J.3.1read V2(ef,ij) <= T2o(ef,ij)bbbb
       call filemanager (2,lunt2o2,rc)
       call getmediate (wrk,wrksize,                                    &
     & lunt2o2,possv20,mapdv2,mapiv2,rc)
!
!J.3.2make Tau V2(ef,ij) from V2(ef,ij)
       call mktau (wrk,wrksize,                                         &
     & mapdv2,mapiv2,mapdt11,mapit11,mapdt12,mapit12,1.0d0,             &
     &             rc)
!
!J.3.3read V3(ef,mn) <= <ef||mn>bbbb
       call filemanager (2,lunabij2,rc)
       call getmediate (wrk,wrksize,                                    &
     & lunabij2,possv30,mapdv3,mapiv3,rc)
!
!J.3.4map V4(mn,ef) <= V3(ef,mn)
       call map (wrk,wrksize,                                           &
     & 4,3,4,1,2,mapdv3,mapiv3,1,mapdv4,mapiv4,possv40,posst,           &
     &           rc)
!
!J.3.5mult V3(mn,ij) = V4(mn,ef) . V2(ef,ij)
       call mult (wrk,wrksize,                                          &
     & 4,4,4,2,mapdv4,mapiv4,1,mapdv2,mapiv2,1,mapdv3,mapiv3,           &
     &            ssc,possv30,rc)
!
!J.3.6add V1(mn,ij) <- V3(mn,ij)
       call add (wrk,wrksize,                                           &
     & 4,4,0,0,0,0,1,1,1.0d0,mapdv3,1,mapdv1,mapiv1,1,rc)
!
!
!
!J.4.0Tau(ab,mn) are in V2(ab,mn) from J.3.2
!     W1(mn,ij)  are in V1(mn,ij)
!
!J.4.1mult V3(ab,ij) = V2(ab,mn) . V1(mn,ij)
       call mult (wrk,wrksize,                                          &
     & 4,4,4,2,mapdv2,mapiv2,1,mapdv1,mapiv1,1,mapdv3,mapiv3,           &
     &            ssc,possv30,rc)
!
!J.4.2add t2n(ab,ij)bbbb <- V3(ab,ij)
       call add (wrk,wrksize,                                           &
...
!
!K.1.1map V1(m,n,i,j) <= <mn||ij>abab
       call map (wrk,wrksize,                                           &
...
!
!
!K.2.1map V2(m,n,j,e) <= <je||mn>baab
       call map (wrk,wrksize,                                           &
     & 4,3,4,1,2,mapdw14,mapiw14,1,mapdv2,mapiv2,possv20,               &
     &           posst,rc)
!
!K.2.2mult V3(m,n,j,i) <= V2(m,n,j,e) . T1o(e,i)aa
       call mult (wrk,wrksize,                                          &
     & 4,2,4,1,mapdv2,mapiv2,1,mapdt11,mapit11,1,mapdv3,                &
     &            mapiv3,ssc,possv30,rc)
!
!K.2.3map V2(m,n,i,j) <= V3(m,n,j,i)
       call map (wrk,wrksize,                                           &
     & 4,1,2,4,3,mapdv3,mapiv3,1,mapdv2,mapiv2,possv20,posst,           &
     &           rc)
!
!K.2.4add V1(m,n,i,j) <- - V2(m,n,i,j)
       call add (wrk,wrksize,                                           &
     & 4,4,0,0,0,0,1,1,-1.0d0,mapdv2,1,mapdv1,mapiv1,1,rc)
!
!
!K.2.5map V2(m,n,i,e) <= <ie||mn>abab
       call map (wrk,wrksize,                                           &
     & 4,3,4,1,2,mapdw13,mapiw13,1,mapdv2,mapiv2,possv20,               &
     &           posst,rc)
!
!K.2.6mult V3(m,n,i,j) <= V2(m,n,i,e) . T1o(e,j)bb
       call mult (wrk,wrksize,                                          &
     & 4,2,4,1,mapdv2,mapiv2,1,mapdt12,mapit12,1,mapdv3,                &
     &            mapiv3,ssc,possv30,rc)
!
!K.2.7add V1(m,n,i,j) <- V3(m,n,i,j)
       call add (wrk,wrksize,                                           &
     & 4,4,0,0,0,0,1,1,1.0d0,mapdv3,1,mapdv1,mapiv1,1,rc)
!
!
!
!K.3.1read V2(e,f,i,j) <= T2o(e,f,i,j)abab
       call filemanager (2,lunt2o3,rc)
       call getmediate (wrk,wrksize,                                    &
     & lunt2o3,possv20,mapdv2,mapiv2,rc)
!
!K.3.2make Tau V2(e,f,i,j) from V2(e,f,i,j)
       call mktau (wrk,wrksize,                                         &
     & mapdv2,mapiv2,mapdt11,mapit11,mapdt12,mapit12,1.0d0,             &
     &             rc)
!
!K.3.3read V3(e,f,m,n) <= <ef||mn>abab
       call filemanager (2,lunabij3,rc)
       call getmediate (wrk,wrksize,                                    &
     & lunabij3,possv30,mapdv3,mapiv3,rc)
!
!K.3.4map V4(m,n,e,f) <= V3(e,f,m,n)
       call map (wrk,wrksize,                                           &
     & 4,3,4,1,2,mapdv3,mapiv3,1,mapdv4,mapiv4,possv40,posst,           &
     &           rc)
!
!K.3.5mult V3(m,n,i,j) = V4(m,n,e,f) . V2(e,f,i,j)
       call mult (wrk,wrksize,                                          &
     & 4,4,4,2,mapdv4,mapiv4,1,mapdv2,mapiv2,1,mapdv3,mapiv3,           &
     &            ssc,possv30,rc)
!
!K.3.6add V1(m,n,i,j) <- V3(m,n,i,j)
       call add (wrk,wrksize,                                           &
     & 4,4,0,0,0,0,1,1,1.0d0,mapdv3,1,mapdv1,mapiv1,1,rc)
!
!
!
!K.4.0Tau(a,b,m,n) are in V2(ab,mn) from K.3.2
!     W1(m,n,i,j)  are in V1(m,n,i,j)
!
!K.4.1mult V3(a,b,i,j) = V2(a,b,m,n) . V1(m,n,i,j)
       call mult (wrk,wrksize,                                          &
     & 4,4,4,2,mapdv2,mapiv2,1,mapdv1,mapiv1,1,mapdv3,mapiv3,           &
     &            ssc,possv30,rc)
!
!K.4.2add t2n(a,b,i,j)abab <- V3(a,b,i,j)
       call add (wrk,wrksize,                                           &
