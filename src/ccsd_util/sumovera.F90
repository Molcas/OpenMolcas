...
!
!     FI3
!F13.1FI(a,e)aa <- -sum(m,f-aa) [ <ma||ef>aaaa . T1o(f,m)aa ] *
!F13.2FI(a,e)aa <- -sum(m,f-bb) [ <ma||ef>baab . T1o(f,m)bb ] *
!F13.3FI(a,e)bb <-  sum(m,f-bb) [ <ma||fe>bbbb . T1o(f,m)bb ] +
!f13.4FI(a,e)bb <-  sum(m,f-aa) [ <ma||fe>abab . T1o(f,m)aa ] +
!
!     WIII1
!W31.1WIII(m,e,b,j)aaaa <= <mb||ej>aaaa *
!W31.2WIII(m,e,b,j)bbbb <= <mb||ej>bbbb +
!W31.3WIII(m,e,b,j)aabb <= <mb||ej>abab +
!W31.4WIII(m,e,b,j)abba <= <mb||ej>abba +
!W31.5WIII(m,e,b,j)baab <= <mb||ej>baab *
!W31.6WIII(m,e,b,j)bbaa <= <mb||ej>baba *
!
!     WIII2
!W32.1WIII(m,e,b,j)aaaa <- + sum(f-a) [  <mb||ef>aaaa . T1o(f,j)aa ] *
!W32.2WIII(m,e,b,j)bbbb <- _ sum(f-b) [  <mb||ef>bbbb . T1o(f,j)bb ] +
!W32.3WIII(m,e,b,j)aabb <- + sum(f-b) [  <mb||ef>abab . T1o(f,j)bb ] +
!W32.4WIII(m,e,b,j)abba <- - sum(f-a) [  <mb||fe>abab . T1o(f,j)aa ] +
!W32.5WIII(m,e,b,j)baab <-  sum(f-b) [  <mb||ef>baab . T1o(f,j)bb ] *
!W32.6WIII(m,e,b,j)bbaa <- - sum(f-a) [  <mb||fe>baab . T1o(f,j)aa ] *
!
!     T15
!T15.1T1n(a,i)aa <- sum(n,f-aa) [ <na||fi>aaaa . T1o(f,n)aa ] *
!T15.2T1n(a,i)aa <- sum(n,f-bb) [ <na||fi>baba . T1o(f,n)bb ] *
!T15.3T1n(a,i)bb <- sum(n,f-bb) [ <na||fi>bbbb . T1o(f,n)bb ] +
!T15.4T1n(a,i)bb <- sum(n,f-aa) [ <na||fi>abab . T1o(f,n)aa ] +
!
!     T16
!T16.1T1n(a,i)aa <- - sum(m,e>f-aaa) [ T2o(ef,i,m)aaaa  . <ma||ef>aaaa ] *
!T16.2T1n(a,i)aa <- - sum(m,e,f-bab) [ T2o(e,f,i,m)abab . <ma||ef>baab ] *
!T16.3T1n(a,i)bb <- - sum(m,e>f-bbb) [ T2o(ef,i,m)bbbb  . <ma||ef>bbbb ] +
!T16.4T1n(a,i)bb <- + sum(m,e,f-aab) [ T2o(e,f,m,i)abab . <ma||ef>abab ] +
!
!     T2Ex
!T2E.12  R(a,m,ij)aaaa   <= - sum(e>f-aa) [ <ma||ef>aaaa . Tau(ef,ij)aaaa ] *
!T2E.1T2n(ab,ij)aaaa   <- - sum(m-a)    [ T1o(b,m)aa   .  R(a,m,ij)aaaa ] *
!T2E.2T2n(ab,ij)aaaa         <-   sum(m-a)    [ T1o(a,m)aa   .  R(b,m,ij)aaaa ] *
!T2E.34         R(a,m,ij)bbbb   <= - sum(e>f-bb) [ <ma||ef>bbbb . Tau(ef,ij)bbbb ] +
!T2E.3T2n(ab,ij)bbbb   <- - sum(m-b)    [ T1o(b,m)bb   .  R(a,m,ij)bbbb ] +
!T2E.4T2n(ab,ij)bbbb         <-   sum(m-b)    [ T1o(a,m)bb   .  R(b,m,ij)bbbb ] +
!T2E.5R1(a,m,i,j)abab <= - sum(e,f-ab) [ <ma||ef>baab . Tau(e,f,i,j)abab ] *
!T2E.5T2n(a,b,i,j)abab <- - sum(m-b)    [ T1o(m,b)bb . R1(a,m,i,j)baab ] *
!T2E.6R2(a,m,i,j)baab <= - sum(e,f-ab) [ <ma||ef>abab . Tau(e,f,i,j)abab ] +
!T2E.6T2n(a,b,i,j)abab <-   sum(m-a)    [ T1o(m,a)aa . R2(a,m,i,j)abab ] +
!
!     T27
!T27.1G(b,m,i,j)aaaa  <= - sum(e-a)  [ <mb||ej>aaaa . T1o(e,i)aa ]  *
!T27.1Q(b,m,ij)aaaa   <= G(b,m,i,j)aaaa - G(b,m,j,i)aaaa            *
!T27.1R(c,d,ij)aaaa   <= sum(m-a)    [ T1o(c,m)aa   . Q(d,m,ij)aa ] *
!T27.1T2n(ab,ij)aaaa   <- R(a,b,ij)aaaa - R(b,a,ij)aaaa              *
!T27.2G(b,m,i,j)bbbb  <= - sum(e-b)  [ <mb||ej>bbbb . T1o(e,i)bb ]  +
!T27.2Q(b,m,ij)bbbb   <= G(b,m,i,j)bbbb - G(b,m,j,i)bbbb            +
!T27.2R(c,d,ij)bbbb   <= sum(m-b)    [ T1o(c,m)bb   . Q(d,m,ij)bb ] +
!T27.2T2n(ab,ij)bbbb   <- R(a,b,ij)bbbb - R(b,a,ij)bbbb              +
!T27.3G1(b,m,i,j)abab <= + sum(e-a)  [ <mb||ej>baab . T1o(e,i)aa ]  *
!T27.3<- - sum(e-b)  [ <mb||ei>baba . T1o(e,j)bb ]  *
!T27.3T2n(a,b,i,j)abab <- sum(m-b)    [ T1o(b,m)bb   . G1(a,m,i,j)abab ] *
!T27.4G2(b,m,i,j)baab <= - sum(e-a)  [ <mb||ej>abab . T1o(e,i)aa ] +
!T27.4<- + sum(e-b)  [ <mb||ei>abba . T1o(e,j)bb ] +
!T27.4T2n(a,b,i,j)abab <- sum(m-a)    [ T1o(a,m)aa   . G2(b,m,i,j)baab ] +
!
!
!     General status
!
...
!
!     Integrals are strored as follows:
!
!     1) <ma||ef> for a - alfa
!     lun = n1aalfa
...
!T27.3G1(b,m,i,j)abab <- - sum(e-b)  [ <mb||ei>baba . T1o(e,j)bb ]
!T27.3T2n(a,b,i,j)abab <- sum(m-b)    [ T1o(b,m)bb   . G1(a,m,i,j)abab ]
...
!        if idbaab.ne.idbbaa we need to calc only contribution from
!        2.nd part of G1
!
...
!        if idabba.ne.idaabb we need to calc only contribution from
!        2.nd part of G2
!
...
!T2E.34.1   M3(m,ij) = M1(m,ef) . V1(ef,ij)
     ...
     & possm30,rc)
!
!T2E.3,4.1  M4(b,ij) = T1o(b,m)bb . M3(m,ij)
       call mult (wrk,wrksize,                                          &
     & 2,3,3,1,                                                         &
     & mapdt12,mapit12,1,mapdm3,mapim3,ssm3,mapdm4,mapim4,ssm4,         &
     & possm40,rc)
!
!T2E.3,4.2    T2n(ab,ij)bbbb <- 1.0d0 . M4(b,ij) (@@ pozor na factor @@)
       call add (wrk,wrksize,                                           &
     & 3,4,1,1,a,0,syma,1,1.0d0,                                        &
     & mapdm4,ssm4,mapdt22,mapit22,1,rc)
!
!     ------- cont to W32
!W32.2WIII(m,e,b,j)bbbb <- + sum(f-b) [  <mb||ef>bbbb . T1o(f,j)bb ]
!
...
!     ------- cont to T2Ex
!T2E.6R2(a,m,i,j)baab <= - sum(e,f-ab) [ <ma||ef>abab . Tau(e,f,i,j)abab ]
!T2E.6T2n(a,b,i,j)abab <-   sum(m-a)    [ T1o(m,a)aa . R2(b,m,i,j)abab ]
!
