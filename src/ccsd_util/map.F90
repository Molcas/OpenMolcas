...
subroutine map(wrk,wrksize,nind,p,q,r,s,mapda,mapia,ssa,mapdb,mapib,possb0,posst,rc)
...
! p      - position of 1-st index od mtx A in mtx B  (Input)
! q      - position of 2-nd index od mtx A in mtx B  (Input)
! r      - position of 3-rd index od mtx A in mtx B  (Input)
! s      - position of 4-th index od mtx A in mtx B  (Input)
...
if (nind == 1) then
  call noperm(wrk,wrksize,mapda,mapia,mapdb,mapib,possb0,posst)
  return
end if
