...
!I    **************  start   section **************
!
!I.1.1read input data form INPDAT (reorg) and input file
!
       call reainput
!
!I.1.2write head to output file
!
...
!
!I.1.3check if there is something to do in CCSD
...
!
!I.1.4def parametrs for filemgr
!
       call mkfilemgrcom
!
...
!
!I.2.5read static integrals from INTSTA (reorg) file
!
       call reaintsta (Work(iOff),wrksize)
!
!I.2.6divide fok to faa,fai,fii and dp
!
...
!I.2.93 T2naaaa,T2nbbbb and T2nabab are 0, we can write them as T2old
       call wrtmediate (Work(iOff),wrksize,                             &
...
!
!      store differntial wc time at this point to calculate idle time
...
!
!
!3.1  division by denominators (divided amplitudes are stored in the same files
!     like T1n and T2n
!
!3.1.1div. T1n
...
!
!
!3.4  save restart informations - amplitudes
       if (keyrst.ne.0) then
...
!
!
!3.5  write Tn into place of to
!
!3.5.1put t1naa -> t1oaa
       call map (Work(iOff),wrksize,                                    &
     & 2,1,2,0,0,mapdt13,mapit13,1,mapdt11,mapit11,posst110,            &
     &           posst,rc)
!
!3.5.2put t1nbb -> t1obb
       call map (Work(iOff),wrksize,                                    &
     & 2,1,2,0,0,mapdt14,mapit14,1,mapdt12,mapit12,posst120,            &
     &           posst,rc)
!
!3.5.3rewind lunt2o1 and write t2naaaa there
       call filemanager (2,lunt2o1,rc)
       call wrtmediate (Work(iOff),wrksize,                             &
     & lunt2o1,mapdt21,mapit21,rc)
!
!3.5.4rewind lunt2o2 and write t2nbbbb there
       call filemanager (2,lunt2o2,rc)
       call wrtmediate (Work(iOff),wrksize,                             &
     & lunt2o2,mapdt22,mapit22,rc)
!
!3.5.5rewind lunt2o3 and write t2nabab there
...
!
!3.6.2calc fai(a,i)bb . t1(a,i)bb
       call multdot (Work(iOff),wrksize,                                &
...
!FUE   write(6,22) scalar
!
!3.6.3get <ab||ij>aaaa into V1 and calc V1(abij) . Tau(abij)aaaa
...
!FUE   write(6,22) scalar
!
!3.6.4get <ab||ij>bbbb into V1 and calc V1(abij) . Tau(abij)bbbb
...
!FUE   write(6,22) scalar
!
!3.6.5get <ab||ij>abab into V1 and calc V1(abij) . Tau(abij)abab
...
!
!
!3.7  save restart informations - energy, iteration cycle
       if (keyrst.ne.0) then
...
!tmp   timdifcpu=timtotcpun-timtotcpu
     ...
     &               myRank,timdifwc,timtotwc
       end if
!
!
!3.8  test of convergence or termination
!
...
!
!4.0  type 5 maximal elements + euclidian norms in each type of amplitudes
!
!4.0.1T1aa
...
!
!4.4  close lunrst file if restart informations was stored
       if (keyrst.ne.0) then
