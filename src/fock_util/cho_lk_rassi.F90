...
!
!   FI(ab) = 2 * sum_J  Lab,J * U(J)  -  sum_Jk  Xka,J * Ykb,J
!
...
if (Deco) then
  ...
  if (iOK == 0) then ! point to the "generalized" Cholesky MOs
    ...
    !write(6,*) 'Cholesky MOs used for state A'
    !if (nDen == 2) write(6,*) 'Pseudo Cholesky MOs used for state B'
  else
    write(6,*) '*******************************'
        ...
        do jK=1,nIsh(kSym)
          ...
          if (DoScreen) then
            ...
            if (numSh2 > 0) then
              ...
              do while (jml <= nShell)
                ...
                else if (Fake_CMO2 .and. (MLk(1,jK_a,1) >= xtau)) then
                  numSh1 = numSh1+1
                else
                  jml = nShell ! exit the loop
                end if
              ...
              do ibSh=1,nShell
                ...
                if ((nnBstRSh(JSym,iShp_rs(iShp),iLoc)*nBasSh(lSym,iaSh)*nBasSh(kSym,ibSh) > 0) .and. &
                  ...
                  if (lSym >= kSym) then
                    ...
                    call DGEMV_(Mode(1:1),n1,n2,One,L_Full%SPB(lSym,iShp_rs(iShp),l1)%A21,n1,MO(jDen)%SB(kSym)%A2(iOffShb+1:,jK), &
                                1,ONE,Lab%SB(iaSh,lSym,jDen)%A,1)
                    ...
                    call DGEMV_(Mode(1:1),n1,n2,One,L_Full%SPB(kSym,iShp_rs(iShp),l1)%A12,n1,MO(jDen)%SB(kSym)%A2(iOffShb+1:,jK), &
                                1,ONE,Lab%SB(iaSh,lSym,jDen)%A,1)
          ...
          do lSh=1,Indx(0,jK_a,1)
            ...
            do while (mSh <= Indx(0,jK_a,kDen))
              ...
              ibSkip = merge(1,0,Lab%Keep(ibSh,kDen))
...
if (timings) then
  ...
  write(6,'(2x,A26,2f10.2)') '(TW|XY) INTEGRALS                         ',tintg(1),tintg(2)
  write(6,*)
