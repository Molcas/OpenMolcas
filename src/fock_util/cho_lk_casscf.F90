...
use Data_Structures, only: SBA_Type, Allocate_SBA, Deallocate_SBA
...
!tbp   if (dFmat < 1.0d3*LKThr) then
!tbp     fcorr = dmpk*1.0d-2
!tbp   end if
!tbp   if (dFmat <= LKThr) fcorr = fcorr*1.0d-2
!tbp end if
              ...
              do ibSh=1,nShell
                ...
                if ((nnBstRSh(JSym,iShp_rs(iShp),iLoc)*nBasSh(lSym,iaSh)*nBasSh(kSym,ibSh) > 0) .and. &
                  ...
                  if (lSym >= kSym) then
                    ...
                    if (JDen == 2) then
                      call DGEMV_(Mode(1:1),n1,n2,One,L_Full%SPB(lSym,iShp_rs(iShp),l1)%A21,n1,Ash(2)%SB(kSym)%A2(1+ioffShb:,jK), &
                    ...
                    if (JDen == 2) then
                      call DGEMV_(Mode(1:1),n1,n2,One,L_Full%SPB(kSym,iShp_rs(iShp),l1)%A12,n1,Ash(2)%SB(kSym)%A2(1+ioffShb:,jK), &
                                  1,One,Lab%SB(iaSh,lSym,1)%A,1)
                    else
...
if (timings) then
  ...
  write(6,'(2x,A26,2f10.2)') 'MO HALF-TRANSFORM VECTORS                 ',tmotr(1),tmotr(2)
  write(6,'(2x,A26,2f10.2)') 'EXCHANGE                                  ',texch(1),texch(2)
  write(6,'(2x,A26,2f10.2)') '(PU|VX) INTEGRALS                         ',tintg(1),tintg(2)
  write(6,*)
