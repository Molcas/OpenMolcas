...
!          IF(  MNGSOC(IGAS).GT.NOBPT(IGAS)) THEN
!            IPHGAS(IGAS) = 2
!            IPHGASL = IGAS
!            NPHGAS = NPHGAS + 1
!          ELSE
...
!_Jesper      IF(IPHGASL.NE.NPHGAS) THEN
...
!E         IF(IUSE_PH.EQ.1) THEN
!E           IDELP = 0
!E           IDELM = 0
!E           DO IGAS = 1, NGAS
!E             IF(IPHGAS(IGAS).EQ.2) THEN
!E               NELH =  NELFGP(IOCTYP(IGAS)+IBGPSTR(IGAS)-1)
!E               IF(NELH.LT.MNGSOC(IGAS))
!E    &          IDELM = IDELM +MNGSOC(IGAS)-NELH
!E               IF(NELH.GT.MXGSOC(IGAS))
!E    &          IDELP = IDELP + NELH-MXGSOC(IGAS)
!E             END IF
!E           END DO
!E           IF(IDELM.GT.0.OR.IDELP.GT.MAX(0,IDEL)) THEN
!E             I_AM_OKAY = 0
