...
__global__ void abtor2_cuda_kernel(double *rho2b, const double *skii, const double *ckjj, const std::int64_t *kbib,
                                   const double *xkbib, const std::int64_t *kbjb, const double *xkbjb, std::int64_t nka,
                                   std::int64_t nkb, std::int64_t ni, std::int64_t nj, std::int64_t nk, std::int64_t nl,
                                   std::int64_t maxk, std::int64_t nib, std::int64_t njb, std::size_t split_count,
                                   std::size_t maps_in_split) {
  ...
  const std::size_t map_end
      = map_begin + maps_in_split < static_cast<std::size_t>(nkb) ? map_begin + maps_in_split : static_cast<std::size_t>(nkb);
  double result = 0.0;
    ...
    if (ib != 0 && jb != 0) {
      ...
      for (std::size_t base = 0; base < nka_size; base += tile_size) {
        ...
        a_tile[threadIdx.y][threadIdx.x] = i < ni_size && a_ka < nka_size
                                               ? skii[a_ka + i * nka_size + static_cast<std::size_t>(ib - 1) * ni_size * nka_size]
                                               : 0.0;
        b_tile[threadIdx.y][threadIdx.x] = j < nj_size && b_ka < nka_size
                                               ? ckjj[b_ka + j * nka_size + static_cast<std::size_t>(jb - 1) * nj_size * nka_size]
                                               : 0.0;
        __syncthreads();
...
extern "C" int64_t lucia_abtor2_cuda_route(double *rho2b, const double *skii, const double *ckjj, int64_t nka, int64_t nkb,
                                           int64_t ni, int64_t nj, int64_t nk, int64_t nl, int64_t maxk, const int64_t *kbib,
                                           const double *xkbib, const int64_t *kbjb, const double *xkbjb, int64_t ikord,
                                           int64_t nib, int64_t njb) {
  if (ikord != 0 || rho2b == nullptr || skii == nullptr || ckjj == nullptr || kbib == nullptr || xkbib == nullptr || kbjb == nullptr
      || xkbjb == nullptr) {
    return 0;
  }
  ...
  if (!lucia_cuda::positive_size(nka, &nka_size) || !lucia_cuda::positive_size(nkb, &nkb_size)
      ...
      || !lucia_cuda::positive_size(maxk, &maxk_size) || !lucia_cuda::positive_size(nib, &nib_size)
  ...
  for (std::size_t kb = 0; kb < nkb_size; ++kb) {
    ...
    for (std::size_t k = 0; k < nk_size; ++k) {
      if (kbib[kb + k * maxk_size] != 0)
        ++active_k;
    }
    std::size_t active_l = 0;
    for (std::size_t l = 0; l < nl_size; ++l) {
      if (kbjb[kb + l * maxk_size] != 0)
        ++active_l;
    }
    std::size_t kb_pairs = 0;
    if (!lucia_cuda::checked_mul(active_k, active_l, &kb_pairs) || !checked_add(active_pairs, kb_pairs, &active_pairs)) {
      return 0;
    }
  }
  ...
  if (!lucia_cuda::checked_mul(active_work, ni_size, &active_work) || !lucia_cuda::checked_mul(active_work, nj_size, &active_work)
      || !lucia_cuda::checked_mul(active_work, nka_size, &active_work) || active_work < minimum_active_work) {
    return 0;
  }
  ...
  if (!lucia_cuda::checked_mul(nka_size, ni_size, &partial) || !lucia_cuda::checked_mul(partial, nib_size, &skii_count)) {
    return 0;
  }

  std::size_t ckjj_count = 0;
  if (!lucia_cuda::checked_mul(nka_size, nj_size, &partial) || !lucia_cuda::checked_mul(partial, njb_size, &ckjj_count)) {
    return 0;
  }
  ...
  std::size_t grid_z = 0;
  if (!lucia_cuda::checked_mul(nk_size, nl_size, &plane_count) || !lucia_cuda::checked_mul(plane_count, split_count, &grid_z)
  ...
  if (!workspace.rho2b.copy_from(rho2b, rho2b_count) || !workspace.skii.copy_from(skii, skii_count)
      || !workspace.ckjj.copy_from(ckjj, ckjj_count) || !workspace.kbib.copy_from(kbib, kbib_count)
      || !workspace.xkbib.copy_from(xkbib, kbib_count) || !workspace.kbjb.copy_from(kbjb, kbjb_count)
      || !workspace.xkbjb.copy_from(xkbjb, kbjb_count) || !workspace.staging.reserve(rho2b_count)) {
    return 0;
  }
  ...
  abtor2_cuda_kernel<<<grid, block>>>(workspace.rho2b.data, workspace.skii.data, workspace.ckjj.data, workspace.kbib.data,
                                      workspace.xkbib.data, workspace.kbjb.data, workspace.xkbjb.data, nka, nkb, ni, nj, nk, nl,
                                      maxk, nib, njb, split_count, maps_in_split);
  bool success = true;
  ...
  if (cudaMemcpy(workspace.staging.data, workspace.rho2b.data, rho2b_bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
    return 0;
  }
