...
use lucia_data, only: ABTOR2_TINY_M_MAX, ABTOR2_TINY_N_MAX
#ifdef _CUDA_BLAS_
use, intrinsic :: iso_c_binding, only: c_int64_t
use LUCIA_CUDA_INTERFACE, only: LUCIA_ABTOR2_CUDA_ROUTE
#endif
...
#ifdef _CUDA_BLAS_
if ((NKA > 0) .and. (NKB > 0) .and. (NI > 0) .and. (NJ > 0) .and. (NK > 0) .and. (NL > 0) .and. (MAXK > 0) .and. &
    (NKB <= MAXK) .and. (IKORD == 0)) then
  NIBCUDA = maxval(KBIB(1:NKB,:))
  NJBCUDA = maxval(KBJB(1:NKB,:))
  if ((NIBCUDA > 0) .and. (NJBCUDA > 0)) then
    CudaStatus = LUCIA_ABTOR2_CUDA_ROUTE(RHO2B,SKII,CKJJ,NKA,NKB,NI,NJ,NK,NL,MAXK,KBIB,XKBIB,KBJB,XKBJB,IKORD,NIBCUDA,NJBCUDA)
    if (CudaStatus == 1) return
  end if
end if
#endif
      ...
      if (IB /= 0) then
        ...
        do L=1,NL
          ...
          if (JB /= 0) then
            ...
            !end if
            if ((IMAX == NI) .and. (NI >= 1) .and. (NI <= ABTOR2_TINY_M_MAX) .and. (NJ >= 1) .and. (NJ <= ABTOR2_TINY_N_MAX)) then
...
subroutine ABTOR2_MATML7_TN_TINY(C,A,B,M,N,K,FACTORAB)
  ...
  integer(kind=iwp) :: KA
  real(kind=wp) :: B1, B2, B3, B4, S11, S12, S13, S14, S21, S22, S23, S24, S31, S32, S33, S34, S41, S42, S43, S44
