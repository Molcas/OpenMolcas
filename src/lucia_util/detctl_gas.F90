...
call ZBLTP(ISMOST(:,jsym),NSMST,IDC,CBLTP,SVST)
call mma_deallocate(SVST)

! Batches  of C vector
call PART_CIV2(IDC,NSTSO(IATP)%A,NSTSO(IBTP)%A,NOCTPA,NOCTPB,NSMST,LCIOIO,ISMOST(1,jsym),NBATCH,CLBT,CLEBT,CI1BT,CIBT,0)
! Number of BLOCKS
...
call MXRESCPH(LCIOIO,IOCTPA,IOCTPB,NOCTPA,NOCTPB,NSMST,NSTFSMSPGP,MXPNSMST,NSMOB,MXPNGAS,NGAS,NOBPTS,MAXK,NELFSPGP,MXCJ,MXCIJA, &
