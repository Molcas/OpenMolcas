...
if ((IATP == JATP) .and. (IASM == JASM)) then
  ...
  call GSBBD1(RHO1,NACOB,IBSM,IBTP,JBSM,JBTP,IJBGRP,NIA,NGAS,IBOC,JBOC,SB,CB,NIB,NJB,MXPNGAS,NOBPTS,IOBPTS,MAXI,MAXK,SSCR, &
  ...
  if ((I12 == 2) .and. (NBEL >= 2)) then
    ...
    call TIMING(CPU0,CPU,WALL0,WALL)
    call GSBBD2A(RHO2,RHO2S,RHO2A,NACOB,IBSM,IBTP,JBSM,JBTP,IJBGRP,NIA,NGAS,IBOC,JBOC,SB,CB,NIB,NJB,MXPNGAS,NOBPTS,IOBPTS,MAXI, &
                 MAXK,SSCR,CSCR,I1,XI1S,I2,XI2S,X,NSMOB,SCLFAC,IPACK)
    call TIMING(CPU1,CPU,WALL1,WALL)
...
if ((IBTP == JBTP) .and. (IBSM == JBSM)) then
  ...
  IAB = 1
  call TIMING(CPU0,CPU,WALL0,WALL)
  call GSBBD1(RHO1,NACOB,IASM,IATP,JASM,JATP,IJAGRP,NIB,NGAS,IAOC,JAOC,SB,CB,NIA,NJA,MXPNGAS,NOBPTS,IOBPTS,MAXI,MAXK,SSCR,CSCR,I1, &
  ...
  if ((I12 == 2) .and. (NAEL >= 2)) then
    ...
    call TIMING(CPU0,CPU,WALL0,WALL)
    call GSBBD2A(RHO2,RHO2S,RHO2A,NACOB,IASM,IATP,JASM,JATP,IJAGRP,NIB,NGAS,IAOC,JAOC,SB,CB,NIA,NJA,MXPNGAS,NOBPTS,IOBPTS,MAXI, &
                 MAXK,SSCR,CSCR,I1,XI1S,I2,XI2S,X,NSMOB,SCLFAC,IPACK)
    call TIMING(CPU1,CPU,WALL1,WALL)
    TDENSI(2) = TDENSI(2)+(WALL1-WALL0)
