...
template <typename T> int upload_device(lucia_cuda::DeviceBuffer<T> &device, const T *host, std::size_t count) noexcept {
  if (count == 0) {
    return 1;
  }
...
struct Workspace {
  ...
  bool session_matches(const double *sb_host, const double *cb_host, std::size_t sb_count, std::size_t cb_count) const noexcept {
    return session_active && session_sb_host == sb_host && session_cb_host == cb_host && session_sb_count == sb_count
           && session_cb_count == cb_count;
  }

  bool begin_session(const double *sb_host, const double *cb_host, std::size_t sb_count, std::size_t cb_count) noexcept {
    if (session_active) {
      return false;
    }
    session_active = true;
  ...
  bool begin_density_session(double *rho2_host, double *rho2s_host, double *rho2a_host, double *s2_host, std::size_t norb,
                             bool ipack, std::size_t density_count) noexcept {
    density_active = true;
    ...
    if (!copied || !s2_delta.reserve(1) || cudaMemset(s2_delta.data, 0, sizeof(double)) != cudaSuccess) {
      clear_density_state();
      return false;
    }
  ...
  bool flush_density() noexcept {
    ...
    if (!lucia_cuda::checked_mul(density_count, sizeof(double), &density_bytes) || density_s2_host == nullptr) {
      return false;
    }
    if (density_ipack) {
      if (cudaMemcpy(density_rho2s_host, rho2s.data, density_bytes, cudaMemcpyDeviceToHost) != cudaSuccess
          || cudaMemcpy(density_rho2a_host, rho2a.data, density_bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
        return false;
      }
    } else if (cudaMemcpy(density_rho2_host, rho2.data, density_bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
      return false;
    }
    double delta = 0.0;
  ...
  bool map_session_matches(const std::int64_t *i1_host, const double *xi1_host, const std::int64_t *i3_host, const double *xi3_host,
                           std::size_t i1_count, std::size_t i3_count) const noexcept {
    return map_session_active && map_i1_host == i1_host && map_xi1_host == xi1_host && map_i3_host == i3_host
           && map_xi3_host == xi3_host && map_i1_count == i1_count && map_i3_count == i3_count;
  }

  bool begin_map_session(const std::int64_t *i1_host, const double *xi1_host, const std::int64_t *i3_host, const double *xi3_host,
  ...
  bool limit_split_count(std::size_t plane_count, std::size_t requested, std::size_t *result) noexcept {
    if (result == nullptr || plane_count == 0 || requested == 0 || !supports_grid(1, 1, 1) || plane_count > max_grid_z) {
      return false;
    }
...
__global__ void gsbbd2b_density_kernel(const double *x, double *rho2, double *rho2s, double *rho2a, double *s2_delta,
                                       std::size_t ni, std::size_t nj, std::size_t nk, std::size_t nl, std::size_t ioff,
                                       std::size_t joff, std::size_t koff, std::size_t loff, std::size_t norb, bool ipack,
                                       bool s2_active) {
...
__global__ void gsbbd2b_cuda_kernel(double *x, const double *sb, const double *cb, const std::int64_t *i1, const double *xi1,
                                    const std::int64_t *i3, const double *xi3, const std::int64_t *i4, const double *xi4,
                                    const std::int64_t *i2, const double *xi2, std::size_t nib, std::size_t njb, std::size_t nkastr,
                                    std::size_t kabot0, std::size_t lkabtc, std::size_t nkbstr, std::size_t ni, std::size_t nj,
                                    std::size_t nk, std::size_t split_count, std::size_t maps_in_split) {
  ...
  const std::size_t map_end = map_begin + maps_in_split < nkbstr ? map_begin + maps_in_split : nkbstr;
  double result = 0.0;
    ...
    if (ib != 0 && jb != 0) {
      ...
      for (std::size_t base = 0; base < lkabtc; base += tile_size) {
        ...
        if (i < ni && a_ka < lkabtc) {
          ...
          if (ia != 0) {
            a_value = xi3[map] * sb[static_cast<std::size_t>(ib - 1) + static_cast<std::size_t>(ia - 1) * nib];
          }
        }
        ...
        if (j < nj && b_ka < lkabtc) {
          ...
          if (ja != 0) {
            b_value = xi1[map] * cb[static_cast<std::size_t>(jb - 1) + static_cast<std::size_t>(ja - 1) * njb];
          }
        }
...
extern "C" int64_t lucia_gsbbd2b_cuda_begin(const double *sb, const double *cb, double *rho2, double *rho2s, double *rho2a,
                                            double *s2_term1, int64_t nia, int64_t nib, int64_t nja, int64_t njb, int64_t norb,
                                            int64_t ipack) {
  if (sb == nullptr || cb == nullptr || s2_term1 == nullptr || workspace.session_active || (ipack != 0 && ipack != 1)
      || (ipack == 0 ? rho2 == nullptr : rho2s == nullptr || rho2a == nullptr)) {
    return 0;
  }
  ...
  if (!lucia_cuda::positive_size(nia, &nia_size) || !lucia_cuda::positive_size(nib, &nib_size)
      ...
      || !lucia_cuda::positive_size(norb, &norb_size) || !lucia_cuda::checked_mul(nia_size, nib_size, &sb_count)
      || !lucia_cuda::checked_mul(nja_size, njb_size, &cb_count) || !lucia_cuda::checked_mul(norb_size, norb_size, &norb_squared)
      || (ipack == 0 ? !checked_ntri(norb_squared, &density_count)
                     : !checked_ntri(norb_size, &norb_squared) || !checked_ntri(norb_squared, &density_count))
      || !workspace.begin_session(sb, cb, sb_count, cb_count)
      || !workspace.begin_density_session(rho2, rho2s, rho2a, s2_term1, norb_size, ipack == 1, density_count)) {
...
extern "C" int64_t lucia_gsbbd2b_cuda_begin_maps(const std::int64_t *i1, const double *xi1, const std::int64_t *i3,
                                                 const double *xi3, int64_t nkastr, int64_t ni, int64_t nj) {
  if (i1 == nullptr || xi1 == nullptr || i3 == nullptr || xi3 == nullptr || workspace.map_session_active) {
    return 0;
  }
  ...
  if (!lucia_cuda::positive_size(nkastr, &nkastr_size) || !lucia_cuda::positive_size(ni, &ni_size)
      || !lucia_cuda::positive_size(nj, &nj_size) || !lucia_cuda::checked_mul(nkastr_size, nj_size, &i1_count)
...
extern "C" int64_t lucia_gsbbd2b_cuda_route(double *x, const double *sb, const double *cb, const int64_t *i1, const double *xi1,
                                            const int64_t *i3, const double *xi3, const int64_t *i4, const double *xi4,
                                            const int64_t *i2, const double *xi2, int64_t nia, int64_t nib, int64_t nja,
                                            int64_t njb, int64_t nkastr, int64_t kabot, int64_t lkabtc, int64_t nkbstr, int64_t ni,
                                            int64_t nj, int64_t nk, int64_t nl, int64_t ikord, int64_t ioff, int64_t joff,
                                            int64_t koff, int64_t loff, int64_t norb, int64_t ipack, int64_t s2_active) {
  if (x == nullptr || sb == nullptr || cb == nullptr || i1 == nullptr || xi1 == nullptr || i3 == nullptr || xi3 == nullptr
      || i4 == nullptr || xi4 == nullptr || i2 == nullptr || xi2 == nullptr || ikord != 0) {
  ...
  if ((ipack != 0 && ipack != 1) || (s2_active != 0 && s2_active != 1) || !valid_extent(ioff_size, ni_size, norb_size)
      || !valid_extent(joff_size, nj_size, norb_size) || !valid_extent(koff_size, nk_size, norb_size)
      || !valid_extent(loff_size, nl_size, norb_size) || (s2_active == 1 && (nk_size != nj_size || nl_size != ni_size))) {
    return workspace.fallback_status();
  }
  if (workspace.density_active && (norb_size != workspace.density_norb || (ipack == 1) != workspace.density_ipack)) {
    return workspace.fallback_status();
  }
  ...
  if (!lucia_cuda::checked_mul(ni_size, nj_size, &partial) || !lucia_cuda::checked_mul(partial, nk_size, &partial)
      || !lucia_cuda::checked_mul(partial, nl_size, &x_count) || !lucia_cuda::checked_mul(nia_size, nib_size, &sb_count)
      || !lucia_cuda::checked_mul(nja_size, njb_size, &cb_count) || !lucia_cuda::checked_mul(nkastr_size, nj_size, &i1_count)
      || !lucia_cuda::checked_mul(nkastr_size, ni_size, &i3_count) || !lucia_cuda::checked_mul(nkbstr_size, nk_size, &i4_count)
  ...
  for (std::size_t kb = 0; kb < nkbstr_size; ++kb) {
    ...
    std::size_t pairs = 0;
    if (!lucia_cuda::checked_mul(active_k, active_l, &pairs) || !checked_add(active_pairs, pairs, &active_pairs)) {
      return workspace.fallback_status();
    }
  }
  std::size_t active_work = active_pairs;
  if (!lucia_cuda::checked_mul(active_work, ni_size, &active_work) || !lucia_cuda::checked_mul(active_work, nj_size, &active_work)
      || !lucia_cuda::checked_mul(active_work, lkabtc_size, &active_work) || active_work < minimum_active_work) {
    return workspace.fallback_status();
  }
  ...
  const std::size_t requested_splits = nkbstr_size / maps_per_split + (nkbstr_size % maps_per_split != 0);
  ...
  if (!lucia_cuda::checked_mul(nk_size, nl_size, &plane_count)
      ...
      || !lucia_cuda::checked_mul(plane_count, split_count, &grid_z) || !workspace.supports_grid(grid_x, grid_y, grid_z)) {
    return workspace.fallback_status();
  }
  const std::size_t maps_in_split = nkbstr_size / split_count + (nkbstr_size % split_count != 0);
  ...
  if (workspace.density_active && (!workspace.density_uploaded || !workspace.supports_grid(density_grid, 1, 1))) {
    return workspace.fallback_status();
  }

  std::size_t x_bytes = 0;
  if (!lucia_cuda::checked_mul(x_count, sizeof(double), &x_bytes) || !workspace.x.reserve(x_count)) {
    return workspace.fallback_status();
  }
  ...
  const dim3 grid(static_cast<unsigned int>(grid_x), static_cast<unsigned int>(grid_y), static_cast<unsigned int>(grid_z));
  gsbbd2b_cuda_kernel<<<grid, block>>>(workspace.x.data, device_sb, device_cb, workspace.i1.data, workspace.xi1.data,
                                       workspace.i3.data, workspace.xi3.data, workspace.i4.data, workspace.xi4.data,
                                       workspace.i2.data, workspace.xi2.data, nib_size, njb_size, nkastr_size, kabot0, lkabtc_size,
                                       nkbstr_size, ni_size, nj_size, nk_size, split_count, maps_in_split);
  if (cudaGetLastError() != cudaSuccess) {
    workspace.invalidate_map_residency();
    return -1;
  }
  if (workspace.density_active) {
    gsbbd2b_density_kernel<<<dim3(static_cast<unsigned int>(density_grid)), dim3(static_cast<unsigned int>(density_block_size))>>>(
        workspace.x.data, workspace.rho2.data, workspace.rho2s.data, workspace.rho2a.data, workspace.s2_delta.data, ni_size,
        nj_size, nk_size, nl_size, ioff_size, joff_size, koff_size, loff_size, norb_size, ipack == 1, s2_active == 1);
    if (cudaGetLastError() != cudaSuccess) {
      workspace.invalidate_map_residency();
      return -1;
    }
  ...
  if (cudaDeviceSynchronize() != cudaSuccess
      || cudaMemcpy(workspace.staging.data, workspace.x.data, x_bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
    workspace.invalidate_map_residency();
    return -1;
  }
