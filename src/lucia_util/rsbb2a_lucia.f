...
!?      NPTOT = 0
!?      IF(ITYP.EQ.3) NPTOT = NPTOT + 1
!?      IF(JTYP.EQ.3) NPTOT = NPTOT + 1
!?      IF(KTYP.EQ.3) NPTOT = NPTOT + 1
!?      IF(LTYP.EQ.3) NPTOT = NPTOT + 1
!?      IF(NPTOT.EQ.3) GOTO 2000
!
        ITYP_ORIG = ITYP
        JTYP_ORIG = JTYP
        KTYP_ORIG = KTYP
...
!?                  IF(MXACIJ.GT.MXACIJO) THEN
!?                    write(6,*) ' New max MXACIJ = ', MXACIJ
!?                    write(6,*) ' ISCTP,ICCTP', ISCTP,ICCTP
!?                    WRITE(6,*) ' ITYP,JTYP,KTYP,LTYP',
!?   &                             ITYP,JTYP,KTYP,LTYP
!?                    WRITE(6,*)'NIJT, NJLT, NIBTC NKBTC',
!?   &                           NIJT, NJLT,NIBTC,NKBTC
!?                  END IF
!
                    FACTORC = Zero
                    FACTORAB = One
