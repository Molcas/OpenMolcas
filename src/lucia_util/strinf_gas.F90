...
use lucia_data, only: IBSPGPFTP, IGSFGP, IGSOCC, INGRP_VAL, IOCLS, IPHGAS, ISMDFGP, ISMSCR, ISPGPFTP, ISTAC, ISTSGP, ISTSO, &
                      ...
                      NELFSPGP, NELFTP, NGAS, NGRP, NHLFSPGP, NMXOCCLS, NOBPT, NORB1, NORB2, NORB3, NSPGPFTP, NSTFGP, NSTFSMGP, &
                      NSTFSMSPGP, NSTSGP, NSTSO, NSTTYP, NTSPGP, OCCSTR, SPGPAN, SPGPCR, STREO, STSTM, ZMAT
...
do IGRP=1,NGRP
  ...
  call GENSTR_GAS(IEL,MNRS1X,MXRS1X,MNRS3X,MXRS3X,ISTSGP,IGRP,IOCTYPX,NSMST,Zmat(IGRP)%A,FREEL,STREO(IGRP)%A,OCCSTR(IGRP)%A, &
...
do ITP=1,NSTTYP
  ...
  do i=1,NSPGPFTP(ITP)
    NSTFSMSPGP(1:NSMST,IBSPGPFTP(ITP)+i-1) = NSTSO(ITP)%A((i-1)*NSMST+1:i*NSMST)
  end do
...
MAX_STR_SPGP = 0
do ISPGP=1,NTSPGP
  NSTR = sum(NSTFSMSPGP(1:NSMST,ISPGP))
