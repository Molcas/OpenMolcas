...
struct Workspace {
  ...
  {
    return xint_session_active && xint_session_host == xint_host &&
  ...
  bool release() noexcept
  {
    ...
    if (!xint.release()) success = false;
    if (!product.release()) success = false;
...
__global__ void rsbb2a_contract_kernel(
    double *product, const double *cb, const double *xint,
    const std::int64_t *active_source, const std::int32_t *active_jl,
    const std::int32_t *active_offset, const double *active_sign,
    std::size_t nrow, std::size_t ibot0, std::size_t nibtc,
    std::size_t nkbtc, std::size_t nik, double factor)
{
  ...
  for (std::int32_t base = 0; base < count; base += tile_size) {
    ...
    if (row < nibtc && base + threadIdx.y < count) {
      a_value = active_sign[entry] *
                cb[ibot0 + row + static_cast<std::size_t>(active_source[entry] - 1) * nrow];
    }
    ...
    b[threadIdx.x][threadIdx.y] =
        b_ik < nik && base + threadIdx.y < count
            ? xint[b_ik + static_cast<std::size_t>(active_jl[b_entry]) * nik]
            : 0.0;
    __syncthreads();
...
__global__ void rsbb2a_scatter_kernel(
    double *sb, const double *product, const std::int64_t *smap, const double *ssign,
    std::size_t nrow, std::size_t ibot0, std::size_t nibtc, std::size_t nkbtc,
    std::size_t nik)
{
...
extern "C" int64_t lucia_rsbb2a_cuda_begin(
    double *sb, const double *cb, int64_t nrow, int64_t nsb, int64_t ncb)
{
...
extern "C" int64_t lucia_rsbb2a_cuda_xint_begin(
    const double *xint, int64_t nik, int64_t njl)
{
  if (xint == nullptr) {
    return 0;
  }
  ...
  if (!lucia_cuda::positive_size(nik, &nik_size) ||
      !lucia_cuda::positive_size(njl, &njl_size) ||
      !lucia_cuda::checked_mul(nik_size, njl_size, &xint_count) ||
      !workspace.begin_xint_session(xint, xint_count)) {
    return 0;
  }
  return 1;
}
...
extern "C" int64_t lucia_rsbb2a_cuda_route(
    double *sb, const double *cb, const double *xint, const int64_t *cmap,
    const double *csign, const int64_t *smap, const double *ssign, int64_t nrow,
    int64_t nsb, int64_t ncb, int64_t ibot, int64_t nibtc, int64_t nkbtc,
    int64_t nik, int64_t njl, double factor)
{
  if (sb == nullptr || cb == nullptr || xint == nullptr || cmap == nullptr || csign == nullptr ||
      smap == nullptr || ssign == nullptr || !std::isfinite(factor)) {
  ...
  if (!lucia_cuda::checked_mul(nrow_size, nsb_size, &sb_count) ||
      !lucia_cuda::checked_mul(nrow_size, ncb_size, &cb_count) ||
      !lucia_cuda::checked_mul(nkbtc_size, njl_size, &c_map_count) ||
      !lucia_cuda::checked_mul(nkbtc_size, nik_size, &s_map_count) ||
      !lucia_cuda::checked_mul(nik_size, njl_size, &xint_count) ||
      !lucia_cuda::checked_mul(nibtc_size, nkbtc_size, &plane) ||
      !lucia_cuda::checked_mul(plane, nik_size, &product_count)) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  if (c_map_count > max_compact || njl_size > max_compact ||
      !checked_add(nkbtc_size, 1, &offset_count) ||
      ...
      !append_section(source_bytes, &source_offset, &payload_bytes) ||
      !append_section(sign_bytes, &sign_offset, &payload_bytes) ||
      !append_section(smap_bytes, &smap_offset, &payload_bytes) ||
      !append_section(ssign_bytes, &ssign_offset, &payload_bytes) ||
      !append_section(jl_bytes, &jl_offset, &payload_bytes) ||
      !append_section(offset_bytes, &offset_offset, &payload_bytes)) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  std::int64_t *active_source_host =
      reinterpret_cast<std::int64_t *>(payload_host + source_offset);
  ...
  std::int32_t *active_jl_host =
      reinterpret_cast<std::int32_t *>(payload_host + jl_offset);
  std::int32_t *active_offset_host =
      reinterpret_cast<std::int32_t *>(payload_host + offset_offset);
  ...
  const std::size_t contract_grid_x =
      nibtc_size / tile_size + (nibtc_size % tile_size != 0);
  ...
  if (!workspace.supports_grid(contract_grid_x, contract_grid_y, nkbtc_size) ||
      !workspace.supports_grid(scatter_blocks, 1, 1)) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  if (!workspace.product.reserve(product_count) ||
  ...
  const std::int64_t *active_source =
      reinterpret_cast<const std::int64_t *>(payload_device + source_offset);
  const double *active_sign = reinterpret_cast<const double *>(payload_device + sign_offset);
  const std::int64_t *smap_device =
      reinterpret_cast<const std::int64_t *>(payload_device + smap_offset);
  const double *ssign_device = reinterpret_cast<const double *>(payload_device + ssign_offset);
  const std::int32_t *active_jl =
      reinterpret_cast<const std::int32_t *>(payload_device + jl_offset);
  const std::int32_t *active_offset =
      reinterpret_cast<const std::int32_t *>(payload_device + offset_offset);

  const dim3 contract_block(static_cast<unsigned int>(tile_size),
                            static_cast<unsigned int>(tile_size));
  const dim3 contract_grid(static_cast<unsigned int>(contract_grid_x),
                           static_cast<unsigned int>(contract_grid_y),
                           static_cast<unsigned int>(nkbtc_size));
  rsbb2a_contract_kernel<<<contract_grid, contract_block>>>(
      workspace.product.data, device_cb, workspace.xint.data,
      active_source, active_jl, active_offset, active_sign,
      nrow_size, ibot0, nibtc_size, nkbtc_size, nik_size, factor);
  bool success = cudaGetLastError() == cudaSuccess;
  if (success) {
    rsbb2a_scatter_kernel<<<static_cast<unsigned int>(scatter_blocks), block_size>>>(
        device_sb, workspace.product.data, smap_device, ssign_device,
        nrow_size, ibot0, nibtc_size, nkbtc_size, nik_size);
