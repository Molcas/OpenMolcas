...
struct Workspace {
  ...
  bool release() noexcept {
    ...
    if (!route[1].release())
      success = false;
    next_route = 0;
    if (!inverse_cursor_host.release())
      success = false;
    if (!staging.release())
      success = false;
    device_ready = false;
...
__global__ void rsbb2bn_cuda_kernel(double *sb, const double *cb, const double *xint, const std::int64_t *i3, const double *xi3s,
                                    const std::int32_t *inverse_offset, const std::int32_t *inverse_k,
                                    const std::int32_t *inverse_kb, const double *inverse_factor, const std::int32_t *l_offset,
                                    const std::int32_t *l_index, const std::int64_t *l_jb, const double *l_factor,
                                    const std::int32_t *j_offset, const std::int32_t *j_index, const std::int64_t *j_ja,
                                    const double *j_factor, std::size_t nib, std::size_t njb, std::size_t nkastr,
                                    std::size_t kabot0, std::size_t lkabtc, std::size_t ni, std::size_t nj, std::size_t nk,
                                    std::size_t nl, bool ordered) {
  ...
  for (std::size_t ib = threadIdx.x; ib < nib; ib += blockDim.x) {
    ...
    for (std::int32_t p = inverse_offset[ib]; p < inverse_offset[ib + 1]; ++p) {
      ...
      for (std::int32_t lp = l_offset[kb]; lp < l_offset[kb + 1]; ++lp) {
        ...
        for (std::int32_t jp = j_offset[ka]; jp < j_offset[ka + 1]; ++jp) {
          ...
          const std::size_t c_offset = static_cast<std::size_t>(l_jb[lp] - 1) + static_cast<std::size_t>(j_ja[jp] - 1) * njb;
    ...
    if (result != 0.0) {
      atomic_add(sb + ib + static_cast<std::size_t>(ia - 1) * nib, xi3s[scatter_map] * result);
    }
  }
}
...
extern "C" int64_t lucia_rsbb2bn_cuda_begin(double *sb, const double *cb, int64_t nia, int64_t nib, int64_t nja, int64_t njb) {
  ...
  if (!lucia_cuda::positive_size(nia, &nia_size) || !lucia_cuda::positive_size(nib, &nib_size)
      ...
      || !lucia_cuda::checked_mul(nia_size, nib_size, &sb_count) || !lucia_cuda::checked_mul(nja_size, njb_size, &cb_count)) {
    return 0;
  }
...
extern "C" int64_t lucia_rsbb2bn_cuda_route(double *sb, const double *cb, const double *xint, const int64_t *i1, const double *xi1s,
                                            const int64_t *i3, const double *xi3s, const int64_t *i4, const double *xi4s,
                                            const int64_t *i2, const double *xi2s, int64_t nia, int64_t nib, int64_t nja,
                                            int64_t njb, int64_t nkastr, int64_t kabot, int64_t lkabtc, int64_t nkbstr, int64_t ni,
                                            int64_t nj, int64_t nk, int64_t nl, int64_t ikord) {
  if (sb == nullptr || cb == nullptr || xint == nullptr || i1 == nullptr || xi1s == nullptr || i3 == nullptr || xi3s == nullptr
      || i4 == nullptr || xi4s == nullptr || i2 == nullptr || xi2s == nullptr || (ikord != 0 && ikord != 1)) {
  ...
  if (kabot0 >= nkastr_size || lkabtc_size > nkastr_size - kabot0 || (ikord == 1 && (nk_size > ni_size || nl_size > nj_size))) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  if (!lucia_cuda::checked_mul(nia_size, nib_size, &sb_count) || !lucia_cuda::checked_mul(nja_size, njb_size, &cb_count)
      || !checked_product4(ni_size, nj_size, nk_size, nl_size, &xint_count)
      || !lucia_cuda::checked_mul(nkastr_size, nj_size, &i1_count) || !lucia_cuda::checked_mul(nkastr_size, ni_size, &i3_count)
      || !lucia_cuda::checked_mul(nkbstr_size, nk_size, &i4_count) || !lucia_cuda::checked_mul(nkbstr_size, nl_size, &i2_count)
  ...
  if (i4_count > max_compact || i2_count > max_compact || nk_size > max_compact || nkbstr_size > max_compact
      || nl_size > max_compact || nj_size > max_compact || !lucia_cuda::checked_mul(lkabtc_size, nj_size, &j_capacity)
      || j_capacity > max_compact || !checked_add(nib_size, 1, &inverse_offset_count)
      || !checked_add(nkbstr_size, 1, &l_offset_count) || !checked_add(lkabtc_size, 1, &j_offset_count)) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  if (!append_section(xint_bytes, &xint_offset, &payload_bytes) || !append_section(i3_bytes, &i3_offset, &payload_bytes)
  ...
  if (!slot.prepare() || !slot.host.reserve(payload_bytes) || !workspace.inverse_cursor_host.reserve(nib_size)) {
    return route_fallback(workspace, sb, cb);
  }
  ...
  std::int32_t *inverse_offset_host = reinterpret_cast<std::int32_t *>(payload_host + inverse_offset_offset);
  ...
  const double *inverse_factor_device = reinterpret_cast<const double *>(payload_device + inverse_factor_offset);
  const std::int64_t *l_jb_device = reinterpret_cast<const std::int64_t *>(payload_device + l_jb_offset);
  const double *l_factor_device = reinterpret_cast<const double *>(payload_device + l_factor_offset);
  const std::int64_t *j_ja_device = reinterpret_cast<const std::int64_t *>(payload_device + j_ja_offset);
  const double *j_factor_device = reinterpret_cast<const double *>(payload_device + j_factor_offset);
  const std::int32_t *inverse_offset_device = reinterpret_cast<const std::int32_t *>(payload_device + inverse_offset_offset);
  const std::int32_t *inverse_k_device = reinterpret_cast<const std::int32_t *>(payload_device + inverse_k_offset);
  const std::int32_t *inverse_kb_device = reinterpret_cast<const std::int32_t *>(payload_device + inverse_kb_offset);
  const std::int32_t *l_offset_device = reinterpret_cast<const std::int32_t *>(payload_device + l_offset_offset);
  const std::int32_t *l_index_device = reinterpret_cast<const std::int32_t *>(payload_device + l_index_offset);
  const std::int32_t *j_offset_device = reinterpret_cast<const std::int32_t *>(payload_device + j_offset_offset);
  const std::int32_t *j_index_device = reinterpret_cast<const std::int32_t *>(payload_device + j_index_offset);

  rsbb2bn_cuda_kernel<<<static_cast<unsigned int>(block_count), 256>>>(
      device_sb, device_cb, xint_device, i3_device, xi3s_device, inverse_offset_device, inverse_k_device, inverse_kb_device,
      inverse_factor_device, l_offset_device, l_index_device, l_jb_device, l_factor_device, j_offset_device, j_index_device,
      j_ja_device, j_factor_device, nib_size, njb_size, nkastr_size, kabot0, lkabtc_size, ni_size, nj_size, nk_size, nl_size,
      ikord == 1);
