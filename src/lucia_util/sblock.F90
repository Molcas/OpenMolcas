...
use lucia_data, only: I12, I_RES_AB, IADVICE, IBSPGPFTP, ICJKAIB, IDC, IDISK, IH0INSPC, IH0SPC, IPART, IPHGAS, ISMOST, ISPGPFTP, &
                      MAX_STR_OC_BLK, MAX_STR_SPGP, MNHL, MOCAA, MXINKA, MXNSTR, MXNTTS, MXPNGAS, MXPNSMST, MXPOBS, MXTSOB, NELEC, &
                      NELFGP, NELFSPGP, NGAS, NHLFSPGP, NOBPTS, NOCOB, NOCTYP, NPTSPC, NSMOB, NSTFSMSPGP, NTOOB, PSSIGN
...
call MXRESCPH(SCIOIO,IOCTPA,IOCTPB,NOCTPA,NOCTPB,NSMST,NSTFSMSPGP,MXPNSMST,NSMOB,MXPNGAS,NGAS,NOBPTS,MAXK,NELFSPGP,MXCJ,MXCIJA, &
...
call SBLOCKS(NBLOCK,IBLOCK(:,IBOFF),CB,HCB,VEC3,CIOIO,ISMOST(:,ICSM),NSTSO(IATP)%A,NSTSO(IBTP)%A,NAEL,IATP,NBEL,IBTP,IOCTPA, &
             ...
             NELFSPGP,IDC,I1,XI1S,I2,XI2S,IDOH2,MXPOBS,SVST,PSSIGN,LUC,ICJKAIB,VEC3,VEC3(1+LSCR2),I3,XI3S,I4,XI4S,MOCAA,CLBT, &
             CLEBT,CI1BT,CIBT,IRESTRICT,CONSPA,CONSPB,LSCLFAC,H0SPC,ICBAT_RES,ICBAT_INI,ICBAT_END,IPHGAS,I_RES_AB)

! CALL SBLOCKS --> 91

if (IDC == 2) then
  ! reform
  call RFTTS(HCB,CB,IBLOCK(:,IBOFF),NBLOCK,NSMST,NSTSO(IATP)%A,NSTSO(IBTP)%A,IDC)
  ! scale
  call SCDTTS(HCB,IBLOCK(:,IBOFF),NBLOCK,NSMST,NSTSO(IATP)%A,NSTSO(IBTP)%A,IDC)
end if
