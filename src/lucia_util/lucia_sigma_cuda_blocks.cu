...
bool range_contains(const HostRegion &region, const void *pointer,
...
bool register_host_region(HostRegion *region, void *base,
                          std::size_t bytes) noexcept
{
...
struct Manager {
  ...
  bool matches(double *sb_host, const double *cb_host, std::size_t requested_sb,
               std::size_t requested_cb) const noexcept
  {
    return active && host_sb == sb_host && host_cb == cb_host && requested_sb > 0 &&
           requested_cb > 0 && requested_sb <= sb_count && requested_cb <= cb_count;
  }
  ...
  bool begin(double *sb_host, const double *cb_host, std::size_t full_sb,
             std::size_t full_cb, bool zero_sb_device = false) noexcept
  {
    if (active) {
      return false;
    }
  ...
  bool acquire(double *sb_host, const double *cb_host, std::size_t requested_sb,
               std::size_t requested_cb, double **sb_device,
               const double **cb_device) noexcept
  {
    if (sb_device == nullptr || cb_device == nullptr ||
        !matches(sb_host, cb_host, requested_sb, requested_cb)) {
      return false;
    }
    if (!resident) {
      std::size_t sb_bytes = 0;
      const bool sb_ready = zero_sb
          ? lucia_cuda::checked_mul(sb_count, sizeof(double), &sb_bytes) &&
                sb.reserve(sb_count) &&
  ...
  bool flush() noexcept
  {
    ...
    if (!staging.reserve(sb_count) ||
        cudaMemcpy(staging.data, sb.data, bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
      return false;
    }
  ...
  std::int64_t host_begin(double *sb_host, const double *cb_host,
                          std::int64_t requested_sb,
                          std::int64_t requested_cb) noexcept
  {
    ...
    if (!lucia_cuda::positive_size(requested_sb, &sb_size) ||
        !lucia_cuda::positive_size(requested_cb, &cb_size) ||
...
extern "C" std::int64_t lucia_sigma_cuda_blocks_begin(
    double *sb, const double *cb, std::int64_t sb_count, std::int64_t cb_count)
{
  ...
  std::size_t sb_size = 0;
  std::size_t cb_size = 0;
  if (!lucia_cuda::positive_size(sb_count, &sb_size) ||
      !lucia_cuda::positive_size(cb_count, &cb_size) ||
...
extern "C" std::int64_t lucia_sigma_cuda_blocks_begin_zeroed(
    double *sb, const double *cb, std::int64_t sb_count, std::int64_t cb_count)
{
  if (sb == nullptr || cb == nullptr || manager.active) {
    return 0;
...
extern "C" std::int64_t lucia_sigma_cuda_blocks_host_begin(
    double *sb, const double *cb, std::int64_t sb_count, std::int64_t cb_count)
{
