...
if ((IATP == JATP) .and. (IASM == JASM)) then
  ...
  call GSBBD1_LUCIA(RHO1,NACOB,IBSM,IBTP,JBSM,JBTP,IJBGRP,NIA,NGAS,IBOC,JBOC,SB,CB,ADSXA,STSTSX,MXPNGAS,NOBPTS,IOBPTS,MAXI,MAXK, &
  ...
  if ((I12 == 2) .and. (NBEL >= 2)) then
    ...
    call TIMING(CPU0,CPU,WALL0,WALL)
    call GSBBD2A_LUCIA(RHO2,RHO2S,RHO2A,NACOB,IBSM,IBTP,JBSM,JBTP,IJBGRP,NIA,NGAS,IBOC,JBOC,SB,CB,ADSXA,STSTSX,SXDXSX,MXPNGAS, &
                       NOBPTS,IOBPTS,MAXI,MAXK,SSCR,CSCR,I1,XI1S,X,NSMOB,NSMST,MXPOBS,SCLFAC,IPACK)
    !    GSBBD2A_LUCIA(RHO2,NACOB,ISCSM,ISCTP,ICCSM,ICCTP,IGRP,NROW,NGAS,ISEL,ICEL,SB,CB,ADSXA,STSTSX,SXDXSX,MXPNGAS,NOBPTS, &
    !                  IOBPTS,MAXI,MAXK,SSCR,CSCR,I1,XI1S,X,NSMOB,NSMST,MXPOBS)
    call TIMING(CPU1,CPU,WALL1,WALL)
...
if ((IBTP == JBTP) .and. (IBSM == JBSM)) then
  ...
  IAB = 1
  call TIMING(CPU0,CPU,WALL0,WALL)
  call GSBBD1_LUCIA(RHO1,NACOB,IASM,IATP,JASM,JATP,IJAGRP,NIB,NGAS,IAOC,JAOC,SB,CB,ADSXA,STSTSX,MXPNGAS,NOBPTS,IOBPTS,MAXI,MAXK, &
                    SSCR,CSCR,I1,XI1S,I2,XI2S,NSMOB,NSMST,MXPOBS,RHO1S,SCLFAC,IPHGAS,IDOSRHO1,SRHO1,IAB)
  call TIMING(CPU1,CPU,WALL1,WALL)
  TDENSI(1) = TDENSI(1)+(WALL1-WALL0)
  ...
  if ((I12 == 2) .and. (NAEL >= 2)) then
    ...
    call TIMING(CPU0,CPU,WALL0,WALL)
    call GSBBD2A_LUCIA(RHO2,RHO2S,RHO2A,NACOB,IASM,IATP,JASM,JATP,IJAGRP,NIB,NGAS,IAOC,JAOC,SB,CB,ADSXA,STSTSX,SXDXSX,MXPNGAS, &
                       NOBPTS,IOBPTS,MAXI,MAXK,SSCR,CSCR,I1,XI1S,X,NSMOB,NSMST,MXPOBS,SCLFAC,IPACK)
    call TIMING(CPU1,CPU,WALL1,WALL)
    TDENSI(2) = TDENSI(2)+(WALL1-WALL0)
