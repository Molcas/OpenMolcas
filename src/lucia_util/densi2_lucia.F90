...
call PART_CIV2(IDC,NSTSO(IATP)%A,NSTSO(IBTP)%A,NOCTPA,NOCTPB,NSMST,SIOIO,ISMOST(1,ISSM),NBATCHL,LLBTL,LLEBTL,LI1BTL,LIBTL,0,ISIMSYM)
...
call PART_CIV2(IDC,NSTSO(IATP)%A,NSTSO(IBTP)%A,NOCTPA,NOCTPB,NSMST,CIOIO,ISMOST(1,ICSM),NBATCHR,LLBTR,LLEBTR,LI1BTR,LIBTR,0,ISIMSYM)
...
if (ICISTR == 1) then
  ...
  S2_TERM1 = Zero
  call GASDN2_LUCIA(I12,RHO1,RHO2,RHO2S,RHO2A,L,R,VEC3,CIOIO,SIOIO,ISMOST(:,ICSM),ISMOST(:,ISSM),CBLTP,SBLTP,NACOB,NSTSO(IATP)%A, &
                    NSTSO(IBTP)%A,NAEL,IATP,NBEL,IBTP,IOCTPA,IOCTPB,NOCTPA,NOCTPB,NSMST,NSMOB,MXPNGAS,NOBPTS,IOBPTS,MAXK,MAXI, &
                    VEC3(1+KCSCR),VEC3,STSTS,SXDXSX,ADSXA,NGAS,NELFSPGP,IDC,I1,XI1S,I2,XI2S,I3,XI3S,I4,XI4S,INSCR,MXPOBS,IPRDEN, &
                    RHO1S,LUL,LUR,PSSIGN,PSSIGN,NBATCHL,LLBTL,LI1BTL,LIBTL,NBATCHR,LLBTR,LI1BTR,LIBTR,CONSPA,CONSPB,LSCLFCL, &
