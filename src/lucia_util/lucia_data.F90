...
integer(kind=iwp), parameter :: LOFFI = 8**6, MXPCSM = 100, MXPICI = 30, MXPIRR = 8, MXPNSMST = 8, MXPOBS = 8, MXPORB = 500, &
...
integer(kind=iwp) :: I12, I1234S, I12S, I2ELIMINATED_IN_GAS(MXPNGAS), I34S, I_AM_OUT(MXPSTT), I_ELIMINATE_GAS, I_RES_AB, IADVICE, &
                     ...
                     ISMOST(MXPCSM,MXPCSM), ISPGPFTP(MXPNGAS,MXPSTT), ISTAC(MXPSTT,2), ITOOBS(MXPOBS), kvec3_length = 0, &
                     LCMBSPC(MXPICI), LCSBLK, LUC, LUDIA, LUHC, LUMOUT, LUSC1, LUSC2, LUSC3, LUSC34, LUSC35, LUSC36, LUSC37, &
                     LUSC38, LUSC39, LUSC40, MAX_STR_OC_BLK, MAX_STR_SPGP, MAXOP, Memory_Needed_Lucia = 0, MINMAX_SM_GP(2,MXPSTT), &
                     MINOP, MNGSOC(MXPNGAS), MNHL, MOCAA, MS2, MULTS, MXGSOC(MXPNGAS), MXINKA, MXNSTR, MXNTTS, MXSB, MXSOOB, &
                     MXTSOB, N_2ELIMINATED_GAS, N_ELIMINATED_BATCHES, N_ELIMINATED_GAS, NACOB, NACOBS(MXPOBS), NACTEL, NBINT1, &
                     NBINT2, NCISPC, NCMBSPC, NCONF_ALL_SYM, NCONF_PER_OPEN(MXPORB+1,MXPCSM), NCONF_PER_SYM(MXPCSM), nconf_tot, &
                     nCSF_HEXS, NCSF_PER_SYM(MXPCSM), NDEOB, NELEC(MXPSTT), NELFGP(MXPSTT), NELFSPGP(MXPNGAS,MXPSTT), &
                     NELFTP(MXPSTT), NELIS(4), NGAS, NGPSTR(MXPNGAS), NGRP, NGSOBT(MXPNGAS), NGSSH(MXPIRR,MXPNGAS), &
                     NHLFSPGP(MXPSTT), NINOB, NINOBS(MXPOBS), NIRREP, NMXOCCLS, NOBPT(6+MXPR4T), NOBPTS(6+MXPR4T,MXPOBS), NOCOB, &
                     NOCSF, NOCTYP(MXPSTT), NOINT, NORB1, NORB2, NORB3, NPCMCNF(MXPORB+1), NPCSCNF(MXPORB+1), NPDTCNF(MXPORB+1), &
                     NPTSPC, NROOT, NSD_PER_SYM(MXPCSM), NSMOB, NSPGPFTP(MXPSTT), NSTFGP(MXPSTT), NSTFSMGP(MXPNSMST,MXPSTT), &
...
integer(kind=iwp), allocatable :: CFTP(:), CI1BT(:), CIBT(:), CLBT(:), CLEBT(:), DFTP(:), IOCLS(:), ISMDFGP(:), ISMSCR(:), &
                                  ISTSGP(:), KINH1(:), KINH1_NOCCSYM(:), LSM1(:), LSM2(:), NACTSYM(:), NSTSGP(:), OCSTR(:,:), &
                                  PINT1(:), PINT2(:), REO(:,:), SPGPAN(:), SPGPCR(:), Z(:,:), ZSCR(:)
...
public :: Allocate_Local_Arrays, CBLTP, CFTP, CI1BT, CI_VEC, CIBT, CLBT, CLEBT, CONF_OCC, CONF_REO, Deallocate_Local_Arrays, DFTP, &
          ...
          ISIMSYM, ISMDFGP, ISMFSO, ISMFTO, ISMOST, ISMSCR, ISPGPFTP, ISTAC, ISTSGP, ISTSO, ITOOBS, KINH1, KINH1_NOCCSYM, &
          kvec3_length, LCMBSPC, LCSBLK, LOFFI, LSM1, LSM2, LUC, LUDIA, LUHC, LUMOUT, LUSC1, LUSC2, LUSC3, LUSC34, LUSC35, LUSC36, &
          LUSC37, LUSC38, LUSC39, LUSC40, MAX_STR_OC_BLK, MAX_STR_SPGP, MAXOP, Memory_Needed_Lucia, MINMAX_SM_GP, MINOP, MNGSOC, &
          MNHL, MOCAA, MS2, MULTS, MXGSOC, MXINKA, MXNSTR, MXNTTS, MXPCSM, MXPIRR, MXPNGAS, MXPNSMST, MXPOBS, MXPORB, MXPSTT, &
          MXPTSOB, MXSB, MXSOOB, MXTSOB, N_2ELIMINATED_GAS, N_ELIMINATED_BATCHES, N_ELIMINATED_GAS, NACOB, NACOBS, NACTEL, &
          NACTSYM, NBINT1, NBINT2, NCISPC, NCMBSPC, NCONF_ALL_SYM, NCONF_PER_OPEN, NCONF_PER_SYM, NCONF_TOT, NCSF_HEXS, &
          NCSF_PER_SYM, NDEOB, NELEC, NELFGP, NELFSPGP, NELFTP, NELIS, NGAS, NGPSTR, NGRP, NGSOBT, NGSSH, NHLFSPGP, NINOB, NINOBS, &
          NIRREP, NMXOCCLS, NOBPT, NOBPTS, NOCOB, NOCSF, NOCTYP, NOINT, NORB1, NORB2, NORB3, NPCMCNF, NPCSCNF, NPDTCNF, NPTSPC, &
          NROOT, NSD_PER_SYM, NSMOB, NSPGPFTP, NSTFGP, NSTFSMGP, NSTFSMSPGP, NSTRKS, NSTSGP, NSTSO, NSTTP, NSTTYP, NTOOB, NTOOBS, &
          NTSPGP, OCCSTR, OCSTR, PAtmp, PGINT1, PGINT1A, PINT1, PINT2, Pscr, PSSIGN, Ptmp, REO, REO_PTDT, RF1, RF2, RHO1, SDREO, &
          SDREO_I, Sigma_on_disk, SIGMA_VEC, SPGPAN, SPGPCR, SRHO1, STREO, STSTM, VEC3, XISPSM, Z, Z_PTDT, Zmat, ZSCR
