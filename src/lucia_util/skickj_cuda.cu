...
bool checked_product4(std::size_t first, std::size_t second, std::size_t third, std::size_t fourth, std::size_t *result) noexcept {
  std::size_t partial = 0;
  return checked_mul(first, second, &partial) && checked_mul(partial, third, &partial) && checked_mul(partial, fourth, result);
}

bool positive_size(int64_t value, std::size_t *result) noexcept {
  if (value <= 0 || static_cast<std::uintmax_t>(value) > static_cast<std::uintmax_t>((std::numeric_limits<std::size_t>::max)())) {
    return false;
  }
...
template <typename T> struct DeviceAllocation {
  T *data = nullptr;
...
__global__ void skickj_route3_kernel(double *skii, const double *ckjj, const double *xijkl, int64_t nka, int64_t nkb, int64_t ni,
                                     int64_t nj, int64_t nk, int64_t nl, int64_t maxk, const int64_t *kbib, const double *xkbib,
                                     const int64_t *kbjb, const double *xkbjb, int64_t ikord, int64_t nib, int64_t njb) {
  ...
  for (std::size_t ka = static_cast<std::size_t>(threadIdx.x); ka < nka_size; ka += blockDim.x) {
    const std::size_t output_offset
        = ka + static_cast<std::size_t>(i) * nka_size + static_cast<std::size_t>(ib) * ni_size * nka_size;
    ...
    for (int64_t kb = 0; kb < nkb; ++kb) {
      for (int64_t k = 0; k < nk; ++k) {
        ...
        const std::size_t map_k_offset = static_cast<std::size_t>(kb) + static_cast<std::size_t>(k) * maxk_size;
        if (kbib[map_k_offset] != ib + 1) {
          continue;
        }
        const double xkbib_value = xkbib[map_k_offset];
        for (int64_t l = 0; l < nl; ++l) {
          const std::size_t map_l_offset = static_cast<std::size_t>(kb) + static_cast<std::size_t>(l) * maxk_size;
          ...
          for (int64_t j = 0; j < nj; ++j) {
            const std::size_t a_offset
                = ka + static_cast<std::size_t>(j) * nka_size + static_cast<std::size_t>(jb - 1) * nj_size * nka_size;
            const std::size_t b_offset
                = static_cast<std::size_t>(j) + static_cast<std::size_t>(i) * nj_size
                  + (static_cast<std::size_t>(l) * static_cast<std::size_t>(nk) + static_cast<std::size_t>(k)) * ni_size * nj_size;
...
extern "C" int64_t lucia_skickj_cuda_route3(double *skii, const double *ckjj, const double *xijkl, int64_t nka, int64_t nkb,
  ...
  if (skii == nullptr || ckjj == nullptr || xijkl == nullptr || kbib == nullptr || xkbib == nullptr || kbjb == nullptr
      || xkbjb == nullptr) {
    return 0;
  }
  ...
  if (!positive_size(nka, &nka_size) || !positive_size(nkb, &nkb_size) || !positive_size(ni, &ni_size)
      || !positive_size(nj, &nj_size) || !positive_size(nk, &nk_size) || !positive_size(nl, &nl_size)
      || !positive_size(maxk, &maxk_size) || !positive_size(nib, &nib_size) || !positive_size(njb, &njb_size)) {
    return 0;
  }
  ...
  if (!checked_product3(nka_size, ni_size, nib_size, &skii_count) || !checked_product3(nka_size, nj_size, njb_size, &ckjj_count)
      || !checked_product4(ni_size, nj_size, nk_size, nl_size, &xijkl_count) || !checked_product2(maxk_size, nk_size, &kbib_count)
      || !checked_product2(maxk_size, nl_size, &kbjb_count) || !checked_product3(nkb_size, nk_size, nl_size, &operation_count)
      || !checked_product2(nib_size, ni_size, &output_plane_count)) {
    return 0;
  }
  std::size_t work_count = 0;
  constexpr std::size_t minimum_work = 300000000;
  if (!checked_product4(nka_size, ni_size, nj_size, operation_count, &work_count) || work_count < minimum_work) {
    return 0;
  }
  ...
  bool success = true;
  do {
    if (!workspace.skii.copy_from(skii, skii_count) || !workspace.ckjj.copy_from(ckjj, ckjj_count)
        || !workspace.xijkl.copy_from(xijkl, xijkl_count) || !workspace.kbib.copy_from(kbib, kbib_count)
        || !workspace.xkbib.copy_from(xkbib, kbib_count) || !workspace.kbjb.copy_from(kbjb, kbjb_count)
        || !workspace.xkbjb.copy_from(xkbjb, kbjb_count) || !workspace.reserve_staging(skii_count)) {
      success = false;
      break;
    }
    ...
    skickj_route3_kernel<<<grid, block>>>(workspace.skii.data, workspace.ckjj.data, workspace.xijkl.data, nka, nkb, ni, nj, nk, nl,
                                          maxk, workspace.kbib.data, workspace.xkbib.data, workspace.kbjb.data,
                                          workspace.xkbjb.data, ikord, nib, njb);
