...
!$$$ Jesper        IF(IOCC_MIN(IORB).LE.IEL.AND.IEL.LE.IOCC_MAX(IORB)) THEN
!
          IF(IEL.EQ. 0 )                                                &
...
!$$$ Jesper ???  CALL REDUCE_VERTEX_WEIGHTS(IVERTEXW(NORB+1,NEL+1), IVERTEXW,
!$$$ Jesper ??? &                           NEL, NORB, IOCC_MIN, IOCC_MAX)
!
      NTEST = 00
