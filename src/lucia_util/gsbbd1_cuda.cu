...
struct Workspace {
  ...
  bool session_matches(double *rho1_host, double *srho1_host, const double *sb_host,
                       const double *cb_host, std::size_t density_count,
                       std::size_t sb_count, std::size_t cb_count,
                       bool update_spin) const noexcept
  {
    return session_active && session_rho1_host == rho1_host &&
           session_srho1_host == srho1_host && session_sb_host == sb_host &&
           session_cb_host == cb_host && session_density_count == density_count &&
           session_sb_count == sb_count && session_cb_count == cb_count &&
           session_update_spin == update_spin;
  }

  bool begin_session(double *rho1_host, double *srho1_host, const double *sb_host,
                     const double *cb_host, std::size_t density_count,
                     std::size_t sb_count, std::size_t cb_count,
                     bool update_spin) noexcept
  {
    if (session_active) {
      return false;
    }
  ...
  bool begin_map_session(const std::int64_t *i1_host, const double *xi1s_host,
                         const std::int64_t *i2_host, const double *xi2s_host,
  ...
  bool map_session_matches(const std::int64_t *i1_host, const double *xi1s_host,
                           const std::int64_t *i2_host, const double *xi2s_host,
                           std::size_t i1_count, std::size_t i2_count) const noexcept
  {
    return map_session_active && map_i1_host == i1_host && map_xi1s_host == xi1s_host &&
           map_i2_host == i2_host && map_xi2s_host == xi2s_host && map_i1_count == i1_count &&
           map_i2_count == i2_count;
  }
  ...
  bool flush_session() noexcept
  {
    ...
    if (!lucia_cuda::checked_mul(session_density_count, sizeof(double), &density_bytes) ||
        !rho1_staging.reserve(session_density_count) ||
        (session_update_spin && !srho1_staging.reserve(session_density_count))) {
      return false;
    }
    const bool rho1_success =
        cudaMemcpy(rho1_staging.data, rho1.data, density_bytes, cudaMemcpyDeviceToHost) == cudaSuccess;
    const bool srho1_success =
        !session_update_spin ||
        cudaMemcpy(srho1_staging.data, srho1.data, density_bytes, cudaMemcpyDeviceToHost) == cudaSuccess;
  ...
  bool release() noexcept
  {
    ...
    clear_map_session();
    if (!rho1.release()) success = false;
    if (!srho1.release()) success = false;
    if (!sb.release()) success = false;
    if (!cb.release()) success = false;
    if (!xi1s.release()) success = false;
    if (!xi2s.release()) success = false;
    if (!i1.release()) success = false;
    if (!i2.release()) success = false;
    if (!rho1_staging.release()) success = false;
    if (!srho1_staging.release()) success = false;
    return success;
  }
};

__global__ void gsbbd1_cuda_kernel(
    double *rho1, double *srho1, const double *sb, const double *cb,
    const std::int64_t *i1, const double *xi1s, const std::int64_t *i2,
    const double *xi2s, std::size_t nacob, std::size_t nrow, std::size_t ibot0,
    std::size_t nibtc, std::size_t nkastr, std::size_t kbot0, std::size_t lkabtc,
    std::size_t d1, std::size_t off1, std::size_t off2, bool update_spin, double xab)
{
...
extern "C" int64_t lucia_gsbbd1_cuda_begin(
    double *rho1, double *srho1, const double *sb, const double *cb, int64_t nacob,
    int64_t nrow, int64_t nsb, int64_t ncb, int64_t idosrho1)
{
  if (rho1 == nullptr || srho1 == nullptr || sb == nullptr || cb == nullptr ||
  ...
  if (!lucia_cuda::positive_size(nacob, &nacob_size) ||
      !lucia_cuda::positive_size(nrow, &nrow_size) ||
      !lucia_cuda::positive_size(nsb, &nsb_size) ||
      !lucia_cuda::positive_size(ncb, &ncb_size) || idosrho1 < 0 || idosrho1 > 1 ||
      ...
      !workspace.begin_session(rho1, srho1, sb, cb, density_count, sb_count, cb_count,
                               idosrho1 == 1)) {
    return 0;
  }
  return 1;
...
extern "C" int64_t lucia_gsbbd1_cuda_maps_begin(
    const int64_t *i1, const double *xi1s, const int64_t *i2, const double *xi2s,
...
extern "C" int64_t lucia_gsbbd1_cuda_route(
    double *rho1, double *srho1, const double *sb, const double *cb, const int64_t *i1,
    const double *xi1s, const int64_t *i2, const double *xi2s, int64_t nacob,
    int64_t nrow, int64_t nsb, int64_t ncb, int64_t ibot, int64_t nibtc,
    int64_t nkastr, int64_t kbot, int64_t lkabtc, int64_t d1, int64_t d2,
    int64_t off1, int64_t off2, int64_t idosrho1, double xab)
{
  if (rho1 == nullptr || srho1 == nullptr || sb == nullptr || cb == nullptr || i1 == nullptr ||
      xi1s == nullptr || i2 == nullptr || xi2s == nullptr || !std::isfinite(xab)) {
  ...
  if (!lucia_cuda::positive_size(nacob, &nacob_size) ||
      ...
      !lucia_cuda::positive_size(kbot, &kbot_size) ||
      !lucia_cuda::positive_size(lkabtc, &lkabtc_size) ||
      !lucia_cuda::positive_size(d1, &d1_size) ||
      !lucia_cuda::positive_size(d2, &d2_size) || off1 < 0 || off2 < 0 ||
      idosrho1 < 0 || idosrho1 > 1) {
    return workspace.fallback_status();
  }
  ...
  if (ibot0 >= nrow_size || nibtc_size > nrow_size - ibot0 ||
      kbot0 >= nkastr_size || lkabtc_size > nkastr_size - kbot0 ||
      off1_size >= nacob_size || d1_size > nacob_size - off1_size ||
      off2_size >= nacob_size || d2_size > nacob_size - off2_size) {
    return workspace.fallback_status();
  }
  ...
  if (!lucia_cuda::checked_mul(nacob_size, nacob_size, &density_count) ||
      !lucia_cuda::checked_mul(nrow_size, nsb_size, &sb_count) ||
      !lucia_cuda::checked_mul(nrow_size, ncb_size, &cb_count) ||
      !lucia_cuda::checked_mul(nkastr_size, d2_size, &i1_count) ||
      !lucia_cuda::checked_mul(nkastr_size, d1_size, &i2_count) ||
      !lucia_cuda::checked_mul(d1_size, d2_size, &pair_count) ||
  ...
  const bool session_match = workspace.session_matches(
      rho1, srho1, sb, cb, density_count, sb_count, cb_count, update_spin);
  ...
  if ((!session_match || upload_blocks) &&
      (!workspace.rho1.copy_from(rho1, density_count) ||
       (update_spin && !workspace.srho1.copy_from(srho1, density_count)))) {
    return workspace.fallback_status();
  }
  ...
  if ((!session_match || upload_blocks) &&
      (!workspace.rho1_staging.reserve(density_count) ||
       (update_spin && !workspace.srho1_staging.reserve(density_count)))) {
    return workspace.fallback_status();
  }

  gsbbd1_cuda_kernel<<<static_cast<unsigned int>(pair_count), block_size>>>(
      workspace.rho1.data, workspace.srho1.data, device_sb, device_cb,
      workspace.i1.data, workspace.xi1s.data, workspace.i2.data, workspace.xi2s.data,
      nacob_size, nrow_size, ibot0, nibtc_size, nkastr_size, kbot0, lkabtc_size,
      d1_size, off1_size, off2_size, update_spin, xab);
  ...
  if (!lucia_cuda::checked_mul(density_count, sizeof(double), &density_bytes) ||
      ...
      (update_spin && cudaMemcpy(workspace.srho1_staging.data, workspace.srho1.data, density_bytes,
