...
!***********************************************************************
  ...
  integer :: Ntime,Noutstep, ihh, imm, iss
  real(8) :: time, error_rk, oldstep, t_temp
  integer, external :: isfreeunit
  procedure(rk_fixed_step) :: classic_rk4,rk4,rk5
  procedure(rk_adapt_step) :: rk45,rkck
  procedure(pulse_func)    :: pulse
  character(64) :: out2_fmt, out3_fmt

#include "timers.fh"

  call dashes()
  write(*,*) 'Propagation starts'
  call dashes()
...
! this file is for the TD-dipole moment data
      ...
      densityt= density0
      call cre_out()
      call mh5_put_dset_array_real(out_ham_r,dble(hamiltonian))
      call mh5_put_dset_array_real(out_ham_i,aimag(hamiltonian))
      call mh5_put_dset_array_real(out_decay_r,dble(decay))
      call mh5_put_dset_array_real(out_decay_i,aimag(decay))
	  if (flag_emiss) then
        call mh5_put_dset_array_real(out_freq, emiss)
!        write(*,*) 'frequencies have been written'
        emiss = 0d0
      endif
      ...
      call pop(time,ii)
      if (flag_fdm) then 
        call mh5_put_dset_array_real(out_tfdm,[time/fstoau],
     &                               [1],[0])
        call mh5_put_dset_array_real(out_fdm,abs(density0),
     &                               [1,d,d],[0,0,0])
      endif
...
!***********************************************************************
      if (method=='RKCK'.or.method=='RK45') then
        ...
        if (ipglob>3) then
          call dashes(72)
          write(*,*) 'Propagation with ',trim(method),' method'
          write(*,*) 'Integration values at each step:'
          write(*,out2_fmt) 'Time','Error','Timestep','Walltime'
        endif
...
!            call dcopy_(d**2,density0,1,densityt,1)
            ...
            if (ipglob>3) write(*,*) ' ------',error_rk,dt/fstoau
          enddo loopstep
!         step succeeded
          time=time+dt
          oldstep=dt
...
!         write info and elapsed time
          ...
          if (flag_fdm.and.time>=time_fdm*jj) then
            ...
            call mh5_put_dset_array_real(out_tfdm,[time/fstoau],
     &                                   [1],[jj])
            call mh5_put_dset_array_real(out_fdm,abs(densityt),
     &                         [1,d,d],[jj,0,0])
            jj = jj + 1
          endif
          ...
          if (ipglob>3) write(*,out3_fmt) time/fstoau, error_rk,
     &                        dt/fstoau, ihh,':',imm,':',iss
!         next step size
          if (method=='RKCK') then
          ...
          Ntime=Ntime+1
          if (dt<1d-8) then
            write(*,*) "The step of integration got too small,"
