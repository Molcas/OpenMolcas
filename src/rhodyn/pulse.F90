...
use rhodyn_data, only: zero, E_field, N_pulse, pulse_type, pulse_vec, pulse_vector, amp, taushift, sigma, power_shape, omega, phi, &
                       lu_pls, temp_vec, out_t, out_pulse, dipole_basis, t_local, omega_local, linear_chirp, flag_acorrection
...
do i=1,N_pulse
  ...
  if (pulse_type(1:3) == 'SIN') then
    if (abs(t_local) <= sigma(i)) then
      E_field = E_field+amp(i)*pulse_vec*sin(pi*t_local/(2.d0*sigma(i)))**power_shape*sin(omega_local*t_local+phi(i))
  ...
  ! duration of the pulse equals 2\sigma
    ...
    if (abs(t_local) <= sigma(i)) then
      E_field = E_field+amp(i)*pulse_vec*cos(pi*t_local/(2.d0*sigma(i)))**power_shape*sin(omega_local*t_local+phi(i))
  ...
  ! think of more clever definition
    ...
    E_field(1) = amp(i)*sin(omega(i)*time+phi(i))*exp(-(time-taushift(i))**2/(2*sigma(i)**2))
    E_field(2) = amp(i)*cos(omega(i)*time+phi(i))*exp(-(time-taushift(i))**2/(2*sigma(i)**2))
    E_field(3) = 0d0

  else if (pulse_type == 'GAUSS_L_CIRCLE') then
    E_field(1) = amp(i)*cos(omega(i)*time+phi(i))*exp(-(time-taushift(i))**2/(2*sigma(i)**2))
    E_field(2) = amp(i)*sin(omega(i)*time+phi(i))*exp(-(time-taushift(i))**2/(2*sigma(i)**2))
    E_field(3) = 0d0
  end if
end do
...
if (count >= 0) then
  write(lu_pls,'(7(g25.15e3,2x))') time*auToFs,(dble(E_field(i)),aimag(E_field(i)),i=1,3)
  do i=1,3
