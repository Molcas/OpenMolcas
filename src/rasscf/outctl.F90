...
if ((IPRLEV >= DEBUG) .and. (.not. lOPTO)) then
  ...
  if (kIVO) then
    write(LF,Fmt2//'A,T47)') 'Improved Virtual Orbitals option is used'
    write(LF,Fmt2//'A,T47)') 'Molecular Orbitals are NOT suitable for CASPT2 & MRCI!'
  end if
...
# endif
  ...
  if (ICICH == 0) then
    ...
    do i=1,nRoots
      ...
      write(LF,Fmt2//'A,T45,10I6)') 'Reference configuartions',(iCI(iRef,i),iRef=1,mxRef)
      write(LF,Fmt2//'A,T45,10F6.3)') 'CI-coeff',(cCI(iRef,i),iRef=1,mxRef)
    end do
  end if
  if (doDMRG) then
    write(LF,Fmt2//'A,T45,I6)') 'highest root included in the DMRG',LROOTS
  else
    write(LF,Fmt2//'A,T45,I6)') 'highest root included in the CI',LROOTS
  end if
  if (irlxroot /= 0) write(LF,Fmt2//'A,T45,I6)') 'Root passed to geometry opt.',iRlxRoot
  if (lRF) then
...
if ((IPRLEV >= USUAL) .and. (.not. lOPTO)) then
  ...
  write(LF,Fmt2//'A)') '------------------------------'
  write(LF,*)
  write(LF,Fmt2//'A,T45,F20.8)') 'Average CI energy',EAV

  if (KeyCION .and. doDMRG) then
    !write(6,*) 'KeyCION.and.doDMRG',KeyCION,doDMRG -- yma
    ! If DMRG, no 2'-DMs thus omit ECAS and other properties
  else
    if (irlxroot == 0) then
      write(LF,Fmt2//'A,T45,F20.8)') 'RASSCF energy',ECAS
    else
      write(LF,Fmt2//'A,I2,T45,F20.8)') 'RASSCF energy for state ',irlxroot,ECAS
    end if

    write(LF,Fmt2//'A,T45,F20.8)') 'Super-CI energy',ESX
    write(LF,Fmt2//'A,T45,F20.8)') 'RASSCF energy change',DE
    write(LF,Fmt2//'A,T50,ES10.3)') 'Max change in MO coefficients',CMAX
    write(LF,Fmt2//'A,T50,ES10.3)') 'Max non-diagonal density matrix element',ROTMAX
    write(LF,Fmt2//'A,T50,ES10.3)') 'Maximum BLB matrix element',CBLBM
...
if ((IPRLEV >= USUAL) .and. (.not. lOPTO)) then
  ...
  if ((nMVInt+nDCInt) /= 0) then
    ...
    do i=1,lRoots
      call PrintResult(LF,Fmt2//'A,I3,A,F16.8)','RASSCF root number',I,' Total energy:',EneTmp(i),1)
    end do
  end if
...
do KROOT=1,LROOTS
  ...
  if (IPRLEV >= USUAL) then
    ...
    do i=1,NTOT ! very "inefficient" but the simplest
      xnu = OCCN(i)/2.0d0
      if (xnu > 0.0d0) vNentropy = vNentropy+xnu*log(xnu)
    end do
    vNentropy = -vNentropy/log(2.0d0)

    write(LF,'(6X,A,I3,A,F8.5)') 'Von Neumann Entropy (Root ',KROOT,') = ',vNentropy
    write(LF,*)

    ! Compute Mulliken's population analysis

    write(LF,'(/6X,A,I3)') 'Mulliken population analysis for root number:',KROOT
    write(LF,'(6X,A)') '-----------------------------------------------'
    write(LF,*)
    ...
    write(LF,'(/6X,A,I3)') 'Expectation values of various properties for root number:',KROOT
    write(LF,'(6X,A)') '-----------------------------------------------------------'
    write(LF,*)
  end if
  ...
  if (KROOT <= MAXORBOUT) then
    ...
    if (IPRLEV >= VERBOSE) then
      ...
      if (ISPDEN == 1) then
        ...
        write(LF,'(/6X,A,I3)') 'Spin density matrix for root number:',KROOT
        write(LF,'(6X,A)') '--------------------------------------'
        write(LF,*)
        IND = 1
        IDIMV = 0
...
50        continue
    ...
    if (IPRLEV >= USUAL) then
      ...
      if (ISPDEN == 1) then
        write(LF,'(/6X,A,I3)') 'Mulliken spin population analysis for root number:',KROOT
        write(LF,'(6X,A)') '---------------------------------------------------'
        write(LF,*)
      ...
      if (get_BasisType('ANO')) then
        write(LF,'(/6X,A,I3)') 'LoProp population analysis for root number:',KROOT
        write(LF,'(6X,A)') '-----------------------------------------------'
        write(LF,*)
        write(LF,*)
        Line = ''
        ...
        call LoProp(iRC)
        write(LF,*)
        write(LF,'(6X,A,I3)') 'Natural Bond Order analysis for root number:',KROOT
