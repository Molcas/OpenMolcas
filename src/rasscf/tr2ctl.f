...
c      CALL iDAFILE(LUTWOMO,1,iTraToc,nTraToc,IAD13)
*
*     Initiate unit LUTWOAO (two-electron integrals in AO basis)
*
      Call f_Inquire(FnTwoAo,FoundTwoEls)
c      inquire(File='ORDINT',opened=i_op,exist=FoundTwoEls,
c     &  number=LutwoAO)
      Call DecideOnDirect(.False.,FoundTwoEls,DoDirect,DoCholesky)
c      If (.not.DoCholesky) CALL OPNORD(IRC,0,FNTWOAO,LUTWOAO)
...
*
      IF ( NSYM2.NE.NSYM ) THEN
         Write (6,*) 'Tr2Ctl_rasscf: NSYM2.NE.NSYM'
         Write (6,*) 'NSYM2=',NSYM2
         Write (6,*) 'NSYM=',NSYM
         Call QTrace()
         Call Abend()
      ...
      DO ISYM=1,NSYM
        ...
        NB2=NBSX(ISYM)
        IF ( NB1.NE.NB2 ) THEN
           Write (6,*) 'Tr2Ctl_rasscf: NB1.NE.NB2'
           Write (6,*) 'NB1=',NB1
           Write (6,*) 'NB2=',NB2
           Call QTrace()
           Call Abend()
...
*
              ...
              IF( KEEPT.NE.0 ) THEN
                Write (6,*) 'Tr2Ctl_rasscf: NORBP.NE.0.AND.KEEPT.NE.0'
                Write (6,*) 'NORBP=',NORBP
                Write (6,*) 'KEEPT=',KEEPT
                Call QTrace()
                Call Abend()
...
*
              IF(MEMX.LT.NOVX) then
                Write (6,*) 'Tr2Ctl_rasscf: MEMX.LT.NOVX'
                Write (6,*) 'MEMX=',MEMX
                Write (6,*) 'NOVX=',NOVX
                Call QTrace()
                Call Abend()
...
* NW2 is size of 'X1' in TRAMO, used as LBUF in call to RDORD and RDORD_.
              CALL TRAMO_rasscf(NW2   ,WORK(LW1),nW1,
     &                          WORK(LW2),nW2,
     &                          WORK(LW3),nW3,
     &                          WORK(LW4),nW4,
     &                          WORK(LW5),MEMX,CMO,
