...
************************************************************************
      SUBROUTINE DONEI_rasscf(DLT,DSQ,CMO)
*
...
*
      ...
      WRITE(6,'(6X,A)')'Frozen one-body density matrix in AO bas'//
     &                 ' DONEI_rasscf'
        ISTLT=1
        DO ISYM=1,NSYM
          NB=NBAS(ISYM)
          IF ( NB.GT.0 ) THEN
            ...
            CALL TRIPRT(' ','(D20.14)',DLT(ISTLT),NB)
            ISTLT=ISTLT+NB*(NB+1)/2
          END IF
        END DO
      END IF
*
      Call qExit('Donei_rasscf')
*
      RETURN
      END
