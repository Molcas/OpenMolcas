...
subroutine read_neci_RDM(iroot,weight,tGUGA,ifinal,DMAT,DSPN,PSMAT,PAMAT)
...
# endif
  ...
  do i=1,NRoots
    do j=1,size(iroot)
      if (iroot(j) == i) then
        if (tGUGA) then
          call read_single_neci_GUGA_RDM(iroot(j),temp_DMAT,temp_DSPN,temp_PSMAT,temp_PAMAT)
#       ifdef _HDF5_
        else if (MCM7) then
          call read_hdf5_denmats(iroot(j),temp_DMAT,temp_DSPN,temp_PSMAT,temp_PAMAT)
#       endif
        else
          call read_single_neci_RDM(iroot(j),temp_DMAT,temp_DSPN,temp_PSMAT,temp_PAMAT)
        end if
        ...
        if (ifinal > 1) then
          call RDM_to_runfile(temp_DMAT,temp_DSPN,temp_PSMAT,temp_PAMAT,jDisk)
#         ifdef _HDF5_
          ...
          call mh5_put_dset(wfn_spindens,decompr_dspn,[nac,nac,1],[0,0,iroot(j)-1])
#         endif
  ...
  if (ifinal == 0) call RDM_to_runfile(DMAT,DSPN,PSMAT,PAMAT,jDisk)
...
subroutine read_single_neci_GUGA_RDM(iroot,DMAT,DSPN,PSMAT,PAMAT)
  ...
  call verify_(tExist,'PSMAT.'//str(iroot)//' does not exist')
  call f_Inquire('PAMAT.'//str(iroot),tExist)
  call verify_(tExist,'PAMAT.'//str(iroot)//' does not exist')
...
subroutine read_single_neci_RDM(iroot,DMAT,DSPN,PSMAT,PAMAT)
  ...
  call verify_(tExist,'TwoRDM_aaaa.'//str(iroot)//' does not exist')
  call f_Inquire('TwoRDM_abab.'//str(iroot),tExist)
  call verify_(tExist,'TwoRDM_abab.'//str(iroot)//' does not exist')
  call f_Inquire('TwoRDM_abba.'//str(iroot),tExist)
  call verify_(tExist,'TwoRDM_abba.'//str(iroot)//' does not exist')
  if (switch) then
    call f_Inquire('TwoRDM_bbbb.'//str(iroot),tExist)
    call verify_(tExist,'TwoRDM_bbbb.'//str(iroot)//' does not exist')
    call f_Inquire('TwoRDM_baba.'//str(iroot),tExist)
    call verify_(tExist,'TwoRDM_baba.'//str(iroot)//' does not exist')
  ...
  rewind(iUnit)
  if (IPRLEV >= DEBUG) then
    write(u6,*) 'p   q   r   s  pq  rs  pqrs RDMval                PSMAT                   PAMAT'
    write(u6,*) '******* AAAA *******'
  end if
  do
    ...
    if ((p > q) .and. (r < s)) PAMAT(pqrs) = PAMAT(pqrs)-fac*RDMval
    if (IPRLEV >= DEBUG) write(u6,'(7I6,3G25.17)') p,q,r,s,pq,rs,pqrs,RDMval,PSMAT(pqrs),PAMAT(pqrs)
    ...
    if (IPRLEV >= DEBUG) write(u6,'(7I6,3G25.17)') p,s,r,q,ps,rq,psrq,RDMval,PSMAT(psrq),PAMAT(psrq)
  ...
  do
    ...
    ! Contribution to PSMAT and PAMAT:
    PSMAT(pqrs) = PSMAT(pqrs)-fac*RDMval
    if (r < s) PAMAT(pqrs) = PAMAT(pqrs)+fac*RDMval
    if (r > s) PAMAT(pqrs) = PAMAT(pqrs)-fac*RDMval
    if (IPRLEV >= DEBUG) write(u6,'(7I6,3G25.17)') p,q,r,s,pq,rs,pqrs,RDMval,PSMAT(pqrs),PAMAT(pqrs)
    ! Contribution to D_alpha (not final):
    if (r == s) D_alpha(pq) = D_alpha(pq)-RDMval
    ...
    do p=1,nAc
      ...
      do k=1,nAc
        ...
        if (pk < ks) then
          ...
          n = merge(1,2,(k == s))
          intermed = intermed+(2/n*psmat(pkks)+fac*pamat(pkks))
        end if
      end do
