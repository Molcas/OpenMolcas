...
C
      ...
      IST=1+NDIM
      DO I=1,NROOT
       CALL COVLP(Q(IST),Q(IST),WORK(LDIA),PA,WORK(LSXN),
...
C First form the overlap matrix
      ...
      DO I=1,NTOTDC
       ...
       DO J=1,NDIMH
        IJ=IJ+1
        CALL COVLP(Q(ISTQ),C(ISTC),WORK(LDIA),PA,WORK(LSXN),
...
C
        JST=1
        DO J=1,NTRIAL
         CALL COVLP(Q(IST),Q(JST),WORK(LDIA),PA,WORK(LSXN),
     *              WORK(LC1),WORK(LC2),WORK(LX),OVL)
         S(J)=-OVL
         JST=JST+NDIM
        END DO
