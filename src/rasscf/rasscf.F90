...
if ((.not. Key('ORBO')) .and. (MAXIT /= 0)) then
# ifdef _DMRG_
...
# endif
  ...
  if (Key('ORTH')) call putOrbFile(CMO=CMO(:),orbital_E=DIAF(:),iDoGAS=iDoGAS)
  ...
  if ((.not. DumpOnly) .and. (ifvb /= 2) .and. (.not. dofcidump)) then
      ...
      if (IPRLEV <= 3) then
        if (DoDMRG .and. (ICIONLY == 0)) then
          ...
          write(u6,'(6X,A)') '   sweeps/ dim  /root weight/root iter     energy        change     param    element     '// &
                             ' value   shift minimum  type update   hh:mm:ss'
...
#     endif
    ...
    !*******************************************************************
    !*******************************************************************
    ! IF CIONLY calculation the convergence is skipped
    !*******************************************************************
    !*******************************************************************

    if (IFINAL == 1) exit outer
    if (DE > One) then
      ...
      if (abs(DE) > THRE) cycle outer
      if (abs(CBLBM) > THRSX) cycle outer
      if (abs(ROTMAX) > THRTE) cycle outer
      if ((ITER <= 3) .and. (ICIONLY == 0)) cycle outer  ! 3->0 checking
  ...
  if (.not. DumpOnly) then
    ...
    if (Key('WRMA')) call dump_fciqmc_mats(dmat=DMAT(:),dspn=DSPN(:),psmat=pmat(:),pamat=pa(:))
  end if
  ...
  if (.not. (l_casdft .or. DumpOnly)) then
...
#     ifdef _HDF5_
      ...
      do jRoot=2,lRoots
        ...
        call DDafile(JOBIPH,2,Tmp,nConf,jDisk)
        call Reord2(SGS,EXS,NAC,NACTEL,STSYM,1,CONF,CFTP,Tmp,VecL,kcnf)
        kDisk = IADR15(4)
        do kRoot=1,jRoot-1
          ! Read and reorder the right CI vector
          call DDafile(JOBIPH,2,Tmp,nConf,kDisk)
          call Reord2(SGS,EXS,NAC,NACTEL,STSYM,1,CONF,CFTP,Tmp,VecR,kcnf)
