...
# endif
  ...
  call mh5_init_attr(dsetid,'DESCRIPTION','Type index of the molecular orbitals arranged as blocks of size [NBAS(i)], i=1,#irreps')
  ...
  call mh5_init_attr(wfn_energy,'DESCRIPTION','Energy for each root in the CI, arranged as array of [NROOTS]')

  ! molecular orbital coefficients
  wfn_mocoef = mh5_create_dset_real(wfn_fileid,'MO_VECTORS',1,[NTOT2])
  call mh5_init_attr(wfn_mocoef,'DESCRIPTION','Coefficients of the average orbitals, arranged as blocks of size '// &
                     '[NBAS(i)**2], i=1,#irreps')

  ! molecular orbital occupation numbers
  wfn_occnum = mh5_create_dset_real(wfn_fileid,'MO_OCCUPATIONS',1,[NTOT])
  call mh5_init_attr(wfn_occnum,'DESCRIPTION','Occupation numbers of the average orbitals arranged as blocks of size '// &
                     '[NBAS(i)], i=1,#irreps')

  ! molecular orbital energies
  wfn_orbene = mh5_create_dset_real(wfn_fileid,'MO_ENERGIES',1,[NTOT])
  call mh5_init_attr(wfn_orbene,'DESCRIPTION','Orbital energies of the average orbitals arranged as blocks of size '// &
                     '[NBAS(i)], i=1,#irreps')

  ! molecular orbital symmetry irreps
  wfn_supsym = mh5_create_dset_int(wfn_fileid,'SUPSYM_IRREP_INDICES',1,[NTOT])
  call mh5_init_attr(wfn_supsym,'DESCRIPTION','Supersymmetry ID of the average orbitals arranged as blocks of size '// &
                     '[NBAS(i)], i=1,#irreps')
...
# endif
    ...
    call mh5_init_attr(wfn_cicoef,'DESCRIPTION','Coefficients of configuration state functions in Split-GUGA ordering, size '// &
                       '[NCONF] for each root in NROOTS: [NROOTS,NCONF].')
# ifdef _DMRG_
  end if
# endif
  ...
  call mh5_init_attr(wfn_dens,'DESCRIPTION','active 1-body density matrix, size [NAC,NAC] for each root in NROOTS: '// &
                     '[NROOTS,NAC,NAC].')
...
# ifdef _DMRG_
  if (doDMRG) then
    ...
    call mh5_init_attr(wfn_dmrg_checkpoint,'DESCRIPTION','QCMaquis checkpoint directory names for each root in [NROOTS].')
  end if
# endif
