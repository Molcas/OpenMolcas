...
# endif
  ! Skip if vdW bond.
  if (iTabBonds(3,iBond) == vdW_Bond) Go To 101
...
do kAtom=1,nAtoms
  ...
  do lAtom=kAtom+1,nAtoms
    lSet = nSet(lAtom)
    if (lSet == kSet) Go To 502

    ! Again, add H-H bonds only if necessary
    iOff = (min(kSet,lSet)-1)*iSet+max(kSet,lSet)
    if ((SetDist(iOff) <= HH_Thr**2) .and. ((iANr(kAtom) == 1) .and. (iANr(lAtom) == 1))) Go To 502
