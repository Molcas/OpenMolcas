...
use xc_f03_lib_m, only: xc_f03_family_from_id, xc_f03_functional_get_number, XC_FAMILY_GGA, XC_FAMILY_HYB_GGA, XC_FAMILY_HYB_LDA, &
                        XC_FAMILY_HYB_MGGA, XC_FAMILY_LDA, XC_FAMILY_MGGA
...
do
  ...
  if (to_upper(Word1) == UpLabel) then
    ...
    do i=1,nFuncs
      ExFac = ExFac+Coeffs(i)*get_exx(func_id(i))
    end do
...
function get_exx(id)
  ...
  use xc_f03_lib_m, only: xc_f03_func_end, xc_f03_func_init, xc_f03_func_t, xc_f03_hyb_exx_coef
...
function needs_laplacian(id)
  ...
  use xc_f03_lib_m, only: xc_f03_func_end, xc_f03_func_get_info, xc_f03_func_init, xc_f03_func_info_get_flags, xc_f03_func_t, &
                          XC_FLAGS_HYB_CAM, XC_FLAGS_HYB_CAMY, XC_FLAGS_NEEDS_LAPLACIAN
  ...
  ! Side-effect: check that the functional flags are supported
