...
use InfSCF, only: nnOc, nnFr, nnB, nnO, nBT, nOT, nBO, nBB, nOO, nOV, mOV, kOV, nOFS, nOFT, MaxBas, MaxOrb, MaxORO, MaxBXO, &
                  MaxFro, MaxBOF, MaxORF, MaxBOO, nD, nOCC, nBas, nFro, nOrb, DSCF, nSym
...
if ((nnB > 2*MxBas) .and. (.not. DSCF)) then
  write(6,*) 'SetUp: nnB > 2*MxBas .and. .not.DSCF'
  write(6,*) 'nnB,MxBas=',nnB,MxBas
  call Abend()
else if ((nnB > 4*MxBas) .and. DSCF) then
  write(6,*) 'SetUp: nnB > 4*MxBas .and. DSCF'
  write(6,*) 'nnB,MxBas=',nnB,MxBas
  call Abend()
end if
