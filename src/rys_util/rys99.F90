...
subroutine Rys99(Arg,nArg,Root,Weight,iPntr,nPntr,x0,nMax,R6,R5,R4,R3,R2,R1,R0,W6,W5,W4,W3,W2,W1,W0,ddx,HerW,HerR2,TMax)
!***********************************************************************
!                                                                      *
...
#include "real.fh"
real*8 Arg(nArg), Root(9,nArg), Weight(9,nArg), x0(nMax), R6(nMax,9), R5(nMax,9), R4(nMax,9), R3(nMax,9), R2(nMax,9), R1(nMax,9), &
       R0(nMax,9), W6(nMax,9), W5(nMax,9), W4(nMax,9), W3(nMax,9), W2(nMax,9), W1(nMax,9), W0(nMax,9), HerW(9), HerR2(9)
integer iPntr(nPntr)
...
do iArg=1,nArg
  if (Arg(iArg) < TMax) then
    ...
    Root(5,iArg) = r
    r = (((((R6(n,6)*z+R5(n,6))*z+R4(n,6))*z+R3(n,6))*z+R2(n,6))*z+R1(n,6))*z+R0(n,6)
    Root(6,iArg) = r
    r = (((((R6(n,7)*z+R5(n,7))*z+R4(n,7))*z+R3(n,7))*z+R2(n,7))*z+R1(n,7))*z+R0(n,7)
    Root(7,iArg) = r
    r = (((((R6(n,8)*z+R5(n,8))*z+R4(n,8))*z+R3(n,8))*z+R2(n,8))*z+R1(n,8))*z+R0(n,8)
    Root(8,iArg) = r
    r = (((((R6(n,9)*z+R5(n,9))*z+R4(n,9))*z+R3(n,9))*z+R2(n,9))*z+R1(n,9))*z+R0(n,9)
    Root(9,iArg) = r
    ...
    Weight(5,iArg) = r
    r = (((((W6(n,6)*z+W5(n,6))*z+W4(n,6))*z+W3(n,6))*z+W2(n,6))*z+W1(n,6))*z+W0(n,6)
    Weight(6,iArg) = r
    r = (((((W6(n,7)*z+W5(n,7))*z+W4(n,7))*z+W3(n,7))*z+W2(n,7))*z+W1(n,7))*z+W0(n,7)
    Weight(7,iArg) = r
    r = (((((W6(n,8)*z+W5(n,8))*z+W4(n,8))*z+W3(n,8))*z+W2(n,8))*z+W1(n,8))*z+W0(n,8)
    Weight(8,iArg) = r
    r = (((((W6(n,9)*z+W5(n,9))*z+W4(n,9))*z+W3(n,9))*z+W2(n,9))*z+W1(n,9))*z+W0(n,9)
    Weight(9,iArg) = r
  else
    ai = 1.0d0/Arg(iArg)
    si = sqrt(ai)
    ...
    Root(8,iArg) = HerR2(8)*ai
    Root(9,iArg) = HerR2(9)*ai
    ...
    Weight(8,iArg) = HerW(8)*si
    Weight(9,iArg) = HerW(9)*si
  end if
end do

return

end subroutine Rys99
