    ...
    if (ptr != NULL) {
      ...
      case 't':
        ...
        *(ptr - 1) = 0;
        break;
      default:
        printf("Unknown units for MOLCAS_MEM\n");
        break;
      }
    }
...
# endif
  for (i = 0; i < MAXREC; i++)
    if (mentries[i].offset == tmp->offset)
      break;
# ifdef _DEBUGPRINT_MEM_
  printf("#i=%ld\n", LIFMT(i));
  if (mentries[i].offset == tmp->offset)
...
# ifdef _DEBUGPRINT_MEM_
  if (tgt) {
    if (strcmp(tgt->elbl, tmp->elbl) != 0)
      printf("WARNING: Data  labels    are not matching: %s (stored) and %s (requested)\n", tgt->elbl, tmp->elbl);
    if (strcmp(tgt->etyp, tmp->etyp) != 0)
      printf("WARNING: Data  types     are not matching: %s (stored) and %s (requested)\n", tgt->etyp, tmp->etyp);
    if (tgt->len != tmp->len)
      printf("WARNING: Entry lengths   are not matching: %ld (stored) and %ld (requested)\n", LIFMT(tgt->len), LIFMT(tmp->len));
  }
# endif
  return (i);
}
...
#   endif
#   ifdef _DEBUGPRINT_MEM_
    if (mlock((void *)wrkspc, (size_t)tmp->len))
      dump_mentry("Cannot lock memory:", tmp);
#   else
    mlock((void *)wrkspc, (size_t)tmp->len);
#   endif
  }
  if (!wrkspc && tmp->len)
    return (ALLOC_FLD); /* This is an exceptional case, i.e. wrkspc==NULL */
  ...
  case ALLO:
    ...
    if (MlM.avmem < tmp->len) {
      if ((MlM.avmem + MlM.mxmem) < tmp->len) {
        ...
        printf("MEMORY ERROR: Available memory = %ld ( %ld Mb ) !\n", LIFMT(MlM.avmem + MlM.mxmem),
               LIFMT((MlM.avmem + MlM.mxmem) / MB));
