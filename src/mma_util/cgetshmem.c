...
# ifdef _DEBUGPRINT_MEM_
  printf("-------- Deleting shared memory entry '%9s' of type = '%9s' with length=%12ld (offset=%12ld)\n", tmp->elbl, tmp->etyp,
         LIFMT(tmp->len), LIFMT(tmp->offset));
# endif
...
# endif
  ...
  default:
    rc = -1;
    printf("Unsupported shared memory operation %s\n", Op);
  }
  return (rc);
}
