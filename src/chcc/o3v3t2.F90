...
!@x              tento if nebol prevereny dostatocne
...
!X2.3              Calc V4(be',u,ga',v) <- H3(T)(a',be') . V3(a',u,ga',v)
     ...
     &                        dimbe,dima,dim1,                          &
     &                        wrk(PossH3),wrk(PossV3),wrk(PossV4))
!
!X2.4f              Add X(be',u,ga',v) <<- +V4(be',u,ga',v)
!c            pozor na faktor, cele X je s vahou 0.5, sem teda asi 2
              dim1=dimbe*dimga*no*no
...
!X3.1                Calc V2(ga',be',v,u) <- V1(ga',be',v,i) . Goo(i,u)
     ...
     &                         dim1,no,no,                              &
     &                         wrk(PossV1),wrk(PossGoo),wrk(PossV2))
!
...
!X4.5                V4(be',ga',u,v) <- V3(be',ga',ij) . A(ij,u,v)
     ...
     &                         dim1,dim2,no*no,                         &
     &                       wrk(PossV3),wrk(PossA),wrk(PossV4))
!
...
!X4.14          V4(be',ga',u,v) <<- - V3(be',ga',i) . H3(i,u,v)
     ...
     &                         dim1,no,no*no,                           &
     &                         wrk(PossV3),wrk(PossH3),wrk(PossV4))
!
...
!c              pozor na faktor, cele X je s vahou 0.5, sem teda asi 1
                dim1=dimbe*dimga*no*no
...
!Xe.2           V3(be',u,v,j) <- H1(=T1)(be',i) . V2(i,u,v,j)
     ...
     &                         dimbe,no,dim1,                           &
     &                         wrk(PossH1),wrk(PossV2),wrk(PossV3))
!
...
!Xe.4           V2(be',u,v,ga') <- V3(be',u,v,j) . V4(j,ga')
     ...
     &                         dim1,no,dimga,                           &
     &                         wrk(PossV3),wrk(PossV4),wrk(PossV2))
!
...
!Y1.4f        Y(be',u,ga',v) <<- V4(be',u,a',i) . V2(a',i,ga',v)
     ...
     &                       dimbe*no,dimga*no,                         &
     &                       dimbe*no,dima*no,dimga*no,                 &
     &                       wrk(PossV4),wrk(PossV2),wrk(PossY))
!
...
!X1.6f        X(be',u,ga',v) <<- V3(be',u,a',i) . V1(a',i,ga',v)
     ...
     &                       dimbe*no,dimga*no,                         &
     &                       dimbe*no,dima*no,dimga*no,                 &
     &                       wrk(PossV3),wrk(PossV1),wrk(PossX))
!
12             adda=adda+dima
            end do
!
...
!         thus will be executed on that node, where be',a'=1 is
!          calculated
...
!
!@@
!        call Chck_Y (wrk(PossY),dimbe,addbe,dimga,addga)
!        call Chck_X (wrk(PossX),dimbe,addbe,dimga,addga)
!@@
          addga=addga+dimga
          end do
!
...
!T12.1      calc V1(i,u) <<- Fvo(T)(a,i) . T1o(a,u)
     ...
     &                   no,nv,no,                                      &
     &                   wrk(PossFvo),wrk(PossT1o),wrk(PossV1))
!
!T12.2      calc V2(be',u) <<- T1o(be',i) . V1(i,u)
     ...
     &                   dimbe,no,no,                                   &
     &                   wrk(PossT1o),wrk(PossV1),wrk(PossV2))
!
...
!T13.2f     Add H4(be',u) <<- V1(T)(a,be') . T1(a,i)
     ...
     &                     dimbe,nv,no,                                 &
     &                     wrk(PossV1),wrk(PossT1o),wrk(PossH4))
!
!
!T14.1      calc V1(be',u) <- H1(be',i) . Hoo(i,u)
            ...
            call mc0c1a3b (dimbe,no,no,no,dimbe,no,                     &
     &                     dimbe,no,no,                                 &
     &                     wrk(PossH1),wrk(PossHoo),wrk(PossV1))
!
