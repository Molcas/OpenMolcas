...
if (fullprint >= 0) write(6,*) ' Work space requirements :',wrksize
...
do symi=symimin,symimax
  ...
  do symj=symjstart,symjstop
    ...
    do i=istart,istop
      ...
      call cct3_getint(Work(iOff),wrksize,i,symi,possr10,mapdr1,mapir1,rc1)
...
999           format(' I,J,K ',3(i3,1x))
            ...
            if (keyyes == 1) then
              ...
              call minusa(Work(iOff),wrksize,mapdw,-1.0d0)
              call setb(Work(iOff),wrksize,mapdw,mapdv,1.0d0)
            ...
            if (keyyes == 1) then
              ...
              call cct3_add(Work(iOff),wrksize,3,3,0,0,0,0,1,1,-1.0d0*nsg,mapdl2,symijk,mapdw,mapiw,symijk,rc1)
            ...
            if (keyyes == 1) then
              ...
              call cct3_map(Work(iOff),wrksize,3,2,3,1,0,mapdl2,mapil2,ssl2,mapdl1,mapil1,possl10,posst,rc1)
              ssl1 = ssl2
              !3.1.1* add (pack) W(a,bc) <- [L1(a,bc)]
              call cct3_add(Work(iOff),wrksize,3,3,0,0,0,0,1,1,1.0d0*nsg,mapdl1,symijk,mapdw,mapiw,symijk,rc1)
