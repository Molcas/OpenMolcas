...
#include "ioind.fh"
     ...
     &     vab,vbc,vac,mij,mi
!mp      SAVE      kab,kca,kcb,kac,kbc,kc,la,lb,lxa,lxb,lxc,t3a,t3b,
!mp     $     vab,vbc,vac,mij,mi,
!mp     $     iasblock,nuga,nugc
!
...
!mp
      enx=0.d0
!mpn        write (*,*) 'Bef isp, energ(isp), enx = ',isp,energ(isp),enx
!mp!!!      if(energ(isp).eq.0.d0)then
...
!mp      call w_alloc(lxa,nnoab(3)*vblock*n,'lbaT3loopb')
     ...
     & nnoab(3)*vblock*n)
!mpn        write (*,*) 'check lxa'
!mpn        write (*,*) 'nnoab(1), (2), (3) = ',
!mpn     &               nnoab(1),nnoab(2),nnoab(3)
!mpn        write (*,*) 'nnuab(1), (2), (3) = ',
!mpn     &               nnuab(1),nnuab(2),nnuab(3)
       if(nuga.ne.1)then
...
!mp      call w_alloc(mi,noab(isp)*vblock**3,'miT3loopb')
     ...
     & noab(isp)*vblock**3)
!mp
!mpn        write (*,*) 'mi check = ',noab(isp)*vblock**3
!mpn        write (*,*) 'nnoab(1), (2), (3) = ',
!mpn     &               nnoab(1),nnoab(2),nnoab(3)
!mpn        write (*,*) 'nnuab(1), (2), (3) = ',
!mpn     &               nnuab(1),nnuab(2),nnuab(3)
...
!mp      call w_alloc(mij,N*vblock,'mijT3loopb')
     ...
     & N*vblock)
!mp!!!      endif     ! energ = 0 - initialization
!
      aset=(nga-1)*vblock
...
!
!
!mp      call t3_bta_aac(nuga,nugc,g(kab),g(kca),g(kac),g(kc),g(la),g(lxa),
!mp     $g(lxc),g(mi),g(mij),adim,cdim,N,noab(isp),nuab(isp),
!mpn        write (*,*) 'ide call t3_bta_aac'
...
!mp     $,enx,g(vab),g(vac),t1a(noab(isp)*aset+t1_offset_a+1),
!mp     $t1b(noab(isp)*aset+t1_offset_a+1),
!mp     $t1a(noab(3-isp)*cset+t1_offset_b+1),
!mp     $t1b(noab(3-isp)*cset+t1_offset_b+1),g(t3a),g(t3b),ifvo)
      else
!mp      call t3_bta_abc(nuga,nugc,g(kab),g(kcb),g(kca),g(kac),g(kbc),g(kc)
!mp     $,g(la),g(lb),
!mp     $g(lxa),g(lxb),g(lxc),g(mi),g(mij),adim,bdim,cdim,N,noab(isp),
!mpn        write (*,*) 'ide call t3_bta_abc'
!mp
     ...
     &t1a(noab(3-isp)*cset+t1_offset_b+1),                              &
     &t1b(noab(3-isp)*cset+t1_offset_b+1),Work(t3a),Work(t3b),ifvo)
!mp     $,enx,g(vab),g(vbc),g(vac),t1a(noab(isp)*aset+t1_offset_a+1),
!mp     $t1b(noab(isp)*aset+t1_offset_a+1),
!mp     $t1a(noab(isp)*bset+t1_offset_a+1),
!mp     $t1b(noab(isp)*bset+t1_offset_a+1),
!mp     $t1a(noab(3-isp)*cset+t1_offset_b+1),
!mp     $t1b(noab(3-isp)*cset+t1_offset_b+1),g(t3a),g(t3b),ifvo)
      endif   ! cases
!mpn        write (*,*) 'isp, energ(isp), enx = ',isp,energ(isp),enx
      energ(isp)=energ(isp)+enx
!mp!!!        write (*,'(A,i5,x,3(i5,2x),f21.19)') 'Tsk, nga, ngb, ngc, inc = ',
!mp!!!     & jjj,nga,ngb,ngc,enx
!321     continue
!mp        write (6,*)
