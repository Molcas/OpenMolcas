...
if (printkey >= 10) then
  ...
  write(6,'(A,3(i5,1x))') 'maxdim,vblock,vblock_my',maxdim,vblock,vblock_my
end if
...
if (printkey >= 10) then
  write(6,*)
  write(6,'(A,f10.1,A,f7.1,A,f3.1,A)') 'Memory required for the reorg. step = ',8.0d0*mem/1024,' kb ',8.0d0*mem/1024**2,' Mb ', &
                                       8.0d0*mem/1024**3,' Gb '
end if
...
!
if (printkey >= 10) then
  write(6,'(A,f10.1,A,f7.1,A,f3.1,A)') 'Available memory                    = ',8.0d0*mem_avail/1024,' kb ', &
                                       8.0d0*mem_avail/1024**2,' Mb ',8.0d0*mem_avail/1024**3,' Gb '
  write(6,*)
end if
