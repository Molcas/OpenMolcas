...
!mp
!mp        write (*,*) 'ze tak teraz ju dam',i
...
!mp        if (abs(mi(1+imm,i)).gt.10000) then
!mp          write (*,*) 'uz mi dojebane 2',imm+1,i,mi(1+imm,i)
!mp          stop
!mp        end if
!mp        end do
!mp
...
!
      ...
      denc=denb-oepc(c)
      ab=ab+1
!mp        if ((i.eq.14).and.(j.eq.1).and.(k.eq.1)) then
!mp          write (*,'(A,3(i5,x),3(f18.10,x))')
!mp     &               'a,b,c, oepa(a), oepb(b), oepc(c)',
!mp     &                a,b,c,oepa(a),oepb(b),oepc(c)
!mp          write (*,*) 'denc = ',denc
!mp          write (*,*) 'ab, t3a(ab) ',ab,t3a(ab)
!mp        end if
      xx=t3a(ab)
      yy=xx/denc
!!      sumt3=sumt3+xx
      enx=enx+yy*xx
!mp        if ((i.eq.14).and.(j.eq.1).and.(k.eq.1)) then
!mp          write (*,'(A,3(f18.10,x))') 'xx,yy,enx = ',xx,yy,enx
!mp        end if
      t3a(ab)=yy
      enddo
      enddo
...
!mp        write (*,'(3(i5,x),f18.10)') i,j,k,enx
         enddo !j
         enddo !i
         enddo !k
      return
