...
! construct Pab
       if(NoOrb.eq.1) then
!        print *,'nCMO,nMOs', nCMO,nMOs
        Call makePab(Work(ipCMO),Work(ipOcc),Work(ipPab),nMOs,nMOs,     &
     &  nIrrep, nBas    )
!       print *,'Pab=', (Work(ipPab+i),i=0,nMOs-1)
       endif
        if(iErr.eq.1) then
          do j=0, nMOs-1
          Work(ipE+j)=Zero
...
!
      ...
      ive1=max(iGridNpt(1)-1,1)
       ...
       iv2=0
       iv1=0
!      if(isAtom.eq.1) goto 6000

      iiiCoord=0
!      if(isCutOff.eq.1) nCoor=iiCoord
...
!
!      if(isXField.eq.1) call dcopy_(mCoor,Work(ipOut),1,Work(ipOutXF),1)
        if(isUHF.eq.1) then
!VV:
          nDrv=0
...
!         ddNorm=ddNorm+dd
       ...
       Enddo !iSec
!ccccccccccccc  main loop ends here  ccccccccccccccccccccccccccccccccc
...
!        write(LuVal_) str
!      endif
! Well, to avoid rewritting of Cerius2 we use old INPORB format
! temporary!
      LuOrb=isFreeUnit(46)
...
!      if(str.ne.'#ORB') goto 4001
5001  read(LuOrb,'(a)',err=5000,end=5000) str
!      if(str(1:1).eq.'#') goto 5001
      iLen=len(str)
      do 5050 i=iLen,1,-1
