...
!***********************************************************************
     Subroutine LYP(mGrid,Coeff,nD,F_xc)
      use xc_f03_lib_m
      ...
      integer*4, parameter :: func_id = 131
...
!
      ...
      data aconst,bconst,cconst,dconst/0.04918d0,0.132d0,0.2533d0,0.349d0/
...
!
      ...
      dec1dra=(ec1*(1.D0-R)/(3.D0*rho_tot))-(4.D0*aconst*R)*(rhob/rho_tot)**2
      dec1drb=(ec1*(1.D0-R)/(3.D0*rho_tot))-(4.D0*aconst*R)*(rhoa/rho_tot)**2
...
!                                                                      *
!      dF/dRho
       vRho(1,iGrid)=vRho(1,iGrid)+Coeff*(dec1dra+dec2dra+dec3dra+dec4dra)
!      dF/dGaa
       vSigma(1,iGrid)=vSigma(1,iGrid)+Coeff*(dec34dsa+dec34ds)+Coeff*(dec34ds)
!      dF/dGab
101   Continue
      End Do
...
!         end if
      ...
      tmp2=-Cfconst2*p*rhob**(11.0D0/3.D0)*rhoa
      ec2=tmp1+tmp2
      dec2dra=(ec2*dlogodr)+(11.D0*tmp1+3.D0*tmp2)/(3.D0*Max(rhoa,0.5D-50))
      dec2drb=(ec2*dlogodr)+(3.D0*tmp1+11.D0*tmp2)/(3.D0*Max(rhob,0.5D-50))
...
!       dF/dRhoa, dF/dRhob
        vRho(1,iGrid)=vRho(1,iGrid)+Coeff*(dec1dra+dec2dra+dec3dra+dec4dra)
        vRho(2,iGrid)=vRho(2,iGrid)+Coeff*(dec1drb+dec2drb+dec3drb+dec4drb)
!       dF/dGaa, dF/dGab, and dF/dGbb
        vSigma(1,iGrid)=vSigma(1,iGrid)+Coeff*(dec34dsa+dec34ds)
        vSigma(2,iGrid)=vSigma(2,iGrid)+2.0d0*Coeff*dec34ds
        vSigma(3,iGrid)=vSigma(3,iGrid)+Coeff*(dec34dsb+dec34ds)
      End Do
      Else
      Do iGrid = 1, mGrid
      rhoa=Max(Rho_min,Rho(1,iGrid))
