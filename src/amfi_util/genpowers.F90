...
#include "param.fh"
#include "dofuc.fh"
dimension powexp(MxprimL,MxprimL,0:Lmax,0:Lmax,0:(Lmax+Lmax+5)), coulovlp(MxprimL,MxprimL,-1:1,-1:1,0:Lmax,0:Lmax)
    ...
    do Lrun=1,(L1+L2+5)
      do irun2=1,nprimit(L2)
        do irun1=1,nprimit(L1)
          ...
          !bs write(6,*) 'fact',fact,'powexp',powexp(irun1,irun2,L1,L2,Lrun-1)
        ...
        do incl1=-1,1
          if (l1+incl1 >= 0) then ! do not lower l for s-functions
            ...
            do iprim2=1,nprimit(l2)
              ...
              !BS write(6,*) 'fact1',fact1,'factor ',factor,'powexp ',powexp(iprim1,iprim2,l1,l2,n1+n2+1)
              end do
            end do
          end if
        end do
