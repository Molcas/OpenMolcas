...
dimension mcombina(2,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax), angint(ncontl1,ncontl2,ncontl3,ncontl4,*), &
          carteX(ncontl1,ncontl3,ncontl2,ncontl4), preXZ(-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax,-Lmax:Lmax), interxyz(*), &
...
do while (interxyz(irun+1) > 0)
  ...
  select case (interxyz(irun))
    ...
    case (1)
      ...
      factor = dble(isgnM(1,1,1,1))*prexz1234*dble(isgnprod(Mabs1,Mabs2,Mabs3,Mabs4))
