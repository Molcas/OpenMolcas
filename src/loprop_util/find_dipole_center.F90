...
!??C      bx = -0.263
!??* First make an initial bracketing of the minima
!??      Call mnBrak2(ax,bx,cx,fa,fb,fc,multipole_expansion,q_a+qn_a,
!??     &             q_b+qn_b,dipole_a,dipole_b,r_a,r_b)
!??* Use Golden to find the minima
!??      golden = golden2(ax,bx,cx,multipole_expansion,r_threshold,
!??     &             e_threshold,r_best,q_a+qn_a,q_b+qn_b,
!??     &             dipole_a,dipole_b,r_a,r_b)
!??      t = (r_best-half*R)/R
!??      write(6,'(A,3F18.10)')
!??     &       't_sum, r_best, golden = ',t, r_best, golden
!??      call xflush(6)
!
      Return
