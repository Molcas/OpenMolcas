...
real*8 T(3,3), R_test(3), R_Q(3,3), Tmp(3,3), rMu_AB(3), rMu_A(3), rMu_B(3)
...
!?? write(6,*) 'My=',x_my,y_my,z_my
!?? write(6,*) 'R=',x_R,y_R,z_R
...
call DGEMM_('T','N',3,1,3,1.0d0,T,3,EC(1,ij),3,0.0d0,R_test,3)
...
tmp(2,1) = rMP(ij,2,0)
tmp(3,1) = rMP(ij,3,0)
call DGEMM_('T','N',3,1,3,1.0d0,T,3,tmp,nij,0.0d0,rMu_AB,3)
call RecPrt('rMu_AB',' ',rMu_AB,1,3)
tmp(1,1) = rMP(ii,1,0)
tmp(2,1) = rMP(ii,2,0)
tmp(3,1) = rMP(ii,3,0)
call DGEMM_('T','N',3,1,3,1.0d0,T,3,tmp,nij,0.0d0,rMu_A,3)
call RecPrt('rMu_A',' ',rMu_A,1,3)
tmp(1,1) = rMP(jj,1,0)
tmp(2,1) = rMP(jj,2,0)
tmp(3,1) = rMP(jj,3,0)
call DGEMM_('T','N',3,1,3,1.0d0,T,3,tmp,nij,0.0d0,rMu_B,3)
...
call DGEMM_('N','T',3,3,3,1.0d0,R_Q,3,T,3,0.0d0,tmp,3)
call DGEMM_('N','N',3,3,3,1.0d0,T,3,tmp,3,0.0d0,R_Q,3)
call RecPrt('R_Q',' ',R_Q,3,3)
QQ = R_Q(3,3)-Half*(R_Q(1,1)+R_Q(2,2))
R_Test(3) = R_Q(3,3)/(Two*QQ)+R_Test(3)
call RecPrt('EC(Bond system) New',' ',R_Test,1,3)

call DGEMM_('N','N',3,3,3,1.0d0,T,3,R_Test,3,0.0d0,tmp,3)
...
call DGEMM_('T','N',3,1,3,1.0d0,T,3,EC(1,ii),3,0.0d0,R_test,3)
call RecPrt('EC(ii)',' ',R_Test,1,3)
R_temp = R_Test(3)
! Rotate EC(1,jj) to the new coordinate system
call DGEMM_('T','N',3,1,3,1.0d0,T,3,EC(1,jj),3,0.0d0,R_test,3)
