...
if (IfTest) then
  ...
  do i=1,nAtoms+nXAtoms
    write(LuWr,'(1X,A,I3,3(1X,I4,1X,F10.6))') Symbols(i),NAT(i),(iZmat(j,i),Zmat(j,i),j=1,3)
  end do
  write(LuWr,*)
end if
if (iErr /= 0) then
...
Skip = .false.
! Atom #1
if (nAtoms+nXAtoms == 1) Skip = .true.
if (.not. Skip) then
  ! Atom #2
  Coords(3,2) = Zmat(1,2)  ! Z(2)=R
  if (nAtoms+nXAtoms == 2) Skip = .true.
end if
if (.not. Skip) then
  ! Atom #3
  if (iZmat(1,3) == 1) then
