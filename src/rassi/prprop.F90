...
#     endif
  ...
  if (IFSO .and. (NSOPR /= 0)) then

    if (PRMES) then
      ! match the SO property list to the SF property list
      call mma_allocate(PMAP,NSOPR,Label='PMap')
      NMISS = 0
      loop1: do ISOPR=1,NSOPR
        PMAP(ISOPR) = 0
        do IPROP=1,NPROP
  ...
  if (IXYZ > 3) then
    ...
    do IXYZ=1,3
      write(u6,'(2x,a2,2x,3(1x,f10.8,1x),4x,a2,i1,a1,2x,f10.8,2x,a2,2x,3(1x,f8.4,1x),3x,a2,i1,a1,2x,f8.3,2x,a5)') &
        xyzchr(IXYZ),(DTENS(IXYZ,JXYZ),JXYZ=1,3),'D_',IXYZ,':',EVR(IXYZ),xyzchr(IXYZ),TMPVEC(IXYZ,:),'D_',IXYZ,':', &
...
if (IFGCAL .and. (.not. IFSO)) then
  ...
  do while (((ENERGY(min(ISTATE,NSTATE))-ENERGY(1)) <= EPRTHR) .and. (ISTATE <= NSTATE))
    ...
    if (MPLET /= 1) then
      ...
      if (IXYZ > 3) then
        ...
        write(u6,*)
        do IXYZ=1,3
          write(u6,'(2x,a2,2x,3(1x,f10.8,1x),4x,a2,i1,a1,2x,f8.4,3x,a8,i1,a2,2x,f10.3,2x,a3)') xyzchr(IXYZ),GTENS(IXYZ,:),'g_', &
                                                                                               IXYZ,':',EVR(IXYZ),'delta(g_', &
                                                                                               IXYZ,'):', &
                                                                                               (EVR(IXYZ)-FEGVAL)*1.0e3_wp,'ppt'
        end do
  ...
  do while ((ISS <= NSS) .and. (ENSOR(min(ISS,NSS))-ENSOR(1) <= EPRTHR))
    ...
    if (KDGN /= 2) then
      ...
      write(u6,*)
      do IXYZ=1,3
        write(u6,'(2x,a2,2x,3(1x,f10.6,1x),4x,a2,i1,a1,2x,f8.4,2x,a2,2x,3(1x,f8.4,1x))') xyzchr(IXYZ),GTENS(IXYZ,:),'g_',IXYZ, &
