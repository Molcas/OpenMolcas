...
!     INTEGER KFSB,IBLK,ISPD,I,IPOS,IORB,ISYM
...
!     INTEGER IERR,ICPL,KSBSMRS,JMORS,NFSB
      INTEGER IERR,                   NFSB
      INTEGER OCC2MRS
      EXTERNAL OCC2MRS
...
!TEST      write(*,*)' Table at KGSORB:'
!TEST      do ipart=0,napart
!TEST       write(*,'(1x,10i5)')(icnftab(kgsorb+isym+(nsym+1)*ipart)
!TEST     &                                             ,isym=0,nsym)
!TEST      end do
!TEST      write(*,*)' Table at KGSLIM:'
!TEST      write(*,'(1x,10i5)')(icnftab(kgslim+2*(ipart-1)),ipart=1,napart)
!TEST      write(*,'(1x,10i5)')(icnftab(kgslim+1+2*(ipart-1)),ipart=1,napart)
...
!TEST      write(*,'(1x,a,10i5)')'Configuration:',
!TEST     &                          (ORBARR(+iel),iel=1,nocc)
!TEST      write(*,*)' Loop over spin determinants.'
! Loop over spin determinants
          DO ISPD=1,NSPD
!TEST      write(*,'(1x,a,20i3)')'Spin determinant:',
!TEST     &                    (ISPNTAB(KSPN-1+I+NOPEN*(ISPD-1)),I=1,nopen)
            IBLK=IBLK+1
...
!TEST      write(*,*)' Identify substrings.'
!TEST      write(*,'(1x,a,10i5)')'Occupation array:',
!TEST     &                          (OCCARR(+isorb),isorb=1,2*norb)
!C construct occupation array in 0,u,d,2 format
...
!C
            IOEND=0
            ISPEND=0
!TEST      write(*,'(1x,a,10i5)')'NAPART:',NAPART
            DO IPART=1,NAPART
!TEST      write(*,'(1x,a,10i5)')' In loop, IPART:',IPART
             NOP=2*ICNFTAB(KGSORB+(NSYM+1)*IPART)
!TEST      write(*,'(1x,a,10i5)')'            NOP:',NOP
!TEST             IF(NOP.EQ.0) write(*,*)' (Skip it.)'
             IF(NOP.EQ.0) cycle
             NSP=(NOP+MORSBITS-1)/MORSBITS
!TEST      write(*,'(1x,a,10i5)')'            NSP:',NSP
             ISPSTA=ISPEND+1
             ISPEND=ISPEND+NSP
             DO ISPART=ISPSTA,ISPEND
!TEST      write(*,'(1x,a,10i5)')'Loop lims ISPSTA,ISPEND:',ISPSTA,ISPEND
              NO=MIN(NOP,MORSBITS)
              NOP=NOP-NO
              IOSTA=IOEND+1
              IOEND=IOEND+NO
!TEST      write(*,'(1x,a,10i5)')'IOSTA,IOEND:',IOSTA,IOEND
!TEST      write(*,'(1x,a,10i5)')'Occ array:',
!TEST     &                     (OCCARR(+ISORB),ISORB=IOSTA,IOEND)
              IMORS=OCC2MRS(NO,OCCARR(+IOSTA))
!TEST      write(*,'(1x,a,10i5)')'IMORS=',IMORS
...
!TEST              IF(IMORS.NE.JMORS)THEN
!TEST                WRITE(*,*)' Mistranslated morsel!!'
!TEST      write(*,'(1x,a,4i12)')'IMORS->ISBSTR:',IMORS,ISBSTR
!TEST      write(*,'(1x,a,4i12)')'but ISBSTR->IMORS:',ISBSTR,JMORS
!TEST      write(*,'(1x,a,4i12)')'KMRSSBS:',KMRSSBS
!TEST      write(*,'(1x,a,4i12)')'KSBSMRS:',KSBSMRS
!TEST                CALL ABEND()
!TEST              END IF
!TEST      write(*,'(1x,a,10i5)')'ISBSTR:',ISBSTR
! Substring type ISST, nr of such substrings is NDIM
              ISST=ISSTAB(IPOS+1)
!TEST      write(*,'(1x,a,10i5)')'ISST  :',ISST
              STARR(+ISPART)=ISST
              DIM(ISPART)=ISSTAB(KSSTTB+5*(ISST-1))
              SSARR(ISPART)=ISBSTR-SBSET(ISST)
             END DO
            END DO
!TEST      write(*,*)' Finally, substring types and substrings:'
!TEST      write(*,'(1x,a,10i5)')'Substr types:',
!TEST     &                   (STARR(+ISPART),ISPART=1,NASPRT)
!TEST      write(*,'(1x,a,10i5)')'Substrings  :',
!TEST     &                    (SSARR(ISPART),ISPART=1,NASPRT)
!TEST      write(*,'(1x,a,10i5)')'Dimensions  :',
!TEST     &                    (DIM(ISPART),ISPART=1,NASPRT)
...
!TEST      write(*,*)' Arguments in HSHGET call:'
!TEST      write(*,'(1x,a,10i5)')'Key:',
!TEST     &                   (STARR(+ISPART),ISPART=1,NASPRT)
!TEST      write(*,'(1x,a,10i5)')'Size of key:',NASPRT
!TEST      write(*,'(1x,a,10i5)')'Size of items stored:',NASPRT+2
!TEST      write(*,'(1x,a,10i5)')'Items stored at KSSTARR=',KSSTARR
!TEST      write(*,'(1x,a,10i5)')'      Map size  NHSHMAP=',NHSHMAP
!TEST      write(*,'(1x,a,10i5)')'  Map stored at KHSHMAP=',KHSHMAP
            CALL HSHGET(STARR,NASPRT,NASPRT+2,IFSBTAB(KSSTARR),         &
     &                            NHSHMAP,IFSBTAB(KHSHMAP),IFSB)
!TEST      write(*,'(1x,a,10i5)')' Map returns index IFSB=',IFSB
!TEST      write(*,'(1x,a,10i5)')' Item stored there is  =',
!TEST     &                 (IFSBTAB(KSSTARR-1+ISPART+(NASPRT+2)*(IFSB-1)),
!TEST     &                                              ISPART=1,NASPRT+2)
