...
!TEST      write(*,*)' Test print in VERTAB.'
!TEST      write(*,*)' NSYM  :',NSYM
!TEST      write(*,*)' NASPRT:',NASPRT
!TEST      write(*,*)' NSSTP :',NSSTP
!TEST      write(*,*)' Substrings:'
...
!TEST       write(*,'(1x,6i8)')isst,nsbs,npop,ksym,ms2,ispart
!TEST      end do
...
! Determine KMS2MAX = Max spin proj in any subpartition
      ...
      END DO
!TEST      write(*,'(1x,a,8i8)')'KPOPMAX:',KPOPMAX
!TEST      write(*,'(1x,a,8i8)')'KMS2MAX:',KMS2MAX
!----------------------------------------------------------------
...
! electrons (cumulative) up to and including subpartition ISPART:
      ...
      DO IPART=1,NPART
       ...
       END DO
      END DO
!TEST      write(*,*)' The LIMARR array:'
!TEST      write(*,'(1x,8i5)')(limarr(1,ispart),ispart=0,nasprt)
!TEST      write(*,'(1x,8i5)')(limarr(2,ispart),ispart=0,nasprt)
...
!TEST      write(*,'(1x,a,8i8)')'iaddr1,iaddr2:',iaddr1,iaddr2
!TEST      do IADDR=IADDR1,IADDR2-1
!TEST        isst=sstarr(IADDR)
!TEST        kpop=ISSTAB(KSSTP+1+5*(ISST-1))
!TEST        ksym=ISSTAB(KSSTP+2+5*(ISST-1))
!TEST        kms2=ISSTAB(KSSTP+3+5*(ISST-1))
!TEST      write(*,'(1x,a,8i8)')'iaddr,isst,kpop,ksym,kms2:',
!TEST     &                      iaddr,isst,kpop,ksym,kms2
!TEST      end do
!TESTC End of test print
...
! Now allocate the vertex and downchain tables, and loop again:
      ...
      IARC =0
!TEST      write(*,'(1x,a,8i8)')'Nr of vertices NVERT=',NVERT
!TEST      write(*,'(1x,a,8i8)')'Nr of arcs     NARC =',NARC
! --------------------------------------------------------------
...
! The total number of walks in the graph:
      ...
      NFSB=0
      NRDETS=0
!TEST      write(*,*)' A list of all generated FS blocks:'
 500  CONTINUE
...
!TEST      write(*,'(1x,a,8i8)')'IVUP,ISW,NSW:',IVUP,ISW,NSW
       IF(NSW.GT.ISW) LOWEST=LEVUP
! Consult the downarc table:
       ISST =DWNTAB(+1+3*(IARC-1))
       IVER1=DWNTAB(+2+3*(IARC-1))
       IVER2=DWNTAB(+3+3*(IARC-1))
!TEST      write(*,'(1x,a,8i8)')'ISST,IVER1,IVER2:',ISST,IVER1,IVER2
...
!TEST      write(*,*)' Total nr of Slater determinants, NRDETS=',NRDETS
!TEST      write(*,*)' Returning from VERTAB.'
