...
*  NEW VERSION 981122, using arrays SGS,ICIS,IXS.
************************************************************************
*  CITRA
*
*> @brief
...
************************************************************************
      SUBROUTINE CITRA(WFTP,SGS,ICIS,IXS,LSM,TRA,NCO,CI)
...
#include "symmul.fh"
      CHARACTER*8 WFTP
      DIMENSION ICIS(NCISIZE),IXS(NXSIZE)
      Type (SGStruct) SGS
...
C The general case:
        ...
        DO ISYM=1,NSYM
          ...
          NO=NOSH(ISYM)
          IF(NA.NE.0) THEN
            CALL SSOTRA(SGS,ICIS,IXS,ISYM,LSM,NA,NO,
