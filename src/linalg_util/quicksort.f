...
C     SUBROUTINE ISORT (IX, IY, N, KFLAG)
      SUBROUTINE QUICKSORT (IX, IY, N, KFLAG)
...
C   980219  Convert REAL to REAL*8 (RL)
...
CFUE  EXTERNAL XERMSG
C     .. Intrinsic Functions ..
      INTRINSIC ABS, INT
C***FIRST EXECUTABLE STATEMENT  ISORT
      NN = N
      IF (NN .LT. 1) THEN
CFUE     CALL XERMSG ('SLATEC', 'ISORT',
CFUE +      'The number of values to be sorted is not positive.', 1, 1)
         RETURN
      ENDIF
C
      KK = ABS(KFLAG)
...
CFUE     CALL XERMSG ('SLATEC', 'ISORT',
CFUE +      'The sort control parameter, K, is not 2, 1, -1, or -2.', 2,
*    +      1)
         RETURN
      ENDIF
C
