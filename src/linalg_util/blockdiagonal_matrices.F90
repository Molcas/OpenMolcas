...
subroutine from_raw(S_buffer,S)
  ...
  do i_block=1,size(S)
    ...
    do col=1,block_size
      ...
      S(i_block)%block(:,col) = S_buffer(offset:offset-1+block_size)
    end do
...
subroutine to_raw(S,S_buffer)
  ...
  do i_block=1,size(S)
    ...
    do col=1,block_size
      offset = idx_block+(col-1)*block_size
      S_buffer(offset:offset-1+block_size) = S(i_block)%block(:,col)
    end do
    idx_block = idx_block+(block_size**2)
  end do
...
subroutine from_symm_raw(S_buffer,S)
  ...
  do i_block=1,size(S)
    ...
    if (block_size > 0) then
      call square(S_buffer(idx_block:),S(i_block)%block,1,block_size,block_size)
    end if
