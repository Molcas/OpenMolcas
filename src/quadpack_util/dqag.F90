...
call dqage(f,a,b,epsabs,epsrel,key,limit,result,abserr,neval,ier,work(1),work(l1),work(l2),work(l3),iwork,last)
...
real*8 a, abserr, alist, area, area1, area12, area2, a1, a2, b, blist, b1, b2, defabs, defab1, defab2, d1mach, elist, epmach, &
       epsabs, epsrel, errbnd, errmax, error1, error2, erro12, errsum, f, resabs, result, rlist, uflow
...
do 30 last=2,limit
  ...
  if (abs(rlist(maxerr)-area12) <= 0.1d-04*abs(area12) .and. erro12 >= 0.99d+00*errmax) iroff1 = iroff1+1
  ...
  if (max(abs(a1),abs(b2)) <= (0.1d+01+0.1d+03*epmach)*(abs(a2)+0.1d+04*uflow)) ier = 3
...
real*8 a, absc, abserr, b, centr, dhlgth, d1mach, epmach, f, fc, fsum, fval1, fval2, fv1, fv2, hlgth, resabs, resasc, resg, resk, &
...
if (resasc /= 0.0d+00 .and. abserr /= 0.0d+00) abserr = resasc*min(0.1d+01,(0.2d+03*abserr/resasc)**1.5d+00)
...
resasc = resasc*dhlgth
abserr = abs((resk-resg)*hlgth)
if (resasc /= 0.0d+00 .and. abserr /= 0.d+00) abserr = resasc*min(0.1d+01,(0.2d+03*abserr/resasc)**1.5d+00)
if (resabs > uflow/(0.5d+02*epmach)) abserr = max((epmach*0.5d+02)*resabs,abserr)
...
real*8 a, dabsc, abserr, b, centr, dhlgth, d1mach, epmach, f, fc, fsum, fval1, fval2, fv1, fv2, hlgth, resabs, resasc, resg, resk, &
       reskh, result, uflow, wg, wgk, xgk
integer j, jtw, jtwm1
external f
