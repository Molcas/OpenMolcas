...
!***********************************************************************
! This file adapted from from MINRES:                                  *
...
real*8 shift, rtol, Anorm, Acond, rnorm, ynorm, b(n), r1(n), r2(n), v(n), w(n), w1(n), w2(n), x(n), y(n)
! ------------------------------------------------------------------
!
...
real*8 alfa, beta, beta1, cs, dbar, delta, denom, diag, eps, epsa, epsln, epsr, epsx, gamma, gbar, gmax, gmin, oldb, oldeps, &
       qrnorm, phi, phibar, rhs1, rhs2, s, sn, t, tnorm2, ynorm2, z
integer i
logical debug, prnt
...
data msg /'beta2 = 0.  If M = I, b and x are eigenvectors of A','beta1 = 0.  The exact solution is  x = 0', &
          'Requested accuracy achieved, as determined by rtol','Reasonable accuracy achieved, given eps', &
          'x has converged to an eigenvector','Acond has exceeded 0.1/eps','The iteration limit was reached', &
          'Aprod  does not define a symmetric matrix','Msolve does not define a symmetric matrix', &
...
900 continue
if (nout > 0) then
  write(nout,2000) exit,istop,itn,exit,Anorm,Acond,exit,rnorm,ynorm
  write(nout,3000) exit,msg(istop)
end if

return

1000 format(//1p,a,5x,'Solution of symmetric   Ax = b'/' n      =',i7,5x,'checkA =',l4,12x,'precon =',l4/' itnlim =',i7,5x, &
            'rtol   =',e11.2,5x,'shift  =',e23.14)
1200 format(//5x,'itn',8x,'x(1)',10x,'norm(r)',3x,'norm(A)',3X,'cond(A)')
1300 format(1p,i8,e19.10,3e10.2)
1500 format(1x)
2000 format(/1p,a,5x,'istop =',i3,14x,'itn   =',i8/a,5x,'Anorm =',e12.4,5x,'Acond =',e12.4/a,5x,'rnorm =',e12.4,5x,'ynorm =',e12.4)
3000 format(a,5x,a)
