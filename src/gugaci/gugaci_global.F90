...
integer(kind=iwp), parameter :: lenintegral = 4, loputmp = 10000, max_atom = 200, max_extorb = 300, max_h0 = 13000, &
                                ...
                                max_wei = 208000, maxgdm = 8, maxpl = 300000, mtmp = max_orb*(max_orb-1)/2, ntrabuf = 9600, &
                                ntratoc = 106, nvaltype = 200000
...
integer(kind=iwp) :: ibsm_ext(maxgdm), ican_a(max_orb), ican_b(mtmp+max_orb), icano_nnend, icano_nnsta, icnt_base, &
                     idisk_array(13), idisk_lp, idownwei_g131415, iesm_ext(maxgdm), ifrno(max_h0), ildownwei_segdd, ilsegdownwei, &
                     iml, imr, index_lpext3(max_extorb,max_innorb), index_lpext4(max_extorb,max_innorb), index_lpext5(max_extorb), &
                     ...
                     ngw3(max_orb), ngw4(max_orb), nint_g25, nint_g28, nlg1, nlg2, nlsm_all(maxgdm), nlsm_bas(maxgdm), &
                     nlsm_dbl(maxgdm), nlsm_ext(maxgdm), nlsm_frz(maxgdm), no(0:max_innorb), nohy, noidx(8), norb_act, norb_all, &
...
real(kind=wp) :: cm_cri, dm1tmp(max_orb*(max_orb+1)/2), ecih0(max_root), escf(max_root), fg, pd, pdd, pror, ps1, ps2, ps3, ps4, &
                 ...
                 w1plp31, w1plp32 !, cf(max_orb,max_orb), dm1(max_orb,max_orb), dxyz(3,max_atom), p(max_orb,max_orb),
                 !xlgrn(max_orb,max_orb)
