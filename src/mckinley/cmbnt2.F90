      ...
      do iby=0,iybMax
        ...
        do iCoor=1,3
          ...
          if (IfHss(0,iCoor-1,0,iCoor-1)) then
            do iZeta=1,nZeta
              ...
              if (ia(iCoor) > 0) then
                rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)*Four*Alpha(iZeta)*real(ia(iCoor),kind=wp)* &
                                          ((Two*Beta(iZeta))**2* &
                                           (Rnxyz(iZeta,iCoor,ia(iCoor),ib(iCoor)+2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))+ &
                ...
                if (lb > 0) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)+rKappa(iZeta)* &
                                            Four*Beta(iZeta)*real(lb,kind=wp)*Four*Alpha(iZeta)*real(ia(iCoor),kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor),ib(iCoor))* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))
                end if
                if (ib(iCoor) > 1) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)* &
                                            real(ib(icoor)*(ib(icoor)-1),kind=wp)*Four*Alpha(iZeta)*real(ia(iCoor),kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor),ib(iCoor)-2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))
                end if
                if (ib(jCoor) > 1) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)* &
                                            real(ib(jcoor)*(ib(jcoor)-1),kind=wp)*Four*Alpha(iZeta)*real(ia(iCoor),kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor),ib(iCoor))* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor)-2)* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))
                end if
                if (ib(kCoor) > 1) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)* &
                                            real(ib(kcoor)*(ib(kcoor)-1),kind=wp)*Four*Alpha(iZeta)*real(ia(iCoor),kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor),ib(iCoor))* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor)-2)
                end if
              ...
              if (ia(iCoor) > 1) then
                rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)+rKappa(iZeta)*real(ia(iCoor)*(ia(iCoor)-1),kind=wp)* &
                                          ((Two*Beta(iZeta))**2* &
                                           (Rnxyz(iZeta,iCoor,ia(iCoor)-2,ib(iCoor)+2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))+ &
                ...
                if (lb > 0) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)* &
                                            real(ia(iCoor)*(ia(iCoor)-1),kind=wp)*Four*Beta(iZeta)*real(lb,kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor)-2,ib(iCoor))* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor),ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))
                end if
        ...
        do kCoor=1,3
          ...
          if (IfHss(0,iCoor-1,0,jCoor-1)) then
            do iZeta=1,nZeta
              rFinal(iZeta,ipa,ipb,i) = rKappa(iZeta)*(Two*Alpha(iZeta))**2* &
                                        ((Two*Beta(iZeta))**2* &
              ...
              if (ia(icoor) > 0) then
                rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)*real(ia(icoor),kind=wp)*Two*Alpha(iZeta)* &
                                          ((Two*Beta(iZeta))**2* &
                                           (Rnxyz(iZeta,iCoor,ia(iCoor)-1,ib(iCoor)+2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor)+1,ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))+ &
              ...
              if (ia(jcoor) > 0) then
                rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)*real(ia(jcoor),kind=wp)*Two*Alpha(iZeta)* &
                                          ((Two*Beta(iZeta))**2* &
                                           (Rnxyz(iZeta,iCoor,ia(iCoor)+1,ib(iCoor)+2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor)-1,ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))+ &
                ...
                if (ib(kcoor) > 1) then
                  rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)-rKappa(iZeta)* &
                                            Two*Alpha(iZeta)*real(ia(jCoor),kind=wp)*real(ib(kcoor)*(ib(kcoor)-1),kind=wp)* &
                                            Rnxyz(iZeta,iCoor,ia(iCoor)+1,ib(iCoor))* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor)-1,ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor)-2)
                end if
              ...
              if ((ia(iCoor) > 0) .and. (ia(jCoor) > 0)) then
                rFinal(iZeta,ipa,ipb,i) = rFinal(iZeta,ipa,ipb,i)+rKappa(iZeta)*real(ia(iCoor)*ia(jCoor),kind=wp)* &
                                          ((Two*Beta(iZeta))**2* &
                                           (Rnxyz(iZeta,iCoor,ia(iCoor)-1,ib(iCoor)+2)* &
                                            Rnxyz(iZeta,jCoor,ia(jCoor)-1,ib(jCoor))* &
                                            Rnxyz(iZeta,kCoor,ia(kCoor),ib(kCoor))+ &
      ...
      do jCnt=0,1
        do jCar=1,3
          ...
          if (IndHss(iCnt,iCar-1,jCnt,jCar-1,iIrrep) /= 0) then
            ...
            oj = DDot_(nDAO,DAO,1,rFinal(1,1,1,i),1)
