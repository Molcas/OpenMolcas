...
*Bug in gcc 7: https://gcc.gnu.org/bugzilla/show_bug.cgi?id=94270
      ...
      Interface
         SubRoutine Rysg2(iAnga,nRys,nT,Alpha,Beta,Gmma,Delta,Zeta,ZInv,
     &                    nZeta,Eta,EInv,nEta,P,lP,Q,lQ,Coori,Coora,
     &                    CoorAC,Array,nArray,Tvalue,ModU2,Cff2D,PAO,
     &                    nPAO,Hess,nHess,IfGrd,IndGrd,IfHss,IndHss,
     &                    nOp,iuvwx,IfG,mVec,Index_Out,lGrad,lHess,Tr)
         use Definitions, only: wp, iwp
         integer(kind=iwp), intent(in) :: iAnga(4), nRys, nT, nZeta,
     &                                    nEta, lP, lQ, nArray, nPAO,
     &                                    nHess, IndGrd(3,4,0:7),
     &                                    nOp(4), iuvwx(4)
         real(kind=wp), intent(in) :: Alpha(nZeta), Beta(nZeta),
     &                                Gmma(nEta), Delta(nEta),
     &                                Zeta(nZeta), ZInv(nZeta),
     &                                Eta(nEta), EInv(nEta), P(lP,3),
     &                                Q(lQ,3), Coori(3,4), Coora(3,4),
     &                                CoorAC(3,2), PAO(nT,nPAO)
         real(kind=wp), intent(inout) :: Array(nArray), Hess(nHess)
         external :: Tvalue, ModU2, Cff2D
         logical(kind=iwp), intent(inout) :: IfGrd(3,4), IfHss(4,3,4,3),
     &                                       IfG(4), Tr(4)
