...
subroutine CmbnACB1(FA1,FB1,final,Fact,nAlpha,nBeta,C,nC,la,lb,iang,ifgrad,tmp,lsro,index,mvec,idcar)
!***********************************************************************
!
...
! @parameter index  Array storing index for derivatives in final (out)
...
real*8 FA1(nAlpha,nC,(la+1)*(la+2)/2,(2*iang+1),*), FB1(nC,nBeta,(2*iang+1),(lb+1)*(lb+2)/2,*), &
      ...
      do ia=1,nElem(la)
        ...
        do iC=1,(2*iAng+1)
          if (lsro) then
            ...
            call DGEMM_('N','N',nAlpha,nBeta,nC,Fact,Tmp,nAlpha,FB1(1,1,ic,ib,iFb),nC,One,final(1,ia,ib,mvec),nAlpha)
          else
            call DGEMM_('N','N',nAlpha,nBeta,nC,Fact,Fa1(1,1,ia,ic,iFa),nAlpha,Fb1(1,1,ic,ib,iFb),nC,One,final(1,ia,ib,mvec),nAlpha)
          end if
