...
integer lPaper, lLine, Left, iDis, iDisp, kkSym, kkkSym, iSym, nConf3, iRC, ipS1, ipS2, ipST, ipCIT, ipCID, nPre2, iDEnd, jDisp, &
...
if (iSym /= iSym_Old) then
  ...
  ! Initiate CSF <-> SD
  if (CI) call InCSFSD(ieor(iSym-1,State_Sym-1)+1,State_sym,.false.)
...
!else
call RHS(Sigma,Kappa,Temp1,Temp3,Sc2,dKappa,Sc3,Temp4,ipST,iDisp,iSym-1,CMO,jdisp,jspin,CI)
#ifdef _DEBUGPRINT_
...
if (iBreak == 1) then
  ...
  if (doDMRG) then ! yma
    ...
    if (sqrt(resk**2) < abs(Eps)) then
      if (abs(res_tmp-sqrt(resci**2)) < 1.0e-06) goto 300
    end if
...
if (lprint) write(LuWr,Fmt2//'A,i2,A,F12.7,F12.7,F12.7,F12.7,F12.7)') '     ',iter,'       ',delta/delta0,resk,resci,deltac,deltak
...
300 write(LuWr,Fmt2//'A,I4,A,I4,A)') 'Perturbation no: ',idisp,' converged in ',iter-1,' steps.'
irc = ipnout(-1)
!310 continue
!                                                                      *
!***********************************************************************
