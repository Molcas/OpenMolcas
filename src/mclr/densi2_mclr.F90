...
integer idum(1)
integer IPRDEN, NGAS, IATP, IBTP, JATP, JBTP, NOCTPA, NOCTPB, NAEL, NBEL, IOCTPA, IOCTPB, MXSTBL0, MAXA, MAXA1, MAXB, MAXB1, &
        MXSTBL, MXTSOB, IOBTP, IOBSM, LSCR1, INTSCR, IATP2, IBTP2, LSCR2, LSCR12, MAXIK, LSCR3, NOOS, IMNMX, MXCIJA, MXCIJAB, &
        MXCIJB, MXCJ, MXIJST, MXIJSTF, MXSXBL
...
call IAIBCM_MCLR(MNR1IC(ISSPC),MXR3IC(ISSPC),NOCTPA,NOCTPB,Str(IATP)%EL1,Str(IATP)%EL3,Str(IBTP)%EL1,Str(IBTP)%EL3,SIOIO,IPRDEN)

call IAIBCM_MCLR(MNR1IC(ICSPC),MXR3IC(ICSPC),NOCTPA,NOCTPB,Str(IATP)%EL1,Str(IATP)%EL3,Str(IBTP)%EL1,Str(IBTP)%EL3,CIOIO,IPRDEN)
...
call MXRESC(CIOIO,IATP,IBTP,NOCTPA,NOCTPB,nIrrep,Str(IATP)%NSTSO,Str(IBTP)%NSTSO,IATP+1,Str(IATP+1)%NSTSO,NOCTYP(IATP+1), &
            Str(IBTP+1)%NSTSO,NOCTYP(IBTP+1),NSMOB,3,3,NTSOB,IPRCIX,MAXK,Str(IATP2)%NSTSO,NOCTYP(IATP2),Str(IBTP2)%NSTSO, &
...
if ((IDC /= 1) .and. (ICISTR == 1)) then
  ! Left CI vector
  call SCDTC2_MCLR(L,ISMOST(1,ISSM),SBLTP,nIrrep,NOCTPA,NOCTPB,Str(IATP)%NSTSO,Str(IBTP)%NSTSO,SIOIO,IDC,2,IDUMMY,IPRDIA)
  ! Right CI vector
  call SCDTC2_MCLR(R,ISMOST(1,ICSM),CBLTP,nIrrep,NOCTPA,NOCTPB,Str(IATP)%NSTSO,Str(IBTP)%NSTSO,CIOIO,IDC,2,IDUMMY,IPRDIA)
end if
...
if (ICISTR == 1) then
  call GASDN2_MCLR(I12,RHO1,RHO2,R,L,CB,SB,C2,CIOIO,SIOIO,ISMOST(1,ICSM),ISMOST(1,ISSM),CBLTP,SBLTP,NACOB,Str(IATP)%NSTSO, &
                   ...
                   OOS(:,9),OOS(:,10),IX(:,1),XIXS(:,1),IX(:,2),XIXS(:,2),IX(:,3),XIXS(:,3),IX(:,4),XIXS(:,4),INSCR,MXPOBS,IPRDEN, &
                   RHO1S,LUL,LUR,PSSIGN,PSSIGN,RHO1P,XNATO,ieaw,n1,n2)
...
if ((IDC /= 1) .and. (ICISTR == 1)) then
  ...
  call SCDTC2_MCLR(L,ISMOST(1,ISSM),SBLTP,nIrrep,NOCTPA,NOCTPB,Str(IATP)%NSTSO,Str(IBTP)%NSTSO,SIOIO,IDC,1,IDUMMY,IPRDIA)
  call SCDTC2_MCLR(R,ISMOST(1,ICSM),CBLTP,nIrrep,NOCTPA,NOCTPB,Str(IATP)%NSTSO,Str(IBTP)%NSTSO,CIOIO,IDC,1,IDUMMY,IPRDIA)
end if
