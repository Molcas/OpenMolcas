...
!
  Subroutine CGS_pre(nDensC,nConf1,nRoots,Kappa,dKappa,Sigma,Fancy,Sc1, &
  ...
  call dcopy_(nConf1*nRoots,W(ipST)%A,1,W(CGSvec%ipR0)%A,1)
  ...
  call dcopy_(nConf1*nRoots,W(CGSvec%ipUvec)%A,1,W(ipCId)%A,1)
  Call DCopy_(nconf1*nRoots,W(ipCId)%A,1,W(CGSvec%ipPvec)%A,1)
  if (.not.InvSCF) then
    ISR%Pvec(:,:) = ISR%Uvec(:,:)
  ...
  if (PCGS == 5) then
    call dcopy_(nDensC,CGSvec%R0,1,Sigma,1)
    call dcopy_(nConf1*nRoots,W(CGSvec%ipR0)%A,1,W(ipS1)%A,1)
  end if
