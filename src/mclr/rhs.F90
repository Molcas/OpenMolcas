...
subroutine RHS(Temp1,Temp2,Temp3,Temp4,Temp5,Temp6,temp7,rKappa,ipst,iDisp,lOper,CMO,jdisp,jspin,CI)
!***********************************************************************
!                                                                      *
!    Purpose:                                                          *
    ...
    do jS=1,is
      if (ieor(iS-1,jS-1) == loper) then
        if (nOrb(is)*nOrb(js) /= 0) then
          if (is == js) then
            call Square(Temp7(ipMatLT(is,js)),Temp6,1,nBas(is),nBas(is))
            ip = ip+nBas(is)*(nBas(iS)+1)/2
          else
            call dcopy_(nBas(iS)*nBas(jS),Temp7(ipMatLt(is,js)),1,Temp6,1)
          end if
          call DGEMM_('T','N',nOrb(iS),nBas(jS),nBas(iS),One,CMO(ipCM(iS)),nBas(iS),Temp6,nBas(iS),Zero,Temp5,nOrb(iS))
          call DGEMM_('N','N',nOrb(is),nOrb(jS),nBAs(jS),One,Temp5,nOrb(iS),CMO(ipCM(jS)),nBas(jS),Zero,Temp1(ipMat(iS,jS)), &
                      nOrb(is))
          if (is /= js) then
            call DGEMM_('T','T',nOrb(jS),nBas(iS),nBAs(jS),One,CMO(ipCM(jS)),nBas(js),Temp6,nBas(iS),Zero,Temp5,nOrb(jS))
            call DGEMM_('N','N',nOrb(js),nOrb(iS),nBas(iS),One,Temp5,nOrb(jS),CMO(ipCM(iS)),nBas(iS),Zero,Temp1(ipMat(jS,iS)), &
                        nOrb(jS))
          end if
...
call IntX(FIX,temp7,temp6,temp5,temp4,rkappa,MOX,loper,idisp,rone)
...
if (iand(ntpert(idisp),2**3) == 8) then
  ...
  if (iMethod == 2) call CreQ(Temp6,MOT,G2t,loper+1)

  do iS=1,nSym
    jS = ieor(iS-1,loper)+1
    if (nOrb(js) < 1) cycle
    ! F~=2*Fi~
    if (nIsh(is) > 0) call DaXpY_(nIsh(is)*nOrb(js),Two,Temp4(ipMat(js,is)),1,Temp7(ipMat(js,is)),1)
    if (iMethod == 2) then
      ! F~=F~+2*FA~
      if (nIsh(is) > 0) call DaXpY_(nIsh(is)*nOrb(js),Two,Temp5(ipMat(js,is)),1,Temp7(ipMat(js,is)),1)
      if (nAsh(iS) < 1) cycle
      do iAsh=1,nAsh(iS)
        do jAsh=1,nAsh(is)
          ...
          call DaXpY_(nOrb(jS),Dij,Temp4(ipMat(js,is)+nOrb(js)*(nish(is)+iAsh-1)),1, &
                      Temp7(ipMat(js,is)+nOrb(js)*(nish(is)+jAsh-1)),1)
        end do
      end do
      ! F~=F~+Q~
      call DaXpY_(nAsh(is)*nOrb(js),One,Temp6(ipMatba(js,is)),1,Temp7(ipMat(js,is)+nOrb(js)*nIsh(is)),1)
    end if
  end do ! is
...
call Hess(Temp7,rkappa,Temp1,temp4,Temp5,temp6,Temp3,loper+1,jdisp,idisp)

! F=F~+Fx
if (iand(ntpert(idisp),2**3) == 8) call daxpy_(nDens,One,Temp7,1,rKappa,1)
