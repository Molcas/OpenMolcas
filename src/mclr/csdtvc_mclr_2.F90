...
subroutine CSDTVC_MCLR_2(CSFVEC,DETVEC,DTOCMT,ICTSDT,IREFSM,ICOPY)
...
do ITYP=1,NTYP
  ...
  if (ITYP > 1) then
    ...
    IOFFDT = IOFFDT+NCNFTP(ITYP-1,IREFSM)*NDPCNT(ITYP-1)
    IOFFCD = IOFFCD+NDPCNT(ITYP-1)*NCSFTP(ITYP-1)
  end if
