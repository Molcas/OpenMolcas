...
!***********************************************************************
     ...
     &           ' after functional transformation:', Tau_b2
      write(u6,'(6X,A)') 'NOTE:'
      write(u6,'(6X,2A)') 'Densities after transformation are ',
     &           'intermediate quantities'
      write(u6,'(6X,2A)') 'and should not be interpreted as ',
     &           'real spin densities'
      write(u6,*)
      write(u6,'(6X,A,32X,F18.6)') 'Exchange energy scaling factor',
     &          CoefX
      write(u6,'(6X,A,29X,F18.6)') 'Correlation energy scaling factor',
     &          CoefR
      write(u6,*)
      write(u6,'(6X,A,30X,F18.6)') 'Integrated alpha exchange energy',
     &          Funcaa
      write(u6,'(6X,A,30X,F18.6)') 'Integrated beta  exchange energy',
     &          Funcbb
      write(u6,'(6X,A,30X,F18.6)') 'Integrated  correlation   energy',
     &          Funccc
      write(u6,*)
     ...
     & 'Total MC-PDFT energy for state',jroot,'',[CASDFT_E],1)
      ...
      if ((CoefX*CoefR.ne.0.0).and.(CoefX.ne.1.0.or.CoefR.ne.1.0)) Then
         ...
         write(u6,*)
         write(u6,*)
         write(u6,'(6X,A,19X,F18.6)') 'Integrated alpha exchange '//
     &          'energy (unscaled)',
     &          Funcaa1
         write(u6,'(6X,A,19X,F18.6)') 'Integrated beta  exchange '//
     &          'energy (unscaled)',
     &          Funcbb1
         write(u6,'(6X,A,19X,F18.6)') 'Integrated  correlation   '//
     &          'energy (unscaled)',
     &          Funccc1
         write(u6,'(6X,A,38X,F18.8)') 'On-top energy (unscaled)',E_ot_1
         write(u6,'(6X,A,31X,F18.8)') 'Total MC-PDFT energy '//
     &         '(unscaled)',
     &         CASDFT_E_1
      end if
