...
!***********************************************************************
subroutine cpinp_(lunit, return_code)
    implicit none
...
#include "warnings.h"
    ...
    integer, intent(Out) :: lunit, return_code
    integer :: luspool, IsFreeUnit
    External :: IsFreeUnit

    return_code = _RC_ALL_IS_WELL_
...
! future, so look out!
    ...
    Call Disable_Spool()
    Rewind(LUSpool)
! Now open a new file, and copy only the input between the '&MCPDFT'
...
! Positioning LUSpool after the '&MCPDFT' marker.
