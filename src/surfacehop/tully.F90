...
if (.not.Found) then
  Amatrix(:,:)=(0.0_wp,0.0_wp)
  Amatrix(iRlxRoot,iRlxRoot)=(1.0_wp,0.0_wp)
...
substeps: DO ii=1,NSUBSTEPS
  ...
  do i=1,NSTATE
    do j=1,NSTATE
      AmatrixDT(i,j)=(0.0_wp,0.0_wp)
      do l=1,NSTATE
        AmatrixDT(i,j)=AmatrixDT(i,j)+Amatrix(i,l)*Dmatrix(l,j)-Amatrix(l,j)*Dmatrix(i,l)
        AmatrixDT(i,j)=AmatrixDT(i,j)+(0.0_wp,1.0_wp)*Amatrix(i,l)*V(l,j)-(0.0_wp,1.0_wp)*Amatrix(l,j)*V(i,l)
      end do
    end do
  end do
  ...
  pg: if (decoherence) then
    ...
    do i=1,NSTATE
      if (i /= temproot) then
        TAU(i)=abs(1.0_wp/(V(temproot,temproot)-V(i,i)))*(1.0_wp+DECO/EKIN)
      end if
    end do
