...
!      Real(lind=wp) :: magncoords(2*maxanisofiles,3)
...
!      If(DBG) Write(6,'(A,99I6)') 'RDIN:  i_pair(:,1)=',
!     &                                    (i_pair(i,1),i=1,nPair)
!      If(DBG) Write(6,'(A,99I6)') 'RDIN:  i_pair(:,2)=',
!     &                                    (i_pair(i,2),i=1,nPair)
...
!c       icase =1  --> magnetic field is applied along the main magnetic
!c                     axis of each Doublet (multiplet)
!        If     ( icase.eq.1 ) Then
!          continue
!        Else If ( icase.eq.2 ) Then
!c       icase =2  --> magnetic field is applied along the main magnetic
!c                     axis of the specIfied Doublet (number) number (NDim)
!          READ(Input,*,ERR=997) NmagMult
!        Else If ( icase.eq.3 ) Then
!c       icase =3  --> a new coordination system is defined by the user,
!c                     and the magnetic field is applied along gZ (third axis)
!          Do i=1,3
