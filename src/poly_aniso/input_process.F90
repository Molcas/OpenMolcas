...
do i=1,nneq
  if (itype(i) == 'A') then
    write(6,'(i3,1x,A15,1x,3i5,A)') i,namefile_aniso(i),nsfs(i),nss(i),nexch(i),' -- computed ab initio'
    write(6,'(A,i2,A )') 'ESO(',i,' ):'
    write(6,'(10F10.3)') (ESO(i,j),j=1,nss(i))
  else
    write(6,'(i3,A15,1x,3i5,A)') i,' - - - - - - - ',nsfs(i),nss(i),nexch(i),' -- generated as isotropic'
  end if
end do
...
if (DBG) write(6,'(A,i3)') 'input_process:  icount_B_sites=',icount_B_sites

if (.not. nosym) then
  write(6,'(A,A)') 'SYMM :         = ',' rotation matrices for equivalent magnetic centers:'
  do i=1,nneq
  ...
  write(6,'(A,A )') 'SYMM :         = ',' rotation matrices are "Identity Matrices" for all magnetic centers'
end if

if (ab_initio_all) then
  write(6,'(17x,A,I2,A)') 'all centers have been computed ab initio and ',nneq,' file(s) of "aniso_i.input" type exist.'
else
  write(6,'(17x,I2,A,I2,A)') nneq-icount_B_sites,' center(s) have been computed ab initio and',nneq-icount_B_sites, &
                             ' file(s) of "aniso_i.input" type exist.'
  write(6,'(17x,I2,A,15F7.3)') icount_B_sites,' center(s) have been defined empirical: no ZFS and g_tens = '
...
if (old_aniso_format) then
  write(6,'(A,A)') 'OLDA :         = ',' Input data files are given in OLD format'
end if
...
if (Lines) then
  ...
  do i=1,npair
    write(6,'(17x,2I3,2x, F9.5)') i_pair(i,1),i_pair(i,2),Jex(i)
  end do
end if
...
if (AnisoLines3) then
  ...
  do i=1,npair
    write(6,'(17x,2I3,2x,3F9.5)') i_pair(i,1),i_pair(i,2),(JAex(i,j),j=1,3)
  end do
end if
...
if (AnisoLines9) then
  ...
  do i=1,npair
    write(6,'(17x,2I3,2x,9F9.5)') i_pair(i,1),i_pair(i,2),(JAex9(i,1,j),j=1,3),(JAex9(i,2,j),j=1,3),(JAex9(i,3,j),j=1,3)
  end do
end if
...
if (DM_exchange) then
  ...
  do i=1,npair
    write(6,'(17x,2I3,2x,3F9.5)') i_pair(i,1),i_pair(i,2),(JDMex(i,j),j=1,3)
  end do
end if

if (Dipol) then
  write(6,'(A,A )') 'DIPO :         = ',' dipolar coupling for the above exchange pairs is included'
  write(6,'(A,A )') 'COOR :         = ',' symmetrized cartesian coordinates for each non-equivalent magnetic centers '
  do i=1,nneq
    write(6,'(17x,i3,3F11.6)') i,(MagnCoords(i,l),l=1,3)
  end do
else
  write(6,'(A,A )') 'DIPO :         = ',' dipolar coupling for the above exchange pairs is NOT included.'
end if

if (KE) then
  write(6,'(A,A )') 'LONG :         = ','kinetic exchange coupling is requested:'
...
if (JITO_exchange) then
  write(6,'(A,A )') 'ITOJ :         = ',' Full anisotopic ITO exchange for the above exchange pairs is included (in cm-1)'
  do i=1,npair
      ...
      do iproj1=-irank1,irank1
        do irank2=1,imaxrank(i,2),2
          do iproj2=-irank2,irank2
            write(6,'(23x,4I3,2x,2ES23.14)') irank1,iproj1,irank2,iproj2,JITOexR(i,irank1,iproj1,irank2,iproj2), &
...
if (compute_barrier) then
  if (compute_g_tensors) then
    ...
    write(6,'(A)') 'The program will continue, but UBAR is deactivated'
    compute_barrier = .false.
  end if

  if (Nblock > exch) then
    write(6,'(A)') 'It seems that you have asked for properties of inexistent states.'
...
if (compute_g_tensors .and. (nMult > 0)) then
  ...
  write(6,'(A)') 'Computation of pseudospin Hamiltonians was not requested'
end if

if (compute_g_tensors .and. (nMult == 0)) then
  write(6,'(A,i3)') 'nMult =',nMult
  write(6,'(A   )') 'Computation of pseudospin Hamiltonians is deactivated.'
end if
! ======================================================================

if (Do_structure_abc .and. ((nMult > 0) .or. compute_susceptibility)) then
  write(6,'(2A)') 'ABCC :         = ','the main axes for the computed tensors are written also'
...
if (compute_susceptibility) then
  !-----------------------------------------!
  write(6,'(A)') 'Magnetic susceptibility will be computedin the limit of zero applied magnetic field.'
  !-----------------------------------------!
  write(6,'(A,A )') 'CHIT :         = ',' molar magnetic susceptibility is computed using'
  !-----------------------------------------!
  if (IOPT == 1) then
    write(6,'(18x,A)') 'newly-derived formula for total XT and M'
  else if (IOPT == 2) then
    write(6,'(18x,A)') 'additive formula employed for total XT and M'
  else if (IOPT == 3) then
  ...
  !-----------------------------------------!
  if (TINPUT) then
    write(6,'(A)') 'TEXP :         = the experimental temperature interval is provided:'
    do iT=1,nT
  ...
  !-----------------------------------------!
  if (xField /= 0.0_wp) then
    write(6,'(A)') 'Magnetic susceptibility will be computed also by using:'
    write(6,'(A)') ' X= dM/dH formula and'
    write(6,'(A)') ' X=  M/H formula (commonly used by experimentalists)'
    write(6,'(A,F9.5,A)') 'FIELD :          ',Xfield,' Telsa'
  end if
  !-----------------------------------------!
else
  write(6,'(A)') 'Computation of magnetic susceptibility was not requested.'
end if

! ======================================================================
! Print out of the TORQUE
if (compute_torque) then
  write(6,'(A)') 'TORQ :         = ',' magnetic torque is computed'
  write(6,'(A,I4)') 'AngPoints = ',AngPoints
  write(6,'(A,i4)') 'nP        = ',nP
else
  write(6,'(A)') 'Computation of magnetic torque was not requested.'
end if
...
if (compute_magnetization) then
  write(6,'(2A )') 'MAGN :         = ',' molar magnetization is computed'
  !-----------------------------------------!
  if (nH > 0) then
    if (HINPUT) then
      write(6,'(A)') 'HEXP :         = the experimental field interval is provided:'
      write(fmtline,'(A,i3,A)') '(5x,i4,F11.5,',nTempMagn,'F14.7)'
      do iH=1,nH
        write(6,fmtline) iH,Hexp(iH),(Mexp(iH,iT),iT=1,nTempMagn)
      end do
    else
  ...
  if (nTempMagn <= 10) then
    write(6,'(6x,A,10F7.3)') 'TempMagn = ',(TempMagn(i),i=1,nTempMagn)
  else
    do i=1,nTempMagn,10
      j = min(nTempMagn,i+9)
  ...
  if (zeeman_energy) then
    write(6,'(2A,I3,A)') 'ZEEM :         = ',' Zeeman splitting for the following ',nDirZee
    write(6,'(17x,A  )') 'direction(s) of the magnetic field is given in the "zeeman_energy_xxx.output" file(s)'
    do i=1,nDirZee
  ...
  if (compute_Mdir_vector) then
    write(6,'(2A,I2,a)') 'MVEC :         = ','Magnetization vector(s) for the following ',nDir, &
                         ' direction(s) of the applied magnetic'
    write(6,'(17x,A)') 'field will be computed;'
    do i=1,nDir
      write(6,'(17x,A,I2,A,3F10.6)') 'Dir :',i,' : ',DirX(i),DirY(i),DirZ(i)
    end do
  end if
  ...
  if (nH > 0) then
    ...
    write(6,'(A,I4,A)') 'Magnetization will be computed in',nDirTot,' directions.'
    write(6,'(A,I6,A)') 'There will be ',nDirTot*nH,' calculations in total.'

    !-----------------------------------------------------------|
    if (m_accurate) then
      write(6,'(2A   )') 'MACC :         = ','Contribution to magnetization from local excited states'
      write(6,'(17x,A)') 'is computed exactly.'
    else
      write(6,'(17x,A)') 'is accounted by formula:  M_excited = X_excited * H'
    end if
    !-----------------------------------------------------------|

    !-----------------------------------------------------------|
    if ((exch < 256) .and. (zJ /= 0.0_wp)) m_paranoid = .true.

    if (m_paranoid .and. (zJ /= 0.0_wp)) then
      write(6,'(2A)') 'MPAR :         = ','Average spin < S > of the environment is computed'
      write(6,'(2A)') '                 ','for each temperature point (more accurate)'
    else
      write(6,'(2A)') 'MPAR :         = ','Average spin < S > of the environment is computed'
      write(6,'(2A)') '                 ','for the highest temperature point only and used for all computed temperature points.'
    end if
    !-----------------------------------------------------------|

    !-----------------------------------------------------------|
    if (smagn) then
      write(6,'(2A)') 'SMAG :         = ','Spin-only magnetisation is computed'
    else
      write(6,'(A)') 'Computation of spin magnetization was not requested.'
    end if
    !-----------------------------------------------------------|
  ...
  write(6,'(A)') 'Computation of molar magnetization was not requested.'
end if
! End Print out of the MAGNETIZATION
! ======================================================================
if (compute_susceptibility .or. compute_torque .or. compute_magnetization) then
  if (zJ /= 0.0_wp) then
    write(6,'(A)') 'Mean field intermolecular interaction parameter'
    write(6,'(A,F12.7)') 'ZJPR :         = ',zJ
  end if
end if
! ======================================================================
if ((fitCHI .or. fitM) .and. (tinput .or. hinput)) then
  write(6,'(A)') 'Automatic fitting of exchange parameters is yet in the development'
end if
! ======================================================================
