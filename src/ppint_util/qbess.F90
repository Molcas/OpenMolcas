...
dimension apwr(*), aterm1(*), aterm2(*), binom(*), bpref(*), bpwr(*), bterm1(*), dfac(*), qsum(ltot1,lambu,*), ssi(*)
...
lmplo = lmlo-num1
fctra = (alpha+alpha)**(nu-2)*fcta**(lmlo-1)*prd/sqrt(a4*alpha)*((dfac(2*(2*lmplo+num1)-1)/dfac(2*(lmplo+num1)+1))* &
...
do lamap=lmplo,lmaphi
  ...
  do lambp=lmplo,lmbphi
    ...
    do it=1,lamap
      do iu=1,lambp
        sum = sum+aterm2(iu)*(fctrt*bterm1(it))*ssi((it+(nu-2))+iu)
      end do
      fctrt = fctrt*fctrtn/fctrtd
      fctrtn = fctrtn+a2
      fctrtd = fctrtd+a2
    end do
    qsum(n,lambp+num1,lamap+num1) = qsum(n,lambp+num1,lamap+num1)+fctrb*bpref(lambp)*sum
