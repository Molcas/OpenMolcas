...
subroutine OEISG(REL,SREL,TREL,UREL,ZETA,ZN,MX100,NSYM,NBAS1,unrel,tnrel,hcorr,iprint,VEXTT,PVPT,EVN1,RE1R,AUXI,W1W1)
...
implicit none
integer(kind=iwp), intent(in) :: MX100, NSYM, NBAS1, iprint
...
subroutine OEISG_INTERNAL(REL)
  ...
  real(kind=wp), target, intent(out) :: REL(*)
  ...
  call AT34R(NBAS1,NBSZ,ZN,SREL,UREL,TREL,REL(1),iREL,REL(2*NBSZ+1),REL(3*NBSZ+1),REL(4*NBSZ+1),REL(NBSZ5+1),REL(NBSZ5+NBSQ+1), &
             ...
             REL(NBSZ5+NBSQ5+2*MX100+1),REL(NBSZ5+NBSQ5+3*MX100+1),REL(NBSZ5+NBSQ5+4*MX100+1),iprint,VEXTT,PVPT,EVN1,RE1R,AUXI,W1W1)
  nullify(iREL)
