...
integer(kind=iwp) :: iDisk, iMem, tmp_CI_vec_RecNo
...
if (nConf < 0) then
  write(u6,*) 'Save_tmp_CI_vec: nConf less than 0'
  write(u6,*) 'nConf = ',nConf
  call Abend()
end if
if (iRoot < 0) then
  write(u6,*) 'Save_tmp_CI_vec: iRoot less than 0'
  write(u6,*) 'iRoot = ',iRoot
  call Abend()
end if
if (iRoot > n_Roots) then
  write(u6,*) 'Save_tmp_CI_vec: iRoot greater than nRoots'
...
end subroutine Save_tmp_CI_vec
