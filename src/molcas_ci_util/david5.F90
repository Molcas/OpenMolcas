  ...
  do it=1,my_norb
    do iu=1,it
      ...
      do iv=1,it
        ...
        do ix=1,ixmax
          ituvx = ituvx+1
          !write(6,'(1x,5I4,F21.14)') it,iu,iv,ix,ituvx,gtuvxtri(ituvx)
...
do it_ci=1,mxItr
  ...
  do mRoot=lRoots*idelta+1,lRoots+nnew
    ...
    if (iprlev >= DEBUG) then
      ...
      write(6,'(1X,A,I2,A,I2)') 'CI vector, iter =',it_ci,' mRoot =',mRoot
      write(6,'(1X,A)') '(max. 200 elements)'
      write(6,'(1X,A)') '-----------------------------'
    ...
    if (iprlev >= DEBUG) then
      lPrint = min(nConf,200)
      write(6,*) ' '
      write(6,'(1X,A,I2,A,I2)') 'sigma vector, iter =',it_ci,' mRoot =',mRoot
      write(6,'(1X,A)') '(max. 200 elements)'
      write(6,'(1X,A)') '--------------------------------'
  ...
  if (nBasVec < lRoots) then
    ...
    if (ICIRST == 1) write(6,*) 'CIREstart was used. Check the number of roots in the previous calculation'
    call Abend()
  end if
  if (iprlev >= DEBUG) then
  ...
  do mRoot=1,lRoots
    ...
    if (iprlev >= DEBUG) then
      lPrint = min(nConf,200)
      write(6,'(1X,A,I2,A,I2)') 'new best CI vector, iter =',it_ci,' mRoot =',mRoot
      write(6,'(1X,A)') '(max. 200 elements)'
      write(6,'(1X,A)') '--------------------------------------'
      call dVcPrt(' ',' ',Work(iVec1),lPrint)
      write(6,'(1X,A,I2,A,I2)') 'new best sigma vector, iter =',it_ci,' mRoot =',mRoot
      write(6,'(1X,A)') '(max. 200 elements)'
      write(6,'(1X,A)') '-----------------------------------------'
      call dVcPrt(' ',' ',Work(iVec2),lPrint)
      write(6,'(1X,A,I2,A,I2)') 'new residual vector, iter =',it_ci,' mRoot =',mRoot
      write(6,'(1X,A)') '(max. 200 elements)'
      write(6,'(1X,A)') '-----------------------------------------'
  ...
  if (iprlev > DEBUG) then
    ...
    do jRoot=1,lRoots
      ...
      write(6,'(1X,A,I2,A,F18.10,2(A,F14.10))') ' root ',jRoot,' energy =',CI_conv(1,jroot,it_ci),' dE =',dE,' residual =', &
  ...
  if (nSel > 1) then
    ...
    call DGEMM_('T','N',nSel,nleft,nSel,1.0d0,ExplV,nSel,Work(iScr3+ioff),nSel,0.0d0,Work(iScr5),nSel)
    do mRoot=nconverged+1,lRoots
      E0 = Work(iEs+mRoot-1)
      iOff = (mRoot-nconverged-1)*nSel
      do i=1,nSel
        Z = E0-ExplE(i)
        if (abs(Z) < 0.001d0) Z = 0.001d0
        Work(iScr5+iOff+i-1) = Work(iScr5+iOff+i-1)/Z
      end do
    end do
    call DGEMM_('N','N',nSel,nleft,nSel,1.0d0,ExplV,nSel,Work(iScr5),nSel,0.0d0,Work(iScr3),nSel)
    ioff = nconverged*nSel
    call DGEMM_('T','N',nSel,nleft,nSel,1.0d0,ExplV,nSel,Work(iScr4+ioff),nSel,0.0d0,Work(iScr5),nSel)
    do mRoot=nconverged+1,lRoots
      E0 = Work(iEs+mRoot-1)
      iOff = (mRoot-nconverged-1)*nSel
      do i=1,nSel
    ...
    call DGEMM_('N','N',nSel,nleft,nSel,1.0d0,ExplV,nSel,Work(iScr5),nSel,0.0d0,Work(iScr4),nSel)
  end if
  !---------------------------------------------------------------------
  do mRoot=nconverged+1,lRoots
    E0 = -Work(iEs+mRoot-1)
  ...
  if (nSel > 1) then
    ...
    call DGEMM_('T','N',nSel,nleft,nSel,1.0d0,ExplV,nSel,Work(iScr3),nSel,0.0d0,Work(iScr5),nSel)
    do mRoot=nconverged+1,lRoots
      E0 = Work(iEs+mRoot-1)
      iOff = (mRoot-nconverged-1)*nSel
      do i=1,nSel
...
if (IPRLEV >= USUAL) then
  write(6,*) '       No convergence in the CI section: MAXJT will be increased to ',mxItr
end if
Rc_CI = 16
nItr = nItr-1
