    ...
    if (IFDW .or. (.not. IFXMS)) then
      ...
      do Istate=1,Nstate
        if (Istate /= Jstate) then
          ...
          call FOPAB(FIFA,size(FIFA),Istate,Jstate,H0(Istate,Jstate))
          write(u6,'(A3,I4,A3,F16.8)') ' < ',MSTATE(Istate),' | ',H0(Istate,Jstate)
...
if ((Ngrp > 1) .and. IFXMS .and. (.not. IFDW)) then
  ...
  do J=1,Ngrp
    ...
    call dgemm_('N','N',Nconf,1,Ngrp,One,CIREF,Nconf,U0(:,J),Ngrp,Zero,CIXMS,Nconf)
...
subroutine GPRINI_FINISH()
  ...
  call ORBCTL(CMO,NCMO,TORB,size(TORB),FIFA,size(FIFA),FIMO,size(FIMO))
