...
C  CONSTRUCT A RESTRICTED GRAPH.
        ...
        LM3RAS=NACTEL-NELE3
        CALL RESTR(NVERT0,DRTP,IWORK(LDOWN0),IWORK(LV),
     &                 LV1RAS,LV3RAS,LM1RAS,LM3RAS,NVERT)
        NDRT=5*NVERT
        NDOWN=4*NVERT
        CALL mma_allocate(DRT,NDRT,Label='DRT')
        CALL GETMEM('DOWN','ALLO','INTEG',LDOWN,NDOWN)
        CALL mkDRT(NVERT0,NVERT,DRT0,IWORK(LDOWN0),IWORK(LV),
     &             DRT,IWORK(LDOWN))
...
C CALCULATE DIRECT ARC WEIGHT AND LTV TABLES.
      ...
      CALL MKDAW_CP2(NLEV,NVERT,DRT,IWORK(LDOWN),
     &               IWORK(LDAW),IWORK(LLTV))
...
C CALCULATE SEGMENT VALUES. ALSO, MVL AND MVR TABLES.
      ...
      CALL MKSEG_CP2(DRT,IWORK(LDOWN),IWORK(LLTV),IWORK(LIVR),
