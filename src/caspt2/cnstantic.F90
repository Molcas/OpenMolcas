...
use general_data, only: NASH, nLev
use caspt2_module, only: ENERGY, NASHT, NBAS, NBAST, NBSQT, NCONF, NDEL, NFRO, NISH, NORB, NSTATE, NSYM
...
do iStat=1,nState
  ...
  do jStat=1,nState
    ...
    !call sqprt(tg1,5)
    G1(:,:) = G1(:,:)+Half*(UEFF(iStat,iRoot2)*UEFF(jStat,iRoot1)-UEFF(jStat,iRoot2)*UEFF(iStat,iRoot1))*TG1(:,:)
  end do
end do
!call sqprt(g1,5)
...
WRK1(1:nBasT**2) = (ENERGY(iRoot2)-ENERGY(iRoot1))*DPT2Canti(1:nBasT**2)
...
do i=1,nBasT
  do j=1,i-1
    Scal = Half*(DPT2Canti(i+nBasT*(j-1))-DPT2Canti(j+nBasT*(i-1)))
    DPT2Canti(i+nBasT*(j-1)) = Scal
    DPT2Canti(j+nBasT*(i-1)) = -Scal
  end do
end do
