...
subroutine MLTR1_GH(IMLTOP,LST1,X,NAS1,NIS1,JXOFF,F,NFT,NFI,Y,NAS2,jYLo,jYHi)
...
implicit none
integer(kind=iwp), intent(in) :: IMLTOP, LST1(4,NLST1), NAS1, NIS1, JXOFF, NFT, NFI, NAS2, jYLo, jYHi
real(kind=wp), intent(inout) :: X(NAS1,NIS1), F(NFT,NFI), Y(NAS2,jYHi-jYLo+1)
integer(kind=iwp) :: I, ILST, JX, JY, L1, L2, L3, L4, NI
real(kind=wp) :: A, V
    ...
    do ILST=1,NLST1
      ...
      if ((L3 >= jYLo) .and. (L3 <= jYHi)) then
        ...
        do I=1,NAS2
          ...
          Y(I,JY) = Y(I,JY)+V*DDOT_(NAS1,F(1,L2),1,X(1,JX+NI*(I-1)),1)
        end do
      end if
    end do
  case default
