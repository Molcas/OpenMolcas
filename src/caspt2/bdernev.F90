...
real(kind=wp), allocatable :: BDERA_save(:), BDERC_save(:), Gact(:,:,:,:), Gder(:,:,:,:), Hact(:,:), Hbar(:,:), Hbder(:,:), &
                              Hder(:,:), Htder(:,:), Htilde(:,:)

public :: BDerNEV_E4, BDerNEV_final1, BDerNEV_final2, BDerNEV_initial, BDN_G3, BDNA, BDNB, BDNC, BDND, BDNE, BDNF, BDNG, Gact, Gder
...
# endif
  ...
  do kk=1,NASHT
    Gder(kk,:,kk,:) = Gder(kk,:,kk,:)-Half*Hbder(:,:)-Htder(:,:)
  end do
  Hder(:,:) = Hder(:,:)+Hbder(:,:)+Htder(:,:)
  !! Compensate the division by the number of electrons in OLagNS2 or OLagNS_RI
  Hder(:,:) = Hder(:,:)*real(max(1,nactel),kind=wp)
...
subroutine BDerNEV_E4(NCONF,NLEV,CLag)
  ...
  use caspt2_module, only: NINDEP, NSYM, NTUV, NTUVES

  integer(kind=iwp), intent(in) :: NCONF, NLEV
  real(kind=wp), intent(inout) :: CLag(NCONF)
  integer(kind=iwp) :: ICASE1, ICASE4, ISYM, NASA, NASC, NBA, NBC, NINA, NINC
...
subroutine BDNA(iSym,NAS,NG3,BDER,SDER,G1,G2,G3,DG1,DG2,DG3)
  ...
  integer(kind=iwp) :: iG3, iLUID, it, itu, iu, IV, IVX, IX, IY, IYZ, IZ !, IST, ISU, ISV, ISX, ISY, ISZ
  real(kind=wp) :: DG2VAL, DG3VAL, G3VAL
  integer(kind=byte), allocatable :: idxG3(:,:)

  !! E4 contributions are evaluate later
  BDERA_save(1+NTUVES(iSym):NAS*NAS+NTUVES(iSym)) = pack(BDER(1:NAS,1:NAS),.true.)
  ...
  subroutine Add_MKBNEVA(ITABS_,IUABS_,IVABS_,IXABS_,IYABS_,IZABS_,iG3_)
    ...
    integer(kind=iwp) :: iaabs, ibabs, isab, ituv_, iwabs, ixyz_
...
subroutine BDNB(iSym,iCase,NAS,NG3,BDER0,SDER0,G1,G2,G3,DG1,DG2,DG3)
  ...
  use caspt2_module, only: IASYM, NTGEUES, NTGTUES, NTU, NTUES
  use NEVPT2_mod, only: dertG2, dertG3, tG2, tG3
  ...
  real(kind=wp), intent(inout) :: DG1(nAshT,nAshT), DG2(nAshT,nAshT,nAshT,nAshT), DG3(NG3)
  integer(kind=iwp) :: ID, iG3, iLUID, IST, ISU, ISV, ISX, ISY, ISZ, it, itabs, iTgeUabs, iTgtUabs, itu, itu0, ituabs, ITUVS, iu, &
                       iuabs, IV, ivabs, IVX, iwabs, IX, ixabs, iXgeYabs, iXgtYabs, ixy, ixy0, ixyabs, IXYZS, IY, iyabs, iyx, IYZ, &
                       IZ, izabs, nTask
...
subroutine BDNC(iSym,NAS,NG3,BDER,SDER,G1,G2,G3,DG1,DG2,DG3)
  ...
  !! E4 contributions are evaluate later
  BDERC_save(1+NTUVES(iSym):NAS*NAS+NTUVES(iSym)) = pack(BDER(1:NAS,1:NAS),.true.)
...
subroutine BDND(iSym,NAS,NG3,BDER,SDER,G1,G2,G3,DG1,DG2,DG3)
  ...
  real(kind=wp), intent(in) :: BDER(NAS,NAS), SDER(NAS,NAS), G1(nAshT,nAshT), G2(nAshT,nAshT,nAshT,nAshT), G3(NG3)
  real(kind=wp), intent(inout) :: DG1(nAshT,nAshT), DG2(nAshT,nAshT,nAshT,nAshT), DG3(NG3)
  integer(kind=iwp) :: ID, iG3, iLUID, IST, ISU, ISV, ISX, ISY, ISZ, it, itabs, itu, ituabs, ITUVS, iu, iuabs, IV, ivabs, IVX, &
                       iwabs, IX, ixabs, ixy, ixyabs, IXYZS, IY, iyabs, IYZ, IZ, izabs, nTask
  ...
  BDER1(:,:) = Two*BDER(1:NAS/2,1:NAS/2)-BDER(NAS/2+1:NAS,1:NAS/2)-BDER(1:NAS/2,NAS/2+1:NAS)
  BDER2(:,:) = BDER(NAS/2+1:NAS,NAS/2+1:NAS)
  SDER1(:,:) = Two*SDER(1:NAS/2,1:NAS/2)-SDER(NAS/2+1:NAS,1:NAS/2)-SDER(1:NAS/2,NAS/2+1:NAS)
  SDER2(:,:) = SDER(NAS/2+1:NAS,NAS/2+1:NAS)
  ...
  do IG3=1,NG3
    ...
    ! D(tuvxyz)
    if ((Mul(iSU,iST) == iSym) .or. (Mul(iSU,ISV) == iSym) .or. (Mul(iSX,iSY) == iSym)) call Add_MKBNEVD(iT,iU,iV,iX,iY,iZ,iG3)

    if ((iTU /= iVX) .or. (iVX /= iYZ)) then
      if ((iTU /= iVX) .and. (iTU /= iYZ) .and. (iVX /= iYZ)) then
        ! D(vxtuyz)
        if ((Mul(iSX,iSV) == iSym) .or. (Mul(iSX,iST) == iSym) .or. (Mul(iSU,iSY) == iSym)) call Add_MKBNEVD(iV,iX,iT,iU,iY,iZ,iG3)
        ! D(yzvxtu)
        if ((Mul(iSZ,iSY) == iSym) .or. (Mul(iSZ,iSV) == iSym) .or. (Mul(iSX,iST) == iSym)) call Add_MKBNEVD(iY,iZ,iV,iX,iT,iU,iG3)
        ! D(tuyzvx)
        if ((Mul(iSU,iST) == iSym) .or. (Mul(iSU,iSY) == iSym) .or. (Mul(iSZ,iSV) == iSym)) call Add_MKBNEVD(iT,iU,iY,iZ,iV,iX,iG3)
      end if
      ! D(yztuvx)
      if ((Mul(iSZ,iSY) == iSym) .or. (Mul(iSZ,iST) == iSym) .or. (Mul(iSU,iSV) == iSym)) call Add_MKBNEVD(iY,iZ,iT,iU,iV,iX,iG3)
      ! D(vxyztu)
      if ((Mul(iSX,iSV) == iSym) .or. (Mul(iSX,iSY) == iSym) .or. (Mul(iSZ,iST) == iSym)) call Add_MKBNEVD(iV,iX,iY,iZ,iT,iU,iG3)
    end if
    ...
    ! D(utxvzy)
    if ((Mul(iST,iSU) == iSym) .or. (Mul(iST,iSX) == iSym) .or. (Mul(iSV,iSZ) == iSym)) call Add_MKBNEVD(iU,iT,iX,iV,iZ,iY,iG3)

    if ((iTU == iVX) .and. (iVX == iYZ)) cycle
    if ((iTU /= iVX) .and. (iTU /= iYZ) .and. (iVX /= iYZ)) then
      ! D(xvutzy)
      if ((Mul(iSV,iSX) == iSym) .or. (Mul(iSV,iSU) == iSym) .or. (Mul(iST,iSZ) == iSym)) call Add_MKBNEVD(iX,iV,iU,iT,iZ,iY,iG3)
      ! D(zyxvut)
      if ((Mul(iSY,iSZ) == iSym) .or. (Mul(iSY,iSX) == iSym) .or. (Mul(iSV,iSU) == iSym)) call Add_MKBNEVD(iZ,iY,iX,iV,iU,iT,iG3)
      ! D(utzyxv)
      if ((Mul(iST,iSU) == iSym) .or. (Mul(iST,iSZ) == iSym) .or. (Mul(iSY,iSX) == iSym)) call Add_MKBNEVD(iU,iT,iZ,iY,iX,iV,iG3)
    end if
    ! D(zyutxv)
    if ((Mul(iSY,iSZ) == iSym) .or. (Mul(iSY,iSU) == iSym) .or. (Mul(iST,iSX) == iSym)) call Add_MKBNEVD(iZ,iY,iU,iT,iX,iV,iG3)
    ! D(xvzyut)
    if ((Mul(iSV,iSX) == iSym) .or. (Mul(iSV,iSZ) == iSym) .or. (Mul(iSY,iSU) == iSym)) call Add_MKBNEVD(iX,iV,iZ,iY,iU,iT,iG3)
  end do
  ...
  subroutine Add_MKBNEVD(ITABS_,IUABS_,IVABS_,IXABS_,IYABS_,IZABS_,iG3_)
    ...
    use caspt2_module, only: IASYM, NTUES
...
subroutine BDNE(iSym,NAS,BDER,SDER,G1,G2,DG1,DG2)
  ...
  integer(kind=iwp) :: ID, it, itabs, iu, iuabs, ivabs, ixabs, iyabs, nTask
...
subroutine BDN_G3(DG1,DG2,DG3)
  ...
  integer(kind=iwp) :: iG3, iLUID, IST, ISU, ISV, ISX, ISY, ISZ, IT, ITUVS, IU, IV, IX, IXYZS, IY, IZ
  real(kind=wp) :: G3VAL
  integer(kind=byte), allocatable :: idxG3(:,:)
