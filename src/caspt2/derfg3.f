...
!-is basically the number of buffers we fill with EPQ_PSI vectors.
      iTask=1
      ibuf1=0
      DO ip1=1,nlev2
        itlev=idx2ij(1,ip1)
...
* The indices corresponding to pair index p3:
        ...
        iz=L2ACT(izlev)
        buf2(1:nsgm2) = Zero
        CALL EPQ_PSI(SGS,CIS,EXS,IYLEV,IZLEV,One,STSYM,CI,BUF2)
        DYZ(1:nsgm1) = Zero
        if(issg2 == issg1) then
          buf3(1:nsgm2) = Zero
          do ib=1,ibuf1
...
!
     ...
     &                  One,DEPSA(1,IVLEV),1)
            ...
            CALL EPQ_PSI(SGS,CIS,EXS,IXLEV,IVLEV,One,STSYM,BUF3,DYZ)
        ...
        CALL EPQ_PSI(SGS,CIS,EXS,IZLEV,IYLEV,One,STSYM,DYZ,CLAG)
