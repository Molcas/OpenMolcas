...
subroutine MKBNEVAC_E4(nAshT,NLEV,Gact)
  ...
  integer(kind=iwp), intent(in) :: nAshT, NLEV
  real(kind=wp), intent(in) :: Gact(nAshT,nAshT,nAshT,nAshT)
...
#endif
!     CALL PSBMAT_READ('B',iCase1,iSYM,lg_BA,NASA)
!     CALL PSBMAT_READ('B',iCase4,iSYM,lg_BC,NASC)
#ifdef _MOLCAS_MPP_
    END IF
#endif

    idisk = IDBMAT_NEVPT2(iSym,iCase1,1)
    CALL DDAFILE(LUSBT,2,GA_Arrays(lg_BA)%A(1),NBA,IDISK)
...
!     CALL PSBMAT_READ('B',iCase1,iSYM,lg_BASQ,NASA)
      ...
      IF (ILOA > 0 .AND. JLOA > 0) THEN
        ...
        do ii = iloa, ihia
          do jj = jloa, jhia
            if (ii >= jj) then
              DBL_MB(MA+ii-iloa+LDA*(jj-jloa)) = DBL_MB(MA+ii-iloa+LDA*(jj-jloa)) + GA_Arrays(lg_BA)%A(ii*(ii-1)/2+jj)
            else
              DBL_MB(MA+ii-iloa+LDA*(jj-jloa)) = DBL_MB(MA+ii-iloa+LDA*(jj-jloa)) + GA_Arrays(lg_BA)%A(jj*(jj-1)/2+ii)
            end if
          end do
        end do
...
!     CALL PSBMAT_READ('B',iCase4,iSYM,lg_BCSQ,NASC)
      ...
      IF (ILOC > 0 .AND. JLOC > 0) THEN
        ...
        do ii = iloc, ihic
          do jj = jloc, jhic
            if (ii >= jj) then
              DBL_MB(MC+ii-iloc+LDC*(jj-jloc)) = DBL_MB(MC+ii-iloc+LDC*(jj-jloc)) + GA_Arrays(lg_BC)%A(ii*(ii-1)/2+jj)
            else
              DBL_MB(MC+ii-iloc+LDC*(jj-jloc)) = DBL_MB(MC+ii-iloc+LDC*(jj-jloc)) + GA_Arrays(lg_BC)%A(jj*(jj-1)/2+ii)
            end if
          end do
        end do
