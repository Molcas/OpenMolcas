...
# ifdef _MOLCAS_MPP_
  if (IS_REAL_PAR()) then
    ...
    if ((ILOA > 0) .and. (JLOA > 0)) then
      ...
      do ii=iloa,ihia
        do jj=jloa,jhia
          DBL_MB(MA+ii-iloa+LDA*(jj-jloa)) = DBL_MB(MA+ii-iloa+LDA*(jj-jloa))+GA_Arrays(lg_BA)%A(iTri(ii,jj))
        end do
      end do
    ...
    if ((ILOC > 0) .and. (JLOC > 0)) then
      ...
      do ii=iloc,ihic
        do jj=jloc,jhic
          DBL_MB(MC+ii-iloc+LDC*(jj-jloc)) = DBL_MB(MC+ii-iloc+LDC*(jj-jloc))+GA_Arrays(lg_BC)%A(iTri(ii,jj))
        end do
      end do
