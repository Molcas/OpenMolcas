...
module SC_NEVPT2
  ...
  !! Compute partially contracted or fully internally contracted NEVPT2 (PC- or FIC-NEVPT2) energy
  logical(kind=iwp) :: Do_FIC
  !! Compute strongly contracted NEVPT2 (SC-NEVPT2) energy
  logical(kind=iwp) :: Do_SC
  !! Do property (energy and gradients) calculations using SC-NEVPT2 wavefunction
  logical(kind=iwp) :: SC_prop = .false.
  !! Compute the T-amplitude for SC-NEVPT2 wavefunction
  logical(kind=iwp) :: SC_amplitude = .false.
  !! Threshold for avoiding vanishing denominators: the main purpose is to avoid numerical blow up, not for efficiency
  real(kind=wp)     :: SC_thres = 1.0e-09_wp

  !----- SC-NEVPT2
  ! Correlation energy
  real(kind=wp) :: ECORR_SC(0:8,0:MXCASE)
  ! Wavefunction (correction) overlap
  real(kind=wp) :: OVLAPS_SC(0:8,0:MXCASE)
  ...
  !----- QD-SC-NEVPT2
  ! temporary storage of DVALUE
  real(kind=wp) :: DVALUE_SC
  ! energy
  real(kind=wp), allocatable :: ENERGY_SC(:)
  ! Effective Hamiltonian
  real(kind=wp), allocatable :: HEFF_SC(:,:)
  ! Eigenvector
  real(kind=wp), allocatable :: UEFF_SC(:,:)
...
subroutine SC_NEVPT2_Print()
  ...
  real(kind=wp) :: E2NONV_SC, E2CORR_SC, E2TOT_SC, DENORM_SC, REFWGT_SC, RNORM_SC
  real(kind=wp) :: EVJTU_SC, EVJTI_SC, EATVX_SC, EAIVX_SC, EVJAI_SC, EBVAT_SC, EBJAT_SC, EBJAI_SC
...
! use ChoCASPT2
  ...
  real(kind=wp), allocatable :: BMAT(:), SMAT(:), LBD(:), LID(:), E_value(:)

  integer(kind=iwp) :: idisk, NAS, NIS, isp, ias, jas, iSym, iCase, lg_V
  ...
  ECORR_SC(0:8,0:11) = Zero
  OVLAPS_SC(0:8,0:11) = Zero
...
!
  ...
  do iSym = 1, nSym
    do iCase = 1, MXCASE
!     cycle
!     If (icase.ne.12.and.icase.ne.13) cycle ! H
!     If (icase.ne.10.and.icase.ne.11) cycle ! G
!     If (icase.ne. 6.and.icase.ne. 7) cycle ! E
!     If (icase.ne. 8.and.icase.ne. 9) cycle ! F
!     If (icase.ne. 2.and.icase.ne. 3) cycle ! B
!     If (icase.ne. 5)                 cycle ! D
!     If (icase.ne. 4)                 cycle ! C
!     If (icase.ne. 1)                 cycle ! A
          ...
          if (Is_Real_Par()) then
            ...
            if (ILO1 > 0 .and. JLO1 > 0) then
              ...
              if (IHI1-ILO1+1 /= NAS) then
                write (u6,'(1x,"Something is wrong in SC_NEVPT2_Energy")')
                call abend()
              end if
              do isp = JLO1, JHI1
                valn = Zero
      ...
      call mma_allocate(BMAT,NAS*(NAS+1)/2,Label='BMAT')
      idisk = IDBMAT_NEVPT2(iSym,iCase,1)
      call dDaFile(LUSBT,2,BMAT,NAS*(NAS+1)/2,idisk)

      ! Read the S matrix
      call mma_allocate(SMAT,NAS*(NAS+1)/2,Label='SMAT')
#ifdef _MOLCAS_MPP_
      if (Is_Real_Par() .and. (iCase == 1 .or. iCase == 4)) then
        ...
        call dDaFile(LUSBT,2,SMAT,NAS*(NAS+1)/2,idisk)
#ifdef _MOLCAS_MPP_
      end if
#endif
      ...
      if (Is_Real_Par()) then
          ...
          do isp = JLO1, JHI1
            ...
            do ias = 1, NAS
              do jas = 1, ias-1 ! ias+1, NAS
                valh = valh + Two*DBL_MB(MV+ias-1+NAS*(isp-JLO1))*DBL_MB(MV+jas-1+NAS*(isp-JLO1))*BMAT(ias*(ias-1)/2+jas)
                valn = valn + Two*DBL_MB(MV+ias-1+NAS*(isp-JLO1))*DBL_MB(MV+jas-1+NAS*(isp-JLO1))*SMAT(ias*(ias-1)/2+jas)
              end do
              valh = valh + DBL_MB(MV+ias-1+NAS*(isp-JLO1))*DBL_MB(MV+ias-1+NAS*(isp-JLO1))*BMAT(ias*(ias-1)/2+ias)
              valn = valn + DBL_MB(MV+ias-1+NAS*(isp-JLO1))*DBL_MB(MV+ias-1+NAS*(isp-JLO1))*SMAT(ias*(ias-1)/2+ias)
            end do
        ...
        do isp = JLO1, JHI1
          ...
          do ias = ILO1, IHI1
            do jas = ias+1, IHI1
              valh = valh + Two*GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V)%A(jas+NAS*(isp-1))*BMAT(jas*(jas-1)/2+ias)
              valn = valn + Two*GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V)%A(jas+NAS*(isp-1))*SMAT(jas*(jas-1)/2+ias)
            end do
            valh = valh + GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*BMAT(ias*(ias-1)/2+ias)
            valn = valn + GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*SMAT(ias*(ias-1)/2+ias)
          end do
  ...
  if (is_real_par()) then
    if (do_H) then
      call GADGOP(ECORR_SC(0,0),9*14,'+')
      call GADGOP(OVLAPS_SC(0,0),9*14,'+')
    else
      call GADGOP(ECORR_SC(0,0),9*12,'+')
      call GADGOP(OVLAPS_SC(0,0),9*12,'+')
    end if
  end if
#endif
...
subroutine SC_NEVPT2_Amplitude(NAS,NIS,iCase,iSym,lg_V)
  ...
  integer(kind=iwp) :: isp, ias, idisk, ILO, IHI, JLO, JHI
  real(kind=wp) :: scal
#ifdef _MOLCAS_MPP_
  integer(kind=iwp) :: myRank, LDV, MV
#endif
  ...
  if (is_real_par()) then
    ...
    if (iCase <= 11) then
      do isp = JLO, JHI
        ...
        do ias = ILO, IHI
          GA_Arrays(lg_V)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V)%A(ias+NAS*(isp-1))*scal
        end do
      end do
    else if (iCase == 12 .or. iCase == 13) then
      do isp = JLO, JHI
    ...
    do iSym = 1, nSym
      ...
      if (Is_Real_Par()) then
        ...
        if (ILO1 > 0 .and. JLO1 > 0 .and. ILO2 > 0 .and. JLO2 > 0) then
          ...
          if (IHI1-ILO1+1 /= NAS .or. ILO1 /= ILO2 .or. IHI1 /= IHI2 .or. JLO1 /= JLO2 .or. JHI1 /= JHI2 .or. LDV1 /= LDV2) then
            write (u6,'(1x,"Something is wrong in SC_NEVPT2_CLagD")')
            call abend()
          end if
          do isp = JLO1, JHI1
            vale = Zero
...
!             do jas = 1, ias-1
!               valh = valh + Two*DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*DBL_MB(MV1+jas-1+NAS*(isp-JLO1))*BMAT(ias*(ias-1)/2+jas)
!               valn = valn + Two*DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*DBL_MB(MV1+jas-1+NAS*(isp-JLO1))*SMAT(ias*(ias-1)/2+jas)
!             end do
!             valh = valh + DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*BMAT(ias*(ias-1)/2+ias)
!             valn = valn + DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*DBL_MB(MV1+ias-1+NAS*(isp-JLO1))*SMAT(ias*(ias-1)/2+ias)
!           end do
            if (abs(valn) <= SC_thres) then
              derHNS(isp,1) = Zero
              derHNS(isp,2) = Zero
        ...
        do isp = 1, NIS
          ...
          do ias = 1, NAS
            ...
            do jas = 1, ias-1
              valh = valh + Two*GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*BMAT(ias*(ias-1)/2+jas)
              valn = valn + Two*GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*SMAT(ias*(ias-1)/2+jas)
            end do
            valh = valh + GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*BMAT(ias*(ias-1)/2+ias)
            valn = valn + GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*GA_Arrays(lg_V1)%A(ias+NAS*(isp-1))*SMAT(ias*(ias-1)/2+ias)
          end do
          ...
          do isp = JLO1, JHI1
            ...
            do ias = 1, NAS
              do jas = 1, ias-1
                ...
                DBL_MB(idx_v3+jas) = DBL_MB(idx_v3+jas) + derHNS(isp,1)*DBL_MB(idx_v1+ias)*BMAT(ias*(ias-1)/2+jas)
                DBL_MB(idx_v3+jas) = DBL_MB(idx_v3+jas) + derHNS(isp,2)*DBL_MB(idx_v1+ias)*SMAT(ias*(ias-1)/2+jas)
                DBL_MB(idx_v3+ias) = DBL_MB(idx_v3+ias) + derHNS(isp,1)*DBL_MB(idx_v1+jas)*BMAT(ias*(ias-1)/2+jas)
                DBL_MB(idx_v3+ias) = DBL_MB(idx_v3+ias) + derHNS(isp,2)*DBL_MB(idx_v1+jas)*SMAT(ias*(ias-1)/2+jas)
              end do
              DBL_MB(idx_v3+ias) = DBL_MB(idx_v3+ias) + derHNS(isp,1)*DBL_MB(idx_v1+ias)*BMAT(ias*(ias-1)/2+ias)
              DBL_MB(idx_v3+ias) = DBL_MB(idx_v3+ias) + derHNS(isp,2)*DBL_MB(idx_v1+ias)*SMAT(ias*(ias-1)/2+ias)
...
!                   + derHNS(isp,1)*DBL_MB(MV1+jas-ILO1+LDV*(isp-JLO1))*BMAT(jas*(jas-1)/2+ias)
!                 DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) = DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) &
!                   + derHNS(isp,2)*DBL_MB(MV1+jas-ILO1+LDV*(isp-JLO1))*SMAT(jas*(jas-1)/2+ias)
!               else
!                 DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) = DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) &
!                   + derHNS(isp,1)*DBL_MB(MV1+jas-ILO1+LDV*(isp-JLO1))*BMAT(ias*(ias-1)/2+jas)
!                 DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) = DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) &
!                   + derHNS(isp,2)*DBL_MB(MV1+jas-ILO1+LDV*(isp-JLO1))*SMAT(ias*(ias-1)/2+jas)
!               end if
!             end do
!             ! <lambda|H|Psi0> term(s)
!             DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) = DBL_MB(MV3+ias-ILO1+LDV*(isp-JLO1)) &
            ...
            do jas = 1, NAS
              if (jas >= ias) then
                ...
                GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) &
                  + derHNS(isp,1)*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*BMAT(jas*(jas-1)/2+ias)
                ! <lambda|H0-E0|Psi1> terms
                GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) &
                  + derHNS(isp,2)*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*SMAT(jas*(jas-1)/2+ias)
              else
                GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) &
                  + derHNS(isp,1)*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*BMAT(ias*(ias-1)/2+jas)
                GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) &
                  + derHNS(isp,2)*GA_Arrays(lg_V1)%A(jas+NAS*(isp-1))*SMAT(ias*(ias-1)/2+jas)
              end if
            end do
            ! <lambda|H|Psi0> term(s)
            GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) = GA_Arrays(lg_V3)%A(ias+NAS*(isp-1)) &
...
subroutine SC_NEVPT2_res(VECROT)
  ...
  use caspt2_global, only: jStLag
  use caspt2_module, only: NASUP, NISUP, NSTATE, NSYM
  use Constants, only: Zero
  ...
  integer(kind=iwp) :: lg_V1, lg_V2, iCase, iStLag, iSym, NAS, NIS, NVEC
  real(kind=wp) :: scal
