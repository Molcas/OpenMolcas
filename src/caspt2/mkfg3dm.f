...
C-is basically the number of buffers we fill with EPQ_PSI vectors.
      iTask=1
      ibuf1=0
      DO ip1=1,nlev2
        itlev=idx2ij(1,ip1)
...
*         CALL EPQ_PSI(IULEV,ITLEV,One,STSYM,CI,BUF1(:,ibuf1))
         end if
        end do
        myBuffer=iTask
      ELSE
...
*     CALL EPQ_PSI(IYLEV,IZLEV,One,STSYM,CI,BUF2)
...
*       CALL EPQ_PSI(IVLEV,IXLEV,One,ISSG2,BUF2,BUFT)
*-----------
