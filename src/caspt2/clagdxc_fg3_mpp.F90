...
#ifdef _MOLCAS_MPP_
subroutine CLagDXC_FG3_MPP(ISYM,NASHT,NG3,lg_BDER,lg_SDER,DG1,DG2,DG3,DF1,DF2,DF3,DEPSA,G2,idxG3)
...
do IBLOCK=1,NBLOCKS
  ...
  do iG3=IG3STA,IG3END
    ...
    !if ((ISUP >= iLo) .and. (ISUP <= iHi)) then
    !  do iW=1,nAshT
    !    JSUP = KTUV(iX,iW,iZ)-nTUVES(iSYM)
    !    NSEQ = 1+ISUP-iLo+LDA*(JSUP-jLo)
    !    DEPSA(iW,iY) = DEPSA(iW,iY)-F3VAL*SC(ISUP-iLo,JSUP)
    !  end do
    !end if
...
subroutine empty_CLagDXC_FG3_MPP()
end subroutine empty_CLagDXC_FG3_MPP
#endif
